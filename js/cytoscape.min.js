...
var d,f,e,b,i;if(!this._private.panEnabled){return this}switch(c.length){case 1:if(a.is.plainObject(c[0])){e=c[0];b=e.x;i=e.y;if(a.is.number(b)){g.x+=b}if(a.is.number(i)){g.y+=i}this.trigger("pan")}break;case 2:d=c[0];f=c[1];if((d==="x"||d==="y")&&a.is.number(f)){g[d]+=f}this.trigger("pan");break;default:break}this.notify({type:"viewport"});return this},fit:function(g,i){if(a.is.number(g)&&i===undefined){i=g;g=undefined}if(!this._private.panEnabled||!this._private.zoomEnabled){return this}if(a.is.string(g)){var f=g;
g=this.$(f)}else{if(!a.is.elementOrCollection(g)){g=this.elements()}}var j=g.boundingBox();var d=this.style();var b=parseFloat(d.containerCss("width"));var c=parseFloat(d.containerCss("height"));var e;i=a.is.number(i)?i:0;if(!isNaN(b)&&!isNaN(c)){e=this._private.zoom=Math.min((b-2*i)/j.w,(c-2*i)/j.h);e=e>this._private.maxZoom?this._private.maxZoom:e;e=e<this._private.minZoom?this._private.minZoom:e;this._private.pan={x:(b-e*(j.x1+j.x2))/2,y:(c-e*(j.y1+j.y2))/2}}this.trigger("pan zoom");this.notify({type:"viewport"});
return this},minZoom:function(b){if(b===undefined){return this._private.minZoom}else{if(a.is.number(b)){this._private.minZoom=b}}return this},maxZoom:function(b){if(b===undefined){return this._private.maxZoom}else{if(a.is.number(b)){this._private.maxZoom=b}}return this},zoom:function(d){var j;var k;if(d===undefined){return this._private.zoom}else{if(a.is.number(d)){k=d;j={x:0,y:0}}else{if(a.is.plainObject(d)){k=d.level;if(d.renderedPosition){var l=d.renderedPosition;var b=this._private.pan;var i=this._private.zoom;
j={x:(l.x-b.x)/i,y:(l.y-b.y)/i}}else{if(d.position){j=d.position}}if(j&&!this._private.panEnabled){return this}}}}if(!this._private.zoomEnabled){return this}if(!a.is.number(k)||!a.is.number(j.x)||!a.is.number(j.y)){return this}k=k>this._private.maxZoom?this._private.maxZoom:k;k=k<this._private.minZoom?this._private.minZoom:k;var g=this._private.pan;var h=this._private.zoom;var f=k;var e={x:-f/h*(j.x-g.x)+j.x,y:-f/h*(j.y-g.y)+j.y};this._private.zoom=k;this._private.pan=e;var c=g.x!==e.x||g.y!==e.y;
this.trigger("zoom"+(c?" pan":""));this.notify({type:"viewport"});return this},boundingBox:function(c){var b=this.$(c);return b.boundingBox()},center:function(g){if(!this._private.panEnabled||!this._private.zoomEnabled){return this}if(a.is.string(g)){var b=g;g=cy.elements(b)}else{if(!a.is.elementOrCollection(g)){g=cy.elements()}}var i=g.boundingBox();var e=this.style();var c=parseFloat(e.containerCss("width"));var d=parseFloat(e.containerCss("height"));var f=this._private.zoom;this.pan({x:(c-f*(i.x1+i.x2))/2,y:(d-f*(i.y1+i.y2))/2});
this.trigger("pan");this.notify({type:"viewport"});return this},reset:function(){if(!this._private.panEnabled||!this._private.zoomEnabled){return this}this.pan({x:0,y:0});if(this._private.maxZoom>1&&this._private.minZoom<1){this.zoom(1)}this.notify({type:"viewport"});return this}})})(cytoscape);(function(b){b.fn.collection=b.fn.eles=function(c,e){for(var d in c){var f=c[d];b.Collection.prototype[d]=f}};var a={prefix:{nodes:"n",edges:"e"},id:{nodes:0,edges:0},generate:function(h,d,g){var c=b.is.element(d)?d._private:d;
var e=c.group;var f=g!=null?g:this.prefix[e]+this.id[e];if(h.getElementById(f).empty()){this.id[e]++}else{while(!h.getElementById(f).empty()){f=this.prefix[e]+(++this.id[e])}}return f}};b.Element=function(d,f,h){if(!(this instanceof b.Element)){return new b.Element(d,f,h)}var o=this;h=(h===undefined||h?true:false);if(d===undefined||f===undefined||!b.is.core(d)){b.util.error("An element must have a core reference and parameters set");return}if(f.group!=="nodes"&&f.group!=="edges"){b.util.error("An element must be of type `nodes` or `edges`; you specified `"+f.group+"`");
return}this.length=1;this[0]=this;this._private={cy:d,single:true,data:f.data||{},position:f.position||{},autoWidth:undefined,autoHeight:undefined,listeners:[],group:f.group,style:{},rstyle:{},styleCxts:[],removed:true,selected:f.selected?true:false,selectable:f.selectable===undefined?true:(f.selectable?true:false),locked:f.locked?true:false,grabbed:false,grabbable:f.grabbable===undefined?true:(f.grabbable?true:false),active:false,classes:{},animation:{current:[],queue:[]},rscratch:{},scratch:{},edges:[],children:[]};
...
})(cytoscape);(function(z){var w=("ontouchstart" in window)||window.DocumentTouch&&document instanceof DocumentTouch;var m=function(){return Date.now()};var b={};var r={};var c=A.prototype;var p=400;var e=5,E=0,d=2,v=3,H=4,a=2;function A(J){this.options=J;this.data={select:[undefined,undefined,undefined,undefined,0],renderer:this,cy:J.cy,container:J.cy.container(),canvases:new Array(e),canvasRedrawReason:new Array(e),canvasNeedsRedraw:new Array(e),bufferCanvases:new Array(a)};this.hoverData={down:null,last:null,downTime:null,triggerMode:null,dragging:false,initialPan:[null,null],capture:false};
this.timeoutData={panTimeout:null};this.dragData={possibleDragElements:[]};this.touchData={start:null,capture:false,startPosition:[null,null,null,null,null,null],singleTouchStartTime:null,singleTouchMoved:true,now:[null,null,null,null,null,null],earlier:[null,null,null,null,null,null]};this.zoomData={freeToZoom:false,lastPointerX:null};this.redraws=0;this.bindings=[];this.init();for(var K=0;K<e;K++){this.data.canvases[K]=document.createElement("canvas");this.data.canvases[K].style.position="absolute";
this.data.canvases[K].setAttribute("data-id","layer"+K);this.data.canvases[K].style.zIndex=String(e-K);this.data.container.appendChild(this.data.canvases[K]);this.data.canvasRedrawReason[K]=new Array();this.data.canvasNeedsRedraw[K]=false}this.data.canvases[H].setAttribute("data-id","layer"+H+"-node");this.data.canvases[E].setAttribute("data-id","layer"+E+"-selectbox");this.data.canvases[d].setAttribute("data-id","layer"+d+"-drag");this.data.canvases[v].setAttribute("data-id","layer"+v+"-overlay");
for(var K=0;K<a;K++){this.data.bufferCanvases[K]=document.createElement("canvas");this.data.bufferCanvases[K].style.position="absolute";this.data.bufferCanvases[K].setAttribute("data-id","buffer"+K);this.data.bufferCanvases[K].style.zIndex=String(-K-1);this.data.bufferCanvases[K].style.visibility="visible";this.data.container.appendChild(this.data.bufferCanvases[K])}var I=document.createElement("div");this.data.container.appendChild(I);this.data.overlay=I;I.style.position="absolute";I.style.zIndex=1000;
if(J.showOverlay){var L=document.createElement("a");I.appendChild(L);this.data.link=L;L.innerHTML="cytoscape.js";L.style.font="14px helvetica";L.style.position="absolute";L.style.right=0;L.style.bottom=0;L.style.padding="1px 3px";L.style.paddingLeft="5px";L.style.paddingTop="5px";L.style.opacity=0;L.style["-webkit-tap-highlight-color"]="transparent";L.style.background="red";L.href="http://cytoscape.github.io/cytoscape.js/";L.target="_blank"}this.hideEdgesOnViewport=J.hideEdgesOnViewport;this.load()
}A.prototype.notify=function(i){if(i.type=="destroy"){this.destroy();return}else{if(i.type=="add"||i.type=="remove"||i.type=="load"){this.updateNodesCache();this.updateEdgesCache()}}if(i.type=="viewport"){this.data.canvasNeedsRedraw[E]=true;this.data.canvasRedrawReason[E].push("viewchange")}this.data.canvasNeedsRedraw[d]=true;this.data.canvasRedrawReason[d].push("notify");this.data.canvasNeedsRedraw[H]=true;this.data.canvasRedrawReason[H].push("notify");this.redraws++;this.redraw()};A.prototype.registerBinding=function(K,J,I,i){this.bindings.push({target:K,event:J,handler:I,useCapture:i});
K.addEventListener(J,I,i)};A.prototype.destroy=function(){this.destroyed=true;for(var J=0;J<this.bindings.length;J++){var K=this.bindings[J];var I=K;I.target.removeEventListener(I.event,I.handler,I.useCapture)}};A.prototype.png=function(){var I=this.data;if(this.data.container.clientHeight>0&&this.data.container.clientWidth>0){context=I.bufferCanvases[1].getContext("2d");context.globalCompositeOperation="copy";context.drawImage(I.canvases[4],0,0);context.globalCompositeOperation="source-over";context.drawImage(I.canvases[2],0,0);
context.drawImage(I.canvases[0],0,0);context=I.bufferCanvases[0].getContext("2d");context.globalCompositeOperation="copy";context.drawImage(I.bufferCanvases[1],0,0)}var i=this.data.bufferCanvases[0];return i.toDataURL("image/png")};A.prototype.load=function(){var S=this;var Q=function(ae,Z,af){if(!Z){var ac=ae.parents();for(var ad=0;ad<ac.size();ad++){if(ac[ad]._private.selected){return}}}var ag=ae.descendants();function ab(ai){while(ai.parent().nonempty()&&ai.parent().id()!==ae.id()){parent=ai.parent()[0];
var ah=parent._private.style;if(ah.width.value!=="auto"||ah.height.value!=="auto"){return true}ai=ai.parent()}return false}for(var ad=0;ad<ag.size();ad++){if((Z||!ag[ad]._private.selected)){ag[ad]._private.rscratch.inDragLayer=true;af.push(ag[ad]);for(var aa=0;aa<ag[ad]._private.edges.length;aa++){ag[ad]._private.edges[aa]._private.rscratch.inDragLayer=true}}}};var T=function(aa,ab){aa._private.grabbed=true;aa._private.rscratch.inDragLayer=true;ab.push(aa);for(var Z=0;Z<aa._private.edges.length;Z++){aa._private.edges[Z]._private.rscratch.inDragLayer=true
}};var X=function(ae,ac){var ad=ae;while(ad.parent().nonempty()){ad=ad.parent()[0]}if(ad==ae&&ac){return}var Z=ad.descendants().add(ad);for(var ab=0;ab<Z.size();ab++){Z[ab]._private.rscratch.inDragLayer=ac;for(var aa=0;aa<Z[ab]._private.edges.length;aa++){Z[ab]._private.edges[aa]._private.rscratch.inDragLayer=ac}}};A.prototype.nodeIsDraggable=function(Z){if(Z._private.style.opacity.value!=0&&Z._private.style.visibility.value=="visible"&&!Z._private.locked&&Z._private.grabbable){return true}return false
};S.registerBinding(window,"resize",function(Z){S.data.canvasNeedsRedraw[H]=true;S.data.canvasNeedsRedraw[v]=true;S.matchCanvasSize(S.data.container);S.redraw()},true);S.registerBinding(S.data.container,"contextmenu",function(Z){Z.preventDefault()});S.registerBinding(S.data.container,"mousedown",function(ac){ac.preventDefault();S.hoverData.capture=true;S.hoverData.which=ac.which;var Z=S.data.cy;var af=S.projectIntoViewport(ac.pageX,ac.pageY);var ag=S.data.select;var ab=S.findNearestElement(af[0],af[1],true);
var ai=S.hoverData.down;var ad=S.dragData.possibleDragElements;var ae=new z.Event(ac,{type:"grab"});if(ac.which==3){if(ab){ab.activate();ab.trigger(new z.Event(ac,{type:"cxttapstart"}));S.hoverData.down=ab;S.hoverData.downTime=(new Date()).getTime();S.hoverData.cxtDragged=false}}else{if(ac.which==1){if(ab){ab.activate()}if(ab!=null&&S.nodeIsDraggable(ab)){if(ab._private.group=="nodes"&&ab._private.selected==false){ad=S.dragData.possibleDragElements=[];T(ab,ad);ab.trigger(ae);if(ab._private.style.width.value=="auto"||ab._private.style.height.value=="auto"){Q(ab,true,ad)
}X(ab,true)}if(ab._private.group=="nodes"&&ab._private.selected==true){ad=S.dragData.possibleDragElements=[];var aj=false;var ah=Z.$("node:selected");for(var aa=0;aa<ah.length;aa++){if(S.nodeIsDraggable(ah[aa])){T(ah[aa],ad);if(!aj){ab.trigger(ae);aj=true}if(ah[aa]._private.style.width.value=="auto"||ah[aa]._private.style.height.value=="auto"){Q(ah[aa],false,ad)}X(ah[aa],true)}}}ab.trigger(new z.Event(ac,{type:"mousedown"})).trigger(new z.Event(ac,{type:"tapstart"})).trigger(new z.Event(ac,{type:"vmousedown"}))
}else{if(ab==null){Z.trigger(new z.Event(ac,{type:"mousedown"})).trigger(new z.Event(ac,{type:"tapstart"})).trigger(new z.Event(ac,{type:"vmousedown"}))}}S.hoverData.down=ab;S.hoverData.downTime=(new Date()).getTime();if(ab==null||ab.isEdge()){ag[4]=1;var ak=Math.max(0,p-(+new Date-S.hoverData.downTime));clearTimeout(S.bgActiveTimeout);S.bgActiveTimeout=setTimeout(function(){if(ab){ab.unactivate()}S.data.bgActivePosistion={x:af[0],y:af[1]};S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("bgactive");
S.redraw()},ak)}}}ag[0]=ag[2]=af[0];ag[1]=ag[3]=af[1];S.redraw()},false);S.registerBinding(window,"mousemove",function(aq){var ag=false;var am=S.hoverData.capture;if(!am){var ah=S.findContainerPageCoords();if(aq.pageX>ah[0]&&aq.pageX<ah[0]+S.data.container.clientWidth&&aq.pageY>ah[1]&&aq.pageY<ah[1]+S.data.container.clientHeight){}else{return}}var ab=S.data.cy;var af=S.projectIntoViewport(aq.pageX,aq.pageY);var ak=S.data.select;var ar=S.findNearestElement(af[0],af[1],true);var ai=S.hoverData.last;
var aj=S.hoverData.down;var an=[af[0]-ak[2],af[1]-ak[3]];var al=S.getCachedNodes();var aa=S.getCachedEdges();var ad=S.dragData.possibleDragElements;var ac=aq.shiftKey;ag=true;var ao=new z.Event(aq,{type:"mousemove"});if(ar!=null){ar.trigger(ao)}else{if(ar==null){ab.trigger(ao)}}if(S.hoverData.which===3){var ae=new z.Event(aq,{type:"cxtdrag"});if(aj){aj.trigger(ae)}else{ab.trigger(ae)}S.hoverData.cxtDragged=true}else{if(S.hoverData.dragging){ag=true;if(ab.panningEnabled()){var Z={x:an[0]*ab.zoom(),y:an[1]*ab.zoom()};
ab.panBy(Z)}af=S.projectIntoViewport(aq.pageX,aq.pageY)}else{if(ak[4]==1&&(aj==null||aj.isEdge())&&(!ab.boxSelectionEnabled()||+new Date-S.hoverData.downTime>=p)&&(Math.abs(ak[3]-ak[1])+Math.abs(ak[2]-ak[0])<4)&&ab.panningEnabled()){S.hoverData.dragging=true;ak[4]=0}else{if(ab.boxSelectionEnabled()&&Math.pow(ak[2]-ak[0],2)+Math.pow(ak[3]-ak[1],2)>7&&ak[4]){clearTimeout(S.bgActiveTimeout)}if(aj&&aj.isEdge()&&aj.active()){aj.unactivate()}if(ar!=ai){if(ai){ai.trigger(new z.Event(aq,{type:"mouseout"}))
}if(ar){ar.trigger(new z.Event(aq,{type:"mouseover"}))}S.hoverData.last=ar}if(aj&&aj.isNode()&&S.nodeIsDraggable(aj)){S.dragData.didDrag=true;var at=[];for(var ap=0;ap<ad.length;ap++){if(ad[ap]._private.group=="nodes"&&S.nodeIsDraggable(ad[ap])){ad[ap]._private.position.x+=an[0];ad[ap]._private.position.y+=an[1];at.push(ad[ap])}}(new z.Collection(ab,at)).trigger(new z.Event(aq,{type:"drag"})).trigger(new z.Event(aq,{type:"position"}));if(ak[2]==ak[0]&&ak[3]==ak[1]){S.data.canvasNeedsRedraw[H]=true;
S.data.canvasRedrawReason[H].push("Node(s) and edge(s) moved to drag layer")}S.data.canvasNeedsRedraw[d]=true;S.data.canvasRedrawReason[d].push("Nodes dragged")}if(ab.boxSelectionEnabled()){S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("Mouse moved, redraw selection box")}ag=true}}}ak[2]=af[0];ak[3]=af[1];S.redraw();if(ag){if(aq.stopPropagation){aq.stopPropagation()}if(aq.preventDefault){aq.preventDefault()}aq.cancelBubble=true;aq.returnValue=false;return false}},false);S.registerBinding(window,"mouseup",function(au){var an=S.hoverData.capture;
if(!an){return}S.hoverData.capture=false;var aa=S.data.cy;var af=S.projectIntoViewport(au.pageX,au.pageY);var al=S.data.select;var av=S.findNearestElement(af[0],af[1],true);var am=S.getCachedNodes();var Z=S.getCachedEdges();var ad=S.dragData.possibleDragElements;var ah=S.hoverData.down;var ac=au.shiftKey;S.data.bgActivePosistion=undefined;clearTimeout(S.bgActiveTimeout);if(ah){ah.unactivate()}if(S.hoverData.which===3){var ae=new z.Event(au,{type:"cxttapend"});if(ah){ah.trigger(ae)}else{aa.trigger(ae)
}if(!S.hoverData.cxtDragged){var ak=new z.Event(au,{type:"cxttap"});if(ah){ah.trigger(ak)}else{aa.trigger(ak)}}S.hoverData.cxtDragged=false;S.hoverData.which=null}else{if((ah==null)&&!S.dragData.didDrag&&!(Math.pow(al[2]-al[0],2)+Math.pow(al[3]-al[1],2)>7&&al[4])&&!S.hoverData.dragging){aa.$(":selected").unselect();if(ad.length>0){S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("De-select")}S.dragData.possibleDragElements=ad=[]}if(Math.pow(al[2]-al[0],2)+Math.pow(al[3]-al[1],2)==0){if(av!=null){av.trigger(new z.Event(au,{type:"click"})).trigger(new z.Event(au,{type:"tap"})).trigger(new z.Event(au,{type:"vclick"}))
}else{if(av==null){aa.trigger(new z.Event(au,{type:"click"})).trigger(new z.Event(au,{type:"tap"})).trigger(new z.Event(au,{type:"vclick"}))}}}if(av!=null){av.trigger(new z.Event(au,{type:"mouseup"})).trigger(new z.Event(au,{type:"tapend"})).trigger(new z.Event(au,{type:"vmouseup"}))}else{if(av==null){aa.trigger(new z.Event(au,{type:"mouseup"})).trigger(new z.Event(au,{type:"tapend"})).trigger(new z.Event(au,{type:"vmouseup"}))}}if(av==ah&&!S.dragData.didDrag){if(av!=null&&av._private.selectable){if(!ac){aa.$(":selected").unselect()
}if(av.selected()){av.unselect()}else{av.select()}X(av,false);S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("sglslct")}}else{if(av==ah){if(av!=null&&av._private.grabbed){var ao=aa.$(":grabbed");for(var ar=0;ar<ao.length;ar++){var ai=ao[ar];ai._private.grabbed=false;var aj=ai._private.edges;for(var aq=0;aq<aj.length;aq++){aj[aq]._private.rscratch.inDragLayer=false}X(ai,false)}var at=new z.Event(au,{type:"free"});ao.trigger(at)}}}if(aa.boxSelectionEnabled()&&Math.pow(al[2]-al[0],2)+Math.pow(al[3]-al[1],2)>7&&al[4]){if(!ac){aa.$(":selected").unselect()
}var ab=[];var ag=S.getAllInBox(al[0],al[1],al[2],al[3]);var ap=new z.Event(au,{type:"select"});for(var ar=0;ar<ag.length;ar++){if(ag[ar]._private.selectable){ad.push(ag[ar]);ab.push(ag[ar])}}(new z.Collection(aa,ab)).select();if(ag.length>0){S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("Selection")}}S.hoverData.dragging=false;if(!al[4]){var at=new z.Event(au,{type:"free"});for(var ar=0;ar<ad.length;ar++){if(ad[ar]._private.group=="nodes"){ad[ar]._private.rscratch.inDragLayer=false;
var aj=ad[ar]._private.edges;for(var aq=0;aq<aj.length;aq++){aj[aq]._private.rscratch.inDragLayer=false}X(ad[ar],false)}else{if(ad[ar]._private.group=="edges"){ad[ar]._private.rscratch.inDragLayer=false}}}if(ah){ah.trigger(at)}S.data.canvasNeedsRedraw[d]=true;S.data.canvasRedrawReason[d].push("Node/nodes back from drag");S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("Node/nodes back from drag")}}al[4]=0;S.hoverData.down=null;S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("Mouse up, selection box gone");
S.dragData.didDrag=false;S.redraw()},false);var N=function(ab){var ad=S.data.cy;var ac=S.projectIntoViewport(ab.pageX,ab.pageY);var aa=[ac[0]*ad.zoom()+ad.pan().x,ac[1]*ad.zoom()+ad.pan().y];if(S.zoomData.freeToZoom){ab.preventDefault();var Z=ab.wheelDeltaY/1000||ab.detail/-32;if(ad.panningEnabled()&&ad.zoomingEnabled()){ad.zoom({level:ad.zoom()*Math.pow(10,Z),position:{x:aa[0],y:aa[1]}})}S.data.wheel=true;clearTimeout(S.data.wheelTimeout);S.data.wheelTimeout=setTimeout(function(){S.data.wheel=false;
S.data.canvasNeedsRedraw[H]=true;S.redraw()},100)}};S.registerBinding(S.data.container,"mousewheel",N,true);S.registerBinding(S.data.container,"DOMMouseScroll",N,true);S.registerBinding(S.data.container,"MozMousePixelScroll",function(Z){if(S.zoomData.freeToZoom){Z.preventDefault()}},false);S.registerBinding(S.data.container,"mousemove",function(Z){if(S.zoomData.lastPointerX&&S.zoomData.lastPointerX!=Z.pageX&&!S.zoomData.freeToZoom){S.zoomData.freeToZoom=true}S.zoomData.lastPointerX=Z.pageX},false);
S.registerBinding(S.data.container,"mouseout",function(Z){S.zoomData.freeToZoom=false;S.zoomData.lastPointerX=null},false);S.registerBinding(S.data.container,"mouseout",function(Z){S.data.cy.trigger(new z.Event(Z,{type:"mouseout"}))},false);S.registerBinding(S.data.container,"mouseover",function(Z){S.data.cy.trigger(new z.Event(Z,{type:"mouseover"}))},false);var I,O,U,Y;var V;var L,K;var M,W;var P=S.data.container.clientWidth,R=S.data.container.clientHeight;var i;function J(aa,ac,Z,ab){return Math.sqrt((Z-aa)*(Z-aa)+(ab-ac)*(ab-ac))
}S.registerBinding(S.data.container,"touchstart",function(av){if(av.target!==S.data.link){av.preventDefault()}S.touchData.capture=true;S.data.bgActivePosistion=undefined;var ac=S.data.cy;var an=S.getCachedNodes();var ab=S.getCachedEdges();var aa=S.touchData.now;var ar=S.touchData.earlier;if(av.touches[0]){var af=S.projectIntoViewport(av.touches[0].pageX,av.touches[0].pageY);aa[0]=af[0];aa[1]=af[1]}if(av.touches[1]){var af=S.projectIntoViewport(av.touches[1].pageX,av.touches[1].pageY);aa[2]=af[0];
aa[3]=af[1]}if(av.touches[2]){var af=S.projectIntoViewport(av.touches[2].pageX,av.touches[2].pageY);aa[4]=af[0];aa[5]=af[1]}if(av.touches[1]){function au(ax){for(var ay=0;ay<ax.length;ay++){ax[ay]._private.grabbed=false;ax[ay]._private.rscratch.inDragLayer=false;if(ax[ay].active()){ax[ay].unactivate()}}}au(an);au(ab);var ae=S.findContainerPageCoords();W=ae[1];M=ae[0];I=av.touches[0].pageX-M;O=av.touches[0].pageY-W;U=av.touches[1].pageX-M;Y=av.touches[1].pageY-W;i=0<=I&&I<=P&&0<=U&&U<=P&&0<=O&&O<=R&&0<=Y&&Y<=R;
var am=ac.pan();var Z=ac.zoom();V=J(I,O,U,Y);L=[(I+U)/2,(O+Y)/2];K=[(L[0]-am.x)/Z,(L[1]-am.y)/Z];if(V<100){var ah=S.findNearestElement(aa[0],aa[1],true);var ag=S.findNearestElement(aa[2],aa[3],true);var ad=new z.Event(av,{type:"cxttapstart"});if(ah&&ah.isNode()){ah.activate().trigger(ad);S.touchData.start=ah}else{if(ag&&ag.isNode()){ag.activate().trigger(ad);S.touchData.start=ag}else{ac.trigger(ad);S.touchData.start=null}}if(S.touchData.start){S.touchData.start._private.grabbed=false}S.touchData.cxt=true;
S.touchData.cxtDragged=false;S.data.bgActivePosistion=undefined;S.redraw();return}}if(av.touches[2]){}else{if(av.touches[1]){}else{if(av.touches[0]){var aw=S.findNearestElement(aa[0],aa[1],true);if(aw!=null){aw.activate();S.touchData.start=aw;if(aw._private.group=="nodes"&&S.nodeIsDraggable(aw)){var ai=S.dragData.touchDragEles=[];T(aw,ai);aw.trigger(new z.Event(av,{type:"grab"}));if(aw.selected()){ai=S.dragData.touchDragEles=[];var ak=ac.$("node:selected");for(var ao=0;ao<ak.length;ao++){var at=ak[ao];
if(S.nodeIsDraggable(at)){ai.push(at);at._private.rscratch.inDragLayer=true;var aj=at._private.edges;for(var ap=0;ap<aj.length;ap++){aj[ap]._private.rscratch.inDragLayer=true}if(at._private.style.width.value=="auto"||at._private.style.height.value=="auto"){Q(at,false,ai)}X(at,true)}}}else{if(aw._private.style.width.value=="auto"||aw._private.style.height.value=="auto"){Q(aw,true,ai)}X(aw,true)}}aw.trigger(new z.Event(av,{type:"touchstart"})).trigger(new z.Event(av,{type:"tapstart"})).trigger(new z.Event(av,{type:"vmousdown"}))
}if(aw==null){ac.trigger(new z.Event(av,{type:"touchstart"})).trigger(new z.Event(av,{type:"tapstart"})).trigger(new z.Event(av,{type:"vmousedown"}));S.data.bgActivePosistion={x:af[0],y:af[1]};S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("bgactive")}for(var aq=0;aq<aa.length;aq++){ar[aq]=aa[aq];S.touchData.startPosition[aq]=aa[aq]}S.touchData.singleTouchMoved=false;S.touchData.singleTouchStartTime=m();var al=setTimeout(function(){if(S.touchData.singleTouchMoved==false&&m()-S.touchData.singleTouchStartTime>250){if(S.touchData.start){S.touchData.start.trigger(new z.Event(av,{type:"taphold"}))
}else{S.data.cy.trigger(new z.Event(av,{type:"taphold"}));ac.$(":selected").unselect()}}},1000)}}}S.redraw()},false);S.registerBinding(window,"touchmove",function(aI){var aB=S.data.select;var ao=S.touchData.capture;ao&&aI.preventDefault();var am=S.data.cy;var ax=S.getCachedNodes();var ag=S.getCachedEdges();var aA=S.touchData.now;var aa=S.touchData.earlier;if(aI.touches[0]){var ar=S.projectIntoViewport(aI.touches[0].pageX,aI.touches[0].pageY);aA[0]=ar[0];aA[1]=ar[1]}if(aI.touches[1]){var ar=S.projectIntoViewport(aI.touches[1].pageX,aI.touches[1].pageY);
aA[2]=ar[0];aA[3]=ar[1]}if(aI.touches[2]){var ar=S.projectIntoViewport(aI.touches[2].pageX,aI.touches[2].pageY);aA[4]=ar[0];aA[5]=ar[1]}var ac=[];for(var aF=0;aF<aA.length;aF++){ac[aF]=aA[aF]-aa[aF]}if(ao&&S.touchData.cxt){var aq=aI.touches[0].pageX-M,ad=aI.touches[0].pageY-W;var al=aI.touches[1].pageX-M,Z=aI.touches[1].pageY-W;var av=J(aq,ad,al,Z);var aG=av/V;if(aG>=1.5||av>=150){S.touchData.cxt=false;if(S.touchData.start){S.touchData.start.unactivate();S.touchData.start=null}S.data.bgActivePosistion=undefined;
S.data.canvasNeedsRedraw[E]=true;var ab=new z.Event(aI,{type:"cxttapend"});if(S.touchData.start){S.touchData.start.trigger(ab)}else{am.trigger(ab)}}}if(ao&&S.touchData.cxt){var ab=new z.Event(aI,{type:"cxtdrag"});S.data.bgActivePosistion=undefined;S.data.canvasNeedsRedraw[E]=true;if(S.touchData.start){S.touchData.start.trigger(ab)}else{am.trigger(ab)}if(S.touchData.start){S.touchData.start._private.grabbed=false}S.touchData.cxtDragged=true}else{if(ao&&aI.touches[2]&&am.boxSelectionEnabled()){S.data.bgActivePosistion=undefined;
clearTimeout(this.threeFingerSelectTimeout);this.lastThreeTouch=+new Date;S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("Touch moved, redraw selection box");if(!aB||aB.length===0||aB[0]===undefined){aB[0]=(aA[0]+aA[2]+aA[4])/3;aB[1]=(aA[1]+aA[3]+aA[5])/3;aB[2]=(aA[0]+aA[2]+aA[4])/3+1;aB[3]=(aA[1]+aA[3]+aA[5])/3+1}else{aB[2]=(aA[0]+aA[2]+aA[4])/3;aB[3]=(aA[1]+aA[3]+aA[5])/3}aB[4]=1}else{if(ao&&aI.touches[1]&&am.zoomingEnabled()&&am.panningEnabled()){S.data.bgActivePosistion=undefined;
S.data.canvasNeedsRedraw[E]=true;var aq=aI.touches[0].pageX-M,ad=aI.touches[0].pageY-W;var al=aI.touches[1].pageX-M,Z=aI.touches[1].pageY-W;var av=J(aq,ad,al,Z);var aG=av/V;if(aG!=1&&i){var aK=aq-I;var aJ=ad-O;var aw=al-U;var au=Z-Y;var aE=(aK+aw)/2;var aC=(aJ+au)/2;var af=am.zoom();var ae=af*aG;var ai=am.pan();var az=K[0]*af+ai.x;var ay=K[1]*af+ai.y;var ah={x:-ae/af*(az-ai.x-aE)+az,y:-ae/af*(ay-ai.y-aC)+ay};am._private.zoom=ae;am._private.pan=ah;am.trigger("pan zoom").notify("viewport");V=av;I=aq;
O=ad;U=al;Y=Z;S.pinching=true}if(aI.touches[0]){var ar=S.projectIntoViewport(aI.touches[0].pageX,aI.touches[0].pageY);aA[0]=ar[0];aA[1]=ar[1]}if(aI.touches[1]){var ar=S.projectIntoViewport(aI.touches[1].pageX,aI.touches[1].pageY);aA[2]=ar[0];aA[3]=ar[1]}if(aI.touches[2]){var ar=S.projectIntoViewport(aI.touches[2].pageX,aI.touches[2].pageY);aA[4]=ar[0];aA[5]=ar[1]}}else{if(aI.touches[0]){var ap=S.touchData.start;var aj=S.touchData.last;if(ap!=null&&ap._private.group=="nodes"&&S.nodeIsDraggable(ap)){var ak=S.dragData.touchDragEles;
for(var aD=0;aD<ak.length;aD++){var an=ak[aD];if(S.nodeIsDraggable(an)){S.dragData.didDrag=true;an._private.position.x+=ac[0];an._private.position.y+=ac[1]}}(new z.Collection(am,ak)).trigger(new z.Event(aI,{type:"drag"})).trigger(new z.Event(aI,{type:"position"}));S.data.canvasNeedsRedraw[d]=true;S.data.canvasRedrawReason[d].push("touchdrag node");if(S.touchData.startPosition[0]==aa[0]&&S.touchData.startPosition[1]==aa[1]){S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("node drag started")
}}if(ap!=null){ap.trigger(new z.Event(aI,{type:"touchmove"}))}if(ap==null){var at=S.findNearestElement(aA[0],aA[1],true);if(at!=null){at.trigger(new z.Event(aI,{type:"touchmove"}))}if(at==null){am.trigger(new z.Event(aI,{type:"touchmove"}))}}if(at!=aj){if(aj){aj.trigger(new z.Event(aI,{type:"touchout"}))}if(at){at.trigger(new z.Event(aI,{type:"touchover"}))}}S.touchData.last=at;for(var aH=0;aH<aA.length;aH++){if(aA[aH]&&S.touchData.startPosition[aH]&&Math.abs(aA[aH]-S.touchData.startPosition[aH])>4){S.touchData.singleTouchMoved=true
}}if(ao&&(ap==null||ap.isEdge())&&am.panningEnabled()){if(ap){ap.unactivate();if(!S.data.bgActivePosistion){S.data.bgActivePosistion={x:aA[0],y:aA[1]}}S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("bgactive")}am.panBy({x:ac[0]*am.zoom(),y:ac[1]*am.zoom()});S.swipePanning=true;var ar=S.projectIntoViewport(aI.touches[0].pageX,aI.touches[0].pageY);aA[0]=ar[0];aA[1]=ar[1]}}}}}for(var aF=0;aF<aA.length;aF++){aa[aF]=aA[aF]}S.redraw()},false);S.registerBinding(window,"touchend",function(ar){var ak=S.touchData.capture;
if(!ak){return}S.touchData.capture=false;ar.preventDefault();var ah=S.data.select;S.swipePanning=false;var ac=S.data.cy;var aj=S.getCachedNodes();var ab=S.getCachedEdges();var aa=S.touchData.now;var ao=S.touchData.earlier;var ad=S.touchData.start;if(ar.touches[0]){var ae=S.projectIntoViewport(ar.touches[0].pageX,ar.touches[0].pageY);aa[0]=ae[0];aa[1]=ae[1]}if(ar.touches[1]){var ae=S.projectIntoViewport(ar.touches[1].pageX,ar.touches[1].pageY);aa[2]=ae[0];aa[3]=ae[1]}if(ar.touches[2]){var ae=S.projectIntoViewport(ar.touches[2].pageX,ar.touches[2].pageY);
aa[4]=ae[0];aa[5]=ae[1]}if(S.touchData.cxt){ctxTapend=new z.Event(ar,{type:"cxttapend"});if(ad){ad.unactivate();ad.trigger(ctxTapend)}else{ac.trigger(ctxTapend)}if(!S.touchData.cxtDragged){var ai=new z.Event(ar,{type:"cxttap"});if(ad){ad.trigger(ai)}else{ac.trigger(ai)}}if(S.touchData.start){S.touchData.start._private.grabbed=false}S.touchData.cxt=false;S.touchData.start=null;S.redraw();return}var Z=+new Date;if(!ar.touches[2]&&ac.boxSelectionEnabled()){clearTimeout(this.threeFingerSelectTimeout);
this.threeFingerSelectTimeout=setTimeout(function(){var at=[];var aw=S.getAllInBox(ah[0],ah[1],ah[2],ah[3]);ah[0]=undefined;ah[1]=undefined;ah[2]=undefined;ah[3]=undefined;ah[4]=0;S.data.canvasNeedsRedraw[E]=true;S.data.canvasRedrawReason[E].push("Touch moved, redraw selection box");var av=new z.Event(ar,{type:"select"});for(var au=0;au<aw.length;au++){if(aw[au]._private.selectable){at.push(aw[au])}}(new z.Collection(ac,at)).select();if(aw.length>0){S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("Selection")
}},100)}if(!ar.touches[1]){S.pinching=false}var am=false;if(ad!=null){ad._private.active=false;am=true;ad.trigger(new z.Event(ar,{type:"unactivate"}))}if(ar.touches[2]){S.data.bgActivePosistion=undefined}else{if(ar.touches[1]){}else{if(ar.touches[0]){}else{if(!ar.touches[0]){S.data.bgActivePosistion=undefined;if(ad!=null){if(ad._private.grabbed==true){ad._private.grabbed=false;ad.trigger(new z.Event(ar,{type:"free"}));ad._private.rscratch.inDragLayer=false}var ag=ad._private.edges;for(var an=0;an<ag.length;
an++){ag[an]._private.rscratch.inDragLayer=false}X(ad,false);if(ad.selected()){var af=ac.$("node:selected");for(var al=0;al<af.length;al++){var ap=af[al];ap._private.rscratch.inDragLayer=false;var ag=ap._private.edges;for(var an=0;an<ag.length;an++){ag[an]._private.rscratch.inDragLayer=false}X(ap,false)}}S.data.canvasNeedsRedraw[d]=true;S.data.canvasRedrawReason[d].push("touchdrag node end");S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("touchdrag node end");ad.trigger(new z.Event(ar,{type:"touchend"})).trigger(new z.Event(ar,{type:"tapend"})).trigger(new z.Event(ar,{type:"vmouseup"}));
S.touchData.start=null}else{var aq=S.findNearestElement(aa[0],aa[1],true);if(aq!=null){aq.trigger(new z.Event(ar,{type:"touchend"})).trigger(new z.Event(ar,{type:"tapend"})).trigger(new z.Event(ar,{type:"vmouseup"}))}if(aq==null){ac.trigger(new z.Event(ar,{type:"touchend"})).trigger(new z.Event(ar,{type:"tapend"})).trigger(new z.Event(ar,{type:"vmouseup"}))}}if(ad!=null&&!S.dragData.didDrag&&ad._private.selectable&&(Math.sqrt(Math.pow(S.touchData.startPosition[0]-aa[0],2)+Math.pow(S.touchData.startPosition[1]-aa[1],2)))<6){if(ad.selected()){ad._private.selected=false;
ad.trigger(new z.Event(ar,{type:"unselect"}))}else{ad._private.selected=true;ad.trigger(new z.Event(ar,{type:"select"}))}am=true;S.data.canvasNeedsRedraw[H]=true;S.data.canvasRedrawReason[H].push("sglslct")}if(S.touchData.singleTouchMoved==false){if(ad){ad.trigger(new z.Event(ar,{type:"tap"})).trigger(new z.Event(ar,{type:"vclick"}))}else{ac.trigger(new z.Event(ar,{type:"tap"})).trigger(new z.Event(ar,{type:"vclick"}))}}S.touchData.singleTouchMoved=true}}}}for(var an=0;an<aa.length;an++){ao[an]=aa[an]
}S.dragData.didDrag=false;if(am&&ad){ad.updateStyle(false)}S.redraw()},false)};A.prototype.init=function(){};A.prototype.getCachedNodes=function(){var i=this.data;var I=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedNodes==undefined){i.cache.cachedNodes=I.nodes()}return i.cache.cachedNodes};A.prototype.updateNodesCache=function(){var i=this.data;var I=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedNodes=I.nodes()};A.prototype.getCachedEdges=function(){var i=this.data;
var I=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedEdges==undefined){i.cache.cachedEdges=I.edges()}return i.cache.cachedEdges};A.prototype.updateEdgesCache=function(){var i=this.data;var I=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedEdges=I.edges()};A.prototype.projectIntoViewport=function(J,i){n=this.data.container;var M=false;var K=this.findContainerPageCoords();var L=K[0];var I=K[1];x=J-L;y=i-I;x-=this.data.cy.pan().x;y-=this.data.cy.pan().y;x/=this.data.cy.zoom();
y/=this.data.cy.zoom();return[x,y]};A.prototype.findContainerPageCoords=function(){var i,N;var K=0;var J=0;var M;M=this.data.container;var L=false;while(M!=null){var I=window.getComputedStyle(M);if(I.getPropertyValue("position").toLowerCase()==="fixed"){K=M.offsetLeft+window.scrollX;J=M.offsetTop+window.scrollY;M=null}else{if(typeof(M.offsetLeft)=="number"){K+=M.offsetLeft;J+=M.offsetTop;if(M==document.body||M==document.header){L=true}if(!L){K-=M.scrollLeft;J-=M.scrollTop}}}if(M){M=M.offsetParent
}}return[K,J]};A.prototype.findNearestElement=function(W,V,K){var R=this.data;var L=this.getCachedNodes();var P=this.getCachedEdges();var S=[];var X=this.data.cy.zoom();var N=(w?256:32)/X;var U=(w?16:0)/X;for(var Q=0;Q<L.length;Q++){if(r[this.getNodeShape(L[Q])].checkPointRough(W,V,L[Q]._private.style["border-width"].value,this.getNodeWidth(L[Q])+U,this.getNodeHeight(L[Q])+U,L[Q]._private.position.x,L[Q]._private.position.y)&&r[this.getNodeShape(L[Q])].checkPoint(W,V,L[Q]._private.style["border-width"].value,(this.getNodeWidth(L[Q])+U)/2,(this.getNodeHeight(L[Q])+U)/2,L[Q]._private.position.x,L[Q]._private.position.y)){if(K){if(L[Q]._private.style.opacity.value!=0&&L[Q]._private.style.visibility.value=="visible"){S.push(L[Q])
}}else{S.push(L[Q])}}}var J;for(var Q=0;Q<P.length;Q++){var M=P[Q];var O=M._private.rscratch;J=false;if(O.edgeType=="self"){if((this.inBezierVicinity(W,V,O.startX,O.startY,O.cp2ax,O.cp2ay,O.selfEdgeMidX,O.selfEdgeMidY,Math.pow(M._private.style.width.value/2,2))&&(Math.pow(P[Q]._private.style.width.value/2,2)+N>this.sqDistanceToQuadraticBezier(W,V,O.startX,O.startY,O.cp2ax,O.cp2ay,O.selfEdgeMidX,O.selfEdgeMidY)))||(this.inBezierVicinity(W,V,O.selfEdgeMidX,O.selfEdgeMidY,O.cp2cx,O.cp2cy,O.endX,O.endY,Math.pow(P[Q]._private.style.width.value/2,2))&&(Math.pow(P[Q]._private.style.width.value/2,2)+N>this.sqDistanceToQuadraticBezier(W,V,O.selfEdgeMidX,O.selfEdgeMidY,O.cp2cx,O.cp2cy,O.endX,O.endY)))){J=true
}}else{if(O.edgeType=="straight"){if(this.inLineVicinity(W,V,O.startX,O.startY,O.endX,O.endY,P[Q]._private.style.width.value*2)&&Math.pow(P[Q]._private.style.width.value/2,2)+N>this.sqDistanceToFiniteLine(W,V,O.startX,O.startY,O.endX,O.endY)){J=true}}else{if(O.edgeType=="bezier"){if(this.inBezierVicinity(W,V,O.startX,O.startY,O.cp2x,O.cp2y,O.endX,O.endY,Math.pow(P[Q]._private.style.width.value/2,2))&&(Math.pow(P[Q]._private.style.width.value/2,2)+N>this.sqDistanceToQuadraticBezier(W,V,O.startX,O.startY,O.cp2x,O.cp2y,O.endX,O.endY))){J=true
}}}}if(!S.length||S[S.length-1]!=P[Q]){if((b[P[Q]._private.style["source-arrow-shape"].value].roughCollide(W,V,P[Q]._private.rscratch.arrowStartX,P[Q]._private.rscratch.arrowStartY,this.getArrowWidth(P[Q]._private.style.width.value),this.getArrowHeight(P[Q]._private.style.width.value),[P[Q]._private.rscratch.arrowStartX-P[Q].source()[0]._private.position.x,P[Q]._private.rscratch.arrowStartY-P[Q].source()[0]._private.position.y],0)&&b[P[Q]._private.style["source-arrow-shape"].value].collide(W,V,P[Q]._private.rscratch.arrowStartX,P[Q]._private.rscratch.arrowStartY,this.getArrowWidth(P[Q]._private.style.width.value),this.getArrowHeight(P[Q]._private.style.width.value),[P[Q]._private.rscratch.arrowStartX-P[Q].source()[0]._private.position.x,P[Q]._private.rscratch.arrowStartY-P[Q].source()[0]._private.position.y],0))||(b[P[Q]._private.style["target-arrow-shape"].value].roughCollide(W,V,P[Q]._private.rscratch.arrowEndX,P[Q]._private.rscratch.arrowEndY,this.getArrowWidth(P[Q]._private.style.width.value),this.getArrowHeight(P[Q]._private.style.width.value),[P[Q]._private.rscratch.arrowEndX-P[Q].target()[0]._private.position.x,P[Q]._private.rscratch.arrowEndY-P[Q].target()[0]._private.position.y],0)&&b[P[Q]._private.style["target-arrow-shape"].value].collide(W,V,P[Q]._private.rscratch.arrowEndX,P[Q]._private.rscratch.arrowEndY,this.getArrowWidth(P[Q]._private.style.width.value),this.getArrowHeight(P[Q]._private.style.width.value),[P[Q]._private.rscratch.arrowEndX-P[Q].target()[0]._private.position.x,P[Q]._private.rscratch.arrowEndY-P[Q].target()[0]._private.position.y],0))){J=true
}}if(J){if(K){var I=R.cy.getElementById(P[Q]._private.data.source);var T=R.cy.getElementById(P[Q]._private.data.target);if(P[Q]._private.style.opacity.value!=0&&P[Q]._private.style.visibility.value=="visible"&&I._private.style.opacity.value!=0&&I._private.style.visibility.value=="visible"&&T._private.style.opacity.value!=0&&T._private.style.visibility.value=="visible"){S.push(P[Q])}}else{S.push(P[Q])}}}S.sort(D);if(S.length>0){return S[S.length-1]}else{return null}};A.prototype.getAllInBox=function(K,U,I,T){var P=this.data;
var J=this.getCachedNodes();var N=this.getCachedEdges();var Q=[];var S=Math.min(K,I);var M=Math.max(K,I);var V=Math.min(U,T);var R=Math.max(U,T);K=S;I=M;U=V;T=R;var L;for(var O=0;O<J.length;O++){if(r[this.getNodeShape(J[O])].intersectBox(K,U,I,T,this.getNodeWidth(J[O]),this.getNodeHeight(J[O]),J[O]._private.position.x,J[O]._private.position.y,J[O]._private.style["border-width"].value/2)){Q.push(J[O])}}for(var O=0;O<N.length;O++){if(N[O]._private.rscratch.edgeType=="self"){if((L=this.boxInBezierVicinity(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2ax,N[O]._private.rscratch.cp2ay,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value))&&(L==2||(L==1&&this.checkBezierInBox(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2ax,N[O]._private.rscratch.cp2ay,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value)))||(L=this.boxInBezierVicinity(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2cx,N[O]._private.rscratch.cp2cy,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value))&&(L==2||(L==1&&this.checkBezierInBox(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2cx,N[O]._private.rscratch.cp2cy,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value)))){Q.push(N[O])
}}if(N[O]._private.rscratch.edgeType=="bezier"&&(L=this.boxInBezierVicinity(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2x,N[O]._private.rscratch.cp2y,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value))&&(L==2||(L==1&&this.checkBezierInBox(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.cp2x,N[O]._private.rscratch.cp2y,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value)))){Q.push(N[O])
}if(N[O]._private.rscratch.edgeType=="straight"&&(L=this.boxInBezierVicinity(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.startX*0.5+N[O]._private.rscratch.endX*0.5,N[O]._private.rscratch.startY*0.5+N[O]._private.rscratch.endY*0.5,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value))&&(L==2||(L==1&&this.checkStraightEdgeInBox(K,U,I,T,N[O]._private.rscratch.startX,N[O]._private.rscratch.startY,N[O]._private.rscratch.endX,N[O]._private.rscratch.endY,N[O]._private.style.width.value)))){Q.push(N[O])
}}return Q};A.prototype.updateAllCompounds=function(L){for(var I=L.length-1;I>=0;I--){if(L[I].isNode()&&(L[I]._private.style.width.value=="auto"||L[I]._private.style.height.value=="auto")&&L[I].children().length>0){var K=L[I];var J=this.calcCompoundBounds(K);K._private.position.x=J.x;K._private.position.y=J.y;K._private.autoWidth=J.width;K._private.autoHeight=J.height}}};A.prototype.calcCompoundBounds=function(O){var N=O.descendants().not(":removed");var I={x:O._private.position.x,y:O._private.position.y,width:O._private.autoWidth,height:O._private.autoHeight};
if(O._private.style.visibility.value!="visible"){return I}var J=[];for(var Q=0;Q<N.size();Q++){if(N[Q]._private.style.visibility.value=="visible"){J.push(N[Q])}}if(J.length==0){return I}N=J;var K=this.borderValue(N,"left");var R=this.borderValue(N,"right");var P=this.borderValue(N,"top");var M=this.borderValue(N,"bottom");var U=this.getNodePadding(O);var T=(K-U.left+R+U.right)/2;var S=(P-U.top+M+U.bottom)/2;var L=(R-K)+U.left+U.right;var V=(M-P)+U.top+U.bottom;I={x:T,y:S,width:L,height:V};return I
};A.prototype.borderValue=function(K,S){var O,J;var T=1/0,Q=-1/0;var i=this;var R=function(W){var V={};V.x=W._private.position.x;V.y=W._private.position.y;V.width=W.outerWidth();V.height=W.outerHeight();return V};var M=function(W){var X=String(W._private.style.content.value);var Y=W._private.style["text-transform"].value;if(Y=="none"){}else{if(Y=="uppercase"){X=X.toUpperCase()}else{if(Y=="lowercase"){X=X.toLowerCase()}}}var V=i.data.canvases[4].getContext("2d");return V.measureText(X).width};var U=function(Z){var X={};
var V=i.getNodeWidth(Z);var aa=i.getNodeHeight(Z);X.height=Z._private.style["font-size"].value;X.width=X.height;var Y=Z._private.style["text-halign"].strValue;if(Y=="left"){X.x=Z._private.position.x-V/2;X.left=X.x-X.width;X.right=X.x}else{if(Y=="right"){X.x=Z._private.position.x+V/2;X.left=X.x;X.right=X.x+X.width}else{X.x=Z._private.position.x;X.left=X.x-X.width/2;X.right=X.x+X.width/2}}var W=Z._private.style["text-valign"].strValue;if(W=="top"){X.y=Z._private.position.y-aa/2;X.top=X.y-X.height;X.bottom=X.y
}else{if(W=="bottom"){X.y=Z._private.position.y+aa/2;X.top=X.y;X.bottom=X.y+X.height}else{X.y=Z._private.position.y;X.top=X.y-X.height/2;X.bottom=X.y+X.height/2}}return X};for(F=0;F<K.length;F++){O=R(K[F]);J=U(K[F]);if(S=="left"){var I=Math.min(O.x-O.width/2,J.left);if(I<T){T=I}}else{if(S=="right"){var P=Math.max(O.x+O.width/2,J.right);if(P>Q){Q=P}}else{if(S=="top"){var N=Math.min(O.y-O.height/2,J.top);if(N<T){T=N}}else{if(S=="bottom"){var L=Math.max(O.y+O.height/2,J.bottom);if(L>Q){Q=L}}}}}}if((S=="left")||(S=="top")){return T
}else{return Q}};A.prototype.getNodeWidth=function(i){if(i._private.style.width.value=="auto"||i._private.style.height.value=="auto"){return i._private.autoWidth}else{return i._private.style.width.value}};A.prototype.getNodeHeight=function(i){if(i._private.style.width.value=="auto"||i._private.style.height.value=="auto"){return i._private.autoHeight}else{return i._private.style.height.value}};A.prototype.getNodeShape=function(I){var i=I._private.style.shape.value;if(I.isParent()){if(i==="rectangle"||i==="roundrectangle"){return i
}else{return"rectangle"}}return i};A.prototype.getNodePadding=function(J){var L=J._private.style["padding-left"].value;var I=J._private.style["padding-right"].value;var K=J._private.style["padding-top"].value;var i=J._private.style["padding-bottom"].value;if(isNaN(L)){L=0}if(isNaN(I)){I=0}if(isNaN(K)){K=0}if(isNaN(i)){i=0}return{left:L,right:I,top:K,bottom:i}};A.prototype.matchCanvasSize=function(J){var N=this.data;var M=J.clientWidth;var I=J.clientHeight;var K;for(var L=0;L<e;L++){K=N.canvases[L];
if(K.width!==M||K.height!==I){K.width=M;K.height=I}}for(var L=0;L<a;L++){K=N.bufferCanvases[L];if(K.width!==M||K.height!==I){K.width=M;K.height=I}}this.data.overlay.style.width=M+"px";this.data.overlay.style.height=I+"px"};var f=function(i){if(i._private.group=="nodes"){return i.parents().size()}else{if(i._private.group=="edges"){return Math.max(i.source()[0].parents().size(),i.target()[0].parents().size())}else{return 0}}};A.prototype.getCachedZSortedEles=function(){var N=this.lastZOrderCachedNodes;
var M=this.lastZOrderCachedEdges;var K=this.getCachedNodes();var J=this.getCachedEdges();var I=[];if(!N||!M||N!==K||M!==J){for(var L=0;L<K.length;L++){I.push(K[L])}for(var L=0;L<J.length;L++){I.push(J[L])}I.sort(D);this.cachedZSortedEles=I}else{I=this.cachedZSortedEles}this.lastZOrderCachedNodes=K;this.lastZOrderCachedEdges=J;return I};var D=function(J,I){var i=J._private.style["z-index"].value-I._private.style["z-index"].value;var L=0;var K=0;if(J.cy().hasCompoundNodes()){L=f(J);K=f(I)}if(L-K===0){if(J._private.group==="nodes"&&I._private.group==="edges"){return 1
}else{if(J._private.group==="edges"&&I._private.group==="nodes"){return -1}else{if(i===0){return J._private.index-I._private.index}else{return i}}}}else{return L-K}return 0};A.prototype.redraw=function(M,K){var i=this;if(this.averageRedrawTime===undefined){this.averageRedrawTime=0}var J=1000/60;var O=1000;var N=this.averageRedrawTime;N=Math.max(J,N);N=Math.min(N,O);if(this.lastDrawTime===undefined){this.lastDrawTime=0}var P=+new Date;var I=P-this.lastDrawTime;var L=I>=N;if(!M){if(!L){clearTimeout(this.redrawTimeout);
this.redrawTimeout=setTimeout(function(){i.redraw()},N);return}this.lastDrawTime=P}setTimeout(function(){var ag=P;var ab=100;var U=i.data.cy;var am=i.data;var ai=i.getCachedNodes();var T=i.getCachedEdges();i.matchCanvasSize(am.container);var af=[];for(var aj=0;aj<ai.length;aj++){af.push(ai[aj])}for(var aj=0;aj<T.length;aj++){af.push(T[aj])}if(am.canvasNeedsRedraw[d]||am.canvasNeedsRedraw[H]||K){if(i.hideEdgesOnViewport&&(i.pinching||i.hoverData.dragging||i.data.wheel||i.swipePanning)){}else{i.findEdgeControlPoints(T)
}var af=i.getCachedZSortedEles();if(U.hasCompoundNodes()){i.updateAllCompounds(af)}}var Z;var W;var S;if(am.canvasNeedsRedraw[H]||K){if(!Z||!W){Z=[];W=[];for(var ac=0;ac<af.length;ac++){S=af[ac];if(S._private.rscratch.inDragLayer){Z.push(S)}else{W.push(S)}}}var V=M||am.canvases[H].getContext("2d");V.setTransform(1,0,0,1,0,0);V.clearRect(0,0,V.canvas.width,V.canvas.height);if(!K){V.translate(U.pan().x,U.pan().y);V.scale(U.zoom(),U.zoom())}for(var ac=0;ac<W.length;ac++){S=W[ac];if(S._private.group=="nodes"){i.drawNode(V,S)
}else{if(S._private.group=="edges"){i.drawEdge(V,S)}}}for(var ac=0;ac<W.length;ac++){S=W[ac];if(S._private.group=="nodes"){i.drawNodeText(V,S)}else{if(S._private.group=="edges"){i.drawEdgeText(V,S)}}if(S._private.group=="nodes"){i.drawNode(V,S,true)}else{if(S._private.group=="edges"){i.drawEdge(V,S,true)}}}if(!K){am.canvasNeedsRedraw[H]=false;am.canvasRedrawReason[H]=[]}}if(am.canvasNeedsRedraw[d]||K){if(!Z||!W){Z=[];W=[];for(var ac=0;ac<af.length;ac++){S=af[ac];if(S._private.rscratch.inDragLayer){Z.push(S)
}else{W.push(S)}}}var V=M||am.canvases[d].getContext("2d");if(!K){V.setTransform(1,0,0,1,0,0);V.clearRect(0,0,V.canvas.width,V.canvas.height);V.translate(U.pan().x,U.pan().y);V.scale(U.zoom(),U.zoom())}var S;for(var ac=0;ac<Z.length;ac++){S=Z[ac];if(S._private.group=="nodes"){i.drawNode(V,S)}else{if(S._private.group=="edges"){i.drawEdge(V,S)}}}for(var ac=0;ac<Z.length;ac++){S=Z[ac];if(S._private.group=="nodes"){i.drawNodeText(V,S)}else{if(S._private.group=="edges"){i.drawEdgeText(V,S)}}if(S._private.group=="nodes"){i.drawNode(V,S,true)
}else{if(S._private.group=="edges"){i.drawEdge(V,S,true)}}}if(!K){am.canvasNeedsRedraw[d]=false;am.canvasRedrawReason[d]=[]}}if(am.canvasNeedsRedraw[E]){var V=M||am.canvases[E].getContext("2d");if(!K){V.setTransform(1,0,0,1,0,0);V.clearRect(0,0,V.canvas.width,V.canvas.height);V.translate(U.pan().x,U.pan().y);V.scale(U.zoom(),U.zoom())}var R=U.style()._private.coreStyle;if(am.select[4]==1){var Q=am.cy.zoom();var an=R["selection-box-border-width"].value/Q;V.lineWidth=an;V.fillStyle="rgba("+R["selection-box-color"].value[0]+","+R["selection-box-color"].value[1]+","+R["selection-box-color"].value[2]+","+R["selection-box-opacity"].value+")";
V.fillRect(am.select[0],am.select[1],am.select[2]-am.select[0],am.select[3]-am.select[1]);if(an>0){V.strokeStyle="rgba("+R["selection-box-border-color"].value[0]+","+R["selection-box-border-color"].value[1]+","+R["selection-box-border-color"].value[2]+","+R["selection-box-opacity"].value+")";V.strokeRect(am.select[0],am.select[1],am.select[2]-am.select[0],am.select[3]-am.select[1])}}if(am.bgActivePosistion){var Q=am.cy.zoom();var aa=am.bgActivePosistion;V.fillStyle="rgba("+R["active-bg-color"].value[0]+","+R["active-bg-color"].value[1]+","+R["active-bg-color"].value[2]+","+R["active-bg-opacity"].value+")";
V.beginPath();V.arc(aa.x,aa.y,R["active-bg-size"].pxValue/Q,0,2*Math.PI);V.fill()}if(!K){am.canvasNeedsRedraw[E]=false;am.canvasRedrawReason[E]=[]}}if(i.options.showOverlay){var V=am.canvases[v].getContext("2d");V.lineJoin="round";V.font="14px helvetica";V.strokeStyle="#fff";V.lineWidth="4";V.fillStyle="#666";V.textAlign="right";var ae="cytoscape.js";var ad=V.canvas.width;var ak=V.canvas.height;var ah=4;var al=V.measureText(ae).width;var Y=14;V.clearRect(0,0,ad,ak);V.strokeText(ae,ad-ah,ak-ah);V.fillText(ae,ad-ah,ak-ah);
am.overlayDrawn=true}var X=+new Date;if(i.averageRedrawTime===undefined){i.averageRedrawTime=X-ag}i.averageRedrawTime=i.averageRedrawTime/2+(X-ag)/2},0)};var l={};var h=30*300;A.prototype.getCachedImage=function(I,J){if(l[I]&&l[I].image){l[I].keepTime=h;return l[I].image}var i=l[I];if(i==undefined){l[I]=new Object();l[I].image=new Image();l[I].image.onload=J;l[I].image.src=I;l[I].keepTime=h;i=l[I]}return i.image};A.prototype.swapCachedImage=function(I){if(l[I]){if(l[I].image&&l[I].image.complete){var J=l[I].image;
var i=document.createElement("canvas");i.width=J.width;i.height=J.height;i.getContext("2d").drawImage(J,0,0);l[I].image=i;l[I].swappedWithCanvas=true;return i}else{return null}}else{return null}};A.prototype.updateImageCaches=function(){for(var i in l){if(l[i].keepTime<=0){if(l[i].image!=undefined){l[i].image.src=undefined;l[i].image=undefined}l[i]=undefined}else{l[i]-=1}}};A.prototype.drawImage=function(i,O,N,L,K,P,J){J.widthScale=0.5;J.heightScale=0.5;J.rotate=P;var M;var I;canvas.drawImage(J,O,N)
};A.prototype.drawEdge=function(J,L,M){if(this.hideEdgesOnViewport&&(this.dragData.didDrag||this.pinching||this.hoverData.dragging||this.data.wheel||this.swipePanning)){return}var N,R;N=L.source()[0];R=L.target()[0];if(L._private.style.visibility.value!="visible"||N._private.style.visibility.value!="visible"||R._private.style.visibility.value!="visible"){return}var V=L._private.style["overlay-padding"].value;var T=L._private.style["overlay-opacity"].value;var U=L._private.style["overlay-color"].value;
if(M){J.strokeStyle="rgba( "+U[0]+", "+U[1]+", "+U[2]+", "+T+" )";J.lineCap="round";if(L._private.rscratch.edgeType=="self"){J.lineCap="butt"}}else{J.strokeStyle="rgba("+L._private.style["line-color"].value[0]+","+L._private.style["line-color"].value[1]+","+L._private.style["line-color"].value[2]+","+L._private.style.opacity.value+")"}if(L._private.style.width.value<=0){return}var S=L._private.style.width.value+(M?2*V:0);var P=M?"solid":L._private.style["line-style"].value;J.lineWidth=S;this.findEndpoints(L);
if(L._private.rscratch.edgeType=="self"){var i=L._private.rscratch;this.drawStyledEdge(L,J,[i.startX,i.startY,i.cp2ax,i.cp2ay,i.selfEdgeMidX,i.selfEdgeMidY],P,S);this.drawStyledEdge(L,J,[i.selfEdgeMidX,i.selfEdgeMidY,i.cp2cx,i.cp2cy,i.endX,i.endY],P,S)}else{if(L._private.rscratch.edgeType=="straight"){var K=R._private.position.x-N._private.position.x;var I=R._private.position.y-N._private.position.y;var Q=L._private.rscratch.endX-L._private.rscratch.startX;var O=L._private.rscratch.endY-L._private.rscratch.startY;
if(K*Q+I*O<0){L._private.rscratch.straightEdgeTooShort=true}else{var i=L._private.rscratch;this.drawStyledEdge(L,J,[i.startX,i.startY,i.endX,i.endY],P,S);L._private.rscratch.straightEdgeTooShort=false}}else{var i=L._private.rscratch;this.drawStyledEdge(L,J,[i.startX,i.startY,i.cp2x,i.cp2y,i.endX,i.endY],P,S)}}if(L._private.rscratch.noArrowPlacement!==true&&L._private.rscratch.startX!==undefined){this.drawArrowheads(J,L,M)}};var u=function(Q,M,K){var J=Math.sqrt(Math.pow(Q[4]-Q[0],2)+Math.pow(Q[5]-Q[1],2));
J+=Math.sqrt(Math.pow((Q[4]+Q[0])/2-Q[2],2)+Math.pow((Q[5]+Q[1])/2-Q[3],2));var P=Math.ceil(J/M);var I=J/M;var N;if(P>0){N=new Array(P*2)}else{return null}for(var L=0;L<P;L++){var O=L/P;N[L*2]=Q[0]*(1-O)*(1-O)+2*(Q[2])*(1-O)*O+Q[4]*(O)*(O);N[L*2+1]=Q[1]*(1-O)*(1-O)+2*(Q[3])*(1-O)*O+Q[5]*(O)*(O)}return N};var t=function(Q,M,K){var J=Math.sqrt(Math.pow(Q[2]-Q[0],2)+Math.pow(Q[3]-Q[1],2));var P=Math.ceil(J/M);var N;if(P>0){N=new Array(P*2)}else{return null}var I=[Q[2]-Q[0],Q[3]-Q[1]];for(var L=0;L<P;
L++){var O=L/P;N[L*2]=I[0]*O+Q[0];N[L*2+1]=I[1]*O+Q[1]}return N};var s=function(J,i,I){pt1=_genpts(J,i);pt2=_genpts(J,I)};A.prototype.createBuffer=function(I,J){var i=document.createElement("canvas");i.width=I;i.height=J;return[i,i.getContext("2d")]};A.prototype.drawStyledEdge=function(P,L,ac,M,V){var K=this.data.cy;var J=K.zoom();function ab(af,ae,ad,i){return(1-i)*(1-i)*af+2*(1-i)*i*ae+i*i*ad}if(P._private.rstyle.bezierPts===undefined){P._private.rstyle.bezierPts=[]}var X=P._private.rstyle.bezierPts.length;
if(P.isLoop()){if(X>=12){P._private.rstyle.bezierPts=[]}else{}}else{P._private.rstyle.bezierPts=[]}var S=P._private.rstyle.bezierPts;if(ac.length===6){S.push({x:ab(ac[0],ac[2],ac[4],0.05),y:ab(ac[1],ac[3],ac[5],0.05)});S.push({x:ab(ac[0],ac[2],ac[4],0.25),y:ab(ac[1],ac[3],ac[5],0.25)});S.push({x:ab(ac[0],ac[2],ac[4],0.35),y:ab(ac[1],ac[3],ac[5],0.35)});S.push({x:ab(ac[0],ac[2],ac[4],0.65),y:ab(ac[1],ac[3],ac[5],0.65)});S.push({x:ab(ac[0],ac[2],ac[4],0.75),y:ab(ac[1],ac[3],ac[5],0.75)});S.push({x:ab(ac[0],ac[2],ac[4],0.95),y:ab(ac[1],ac[3],ac[5],0.95)})
}if(M=="solid"){L.beginPath();L.moveTo(ac[0],ac[1]);if(ac.length==3*2){L.quadraticCurveTo(ac[2],ac[3],ac[4],ac[5])}else{L.lineTo(ac[2],ac[3])}L.stroke()}else{if(M=="dotted"){var U;if(ac.length==3*2){U=u(ac,16,true)}else{U=t(ac,16,true)}if(!U){return}var O=Math.max(V*1.6,3.4)*J;var N=O*2,R=O*2;N=Math.max(N,1);R=Math.max(R,1);var Y=this.createBuffer(N,R);var W=Y[1];W.setTransform(1,0,0,1,0,0);W.clearRect(0,0,N,R);W.fillStyle=L.strokeStyle;W.beginPath();W.arc(N/2,R/2,O*0.5,0,Math.PI*2,false);W.fill();
L.beginPath();for(var Z=0;Z<U.length/2;Z++){L.drawImage(Y[0],U[Z*2]-N/2/J,U[Z*2+1]-R/2/J,N/J,R/J)}}else{if(M=="dashed"){var U;if(ac.length==3*2){U=u(ac,14,true)}else{U=t(ac,14,true)}if(!U){return}var N=V*2*J;var R=7.8*J;N=Math.max(N,1);R=Math.max(R,1);var Y=this.createBuffer(N,R);var W=Y[1];W.setTransform(1,0,0,1,0,0);W.clearRect(0,0,N,R);if(L.strokeStyle){W.strokeStyle=L.strokeStyle}W.lineWidth=V*K.zoom();W.beginPath();W.moveTo(N/2,R*0.2);W.lineTo(N/2,R*0.8);W.stroke();var I=false;var Q,aa;if(ac.length==2*2){Q=[ac[2]-ac[0],ac[3]-U[1]];
aa=Math.acos((Q[0]*0+Q[1]*-1)/Math.sqrt(Q[0]*Q[0]+Q[1]*Q[1]));if(Q[0]<0){aa=-aa+2*Math.PI}}else{if(ac.length==3*2){I=true}}for(var Z=0;Z<U.length/2;Z++){var T=Z/(Math.max(U.length/2-1,1));if(I){Q=[2*(1-T)*(ac[2]-ac[0])+2*T*(ac[4]-ac[2]),2*(1-T)*(ac[3]-ac[1])+2*T*(ac[5]-ac[3])];aa=Math.acos((Q[0]*0+Q[1]*-1)/Math.sqrt(Q[0]*Q[0]+Q[1]*Q[1]));if(Q[0]<0){aa=-aa+2*Math.PI}}L.translate(U[Z*2],U[Z*2+1]);L.rotate(aa);L.translate(-N/2/J,-R/2/J);L.drawImage(Y[0],0,0,N/J,R/J);L.translate(N/2/J,R/2/J);L.rotate(-aa);
L.translate(-U[Z*2],-U[Z*2+1])}}else{this.drawStyledEdge(P,L,ac,"solid",V)}}}};A.prototype.drawEdgeText=function(J,K){if(this.hideEdgesOnViewport&&(this.dragData.didDrag||this.pinching||this.hoverData.dragging||this.data.wheel||this.swipePanning)){return}if(K._private.style.visibility.value!="visible"){return}var M=K._private.style["font-size"].pxValue*K.cy().zoom();var I=K._private.style["min-zoomed-font-size"].pxValue;if(M<I){return}J.textAlign="center";J.textBaseline="middle";var N,L;var P,O;if(K._private.rscratch.edgeType=="self"){P=K._private.rscratch.selfEdgeMidX;
O=K._private.rscratch.selfEdgeMidY}else{if(K._private.rscratch.edgeType=="straight"){P=(K._private.rscratch.startX+K._private.rscratch.endX)/2;O=(K._private.rscratch.startY+K._private.rscratch.endY)/2}else{if(K._private.rscratch.edgeType=="bezier"){P=0.25*K._private.rscratch.startX+2*0.5*0.5*K._private.rscratch.cp2x+(0.5*0.5)*K._private.rscratch.endX;O=Math.pow(1-0.5,2)*K._private.rscratch.startY+2*(1-0.5)*0.5*K._private.rscratch.cp2y+(0.5*0.5)*K._private.rscratch.endY}}}N=P;L=O;var i=K._private.rstyle;
i.labelX=N;i.labelY=L;this.drawText(J,K,N,L)};A.prototype.drawNode=function(L,N,M){var V,K;if(N._private.style.visibility.value!="visible"){return}var X=1;var T=N.parents();for(var P=0;P<T.length;P++){var U=T[P];var Q=U._private.style.opacity.value;X=Q*X;if(Q===0){return}}V=this.getNodeWidth(N);K=this.getNodeHeight(N);if(M===undefined||!M){L.fillStyle="rgba("+N._private.style["background-color"].value[0]+","+N._private.style["background-color"].value[1]+","+N._private.style["background-color"].value[2]+","+(N._private.style["background-opacity"].value*N._private.style.opacity.value*X)+")";
L.strokeStyle="rgba("+N._private.style["border-color"].value[0]+","+N._private.style["border-color"].value[1]+","+N._private.style["border-color"].value[2]+","+(N._private.style["border-opacity"].value*N._private.style.opacity.value*X)+")";var J=N._private.style["background-image"].value[2]||N._private.style["background-image"].value[1];if(J!=undefined){var I=this;var O=this.getCachedImage(J,function(){I.data.canvasNeedsRedraw[H]=true;I.data.canvasRedrawReason[H].push("image finished load");I.data.canvasNeedsRedraw[d]=true;
I.data.canvasRedrawReason[d].push("image finished load");I.swapCachedImage(J);I.redraw()});if(O.complete==false){r[I.getNodeShape(N)].drawPath(L,N._private.position.x,N._private.position.y,V,K);L.stroke();L.fillStyle="#555555";L.fill()}else{this.drawInscribedImage(L,O,N)}}else{r[this.getNodeShape(N)].draw(L,N._private.position.x,N._private.position.y,V,K)}L.lineWidth=N._private.style["border-width"].pxValue;if(N._private.style["border-width"].value>0){L.stroke()}}else{var W=N._private.style["overlay-padding"].value;
var R=N._private.style["overlay-opacity"].value;var S=N._private.style["overlay-color"].value;if(R>0){L.fillStyle="rgba( "+S[0]+", "+S[1]+", "+S[2]+", "+R+" )";r[this.getNodeShape(N)].draw(L,N._private.position.x,N._private.position.y,V+W*2,K+W*2)}}};A.prototype.drawInscribedImage=function(J,O,M){var i=this;var Q=this.data.cy._private.zoom;var L=M._private.position.x;var K=M._private.position.y;var P=this.getNodeWidth(M);var I=this.getNodeHeight(M);J.save();r[i.getNodeShape(M)].drawPath(J,L,K,P,I);
J.clip();var N=[O.width,O.height];J.drawImage(O,L-N[0]/2,K-N[1]/2,N[0],N[1]);J.restore();J.stroke()};A.prototype.drawNodeText=function(J,K){if(K._private.style.visibility.value!="visible"){return}var N=K._private.style["font-size"].pxValue*K.cy().zoom();var i=K._private.style["min-zoomed-font-size"].pxValue;if(N<i){return}var O,M;var Q=this.getNodeWidth(K);var I=this.getNodeHeight(K);var L=K._private.style["text-halign"].strValue;if(L=="left"){J.textAlign="right";O=K._private.position.x-Q/2}else{if(L=="right"){J.textAlign="left";
O=K._private.position.x+Q/2}else{if(L=="center"){J.textAlign="center";O=K._private.position.x}else{J.textAlign="center";O=K._private.position.x}}}var P=K._private.style["text-valign"].strValue;if(P=="top"){J.textBaseline="bottom";M=K._private.position.y-I/2}else{if(P=="bottom"){J.textBaseline="top";M=K._private.position.y+I/2}else{if(P=="middle"||P=="center"){J.textBaseline="middle";M=K._private.position.y}else{J.textBaseline="middle";M=K._private.position.y}}}this.drawText(J,K,O,M)};A.prototype.drawText=function(I,M,P,O){var Y=1;
var U=M.parents();for(var N=0;N<U.length;N++){var V=U[N];var S=V._private.style.opacity.value;Y=S*Y;if(S===0){return}}var T=M._private.style["font-style"].strValue;var L=M._private.style["font-size"].value+"px";var X=M._private.style["font-family"].strValue;var R=M._private.style["font-variant"].strValue;var J=M._private.style["font-weight"].strValue;I.font=T+" "+J+" "+L+" "+X;var W=String(M._private.style.content.value);var K=M._private.style["text-transform"].value;if(K=="none"){}else{if(K=="uppercase"){W=W.toUpperCase()
}else{if(K=="lowercase"){W=W.toLowerCase()}}}I.lineJoin="round";I.fillStyle="rgba("+M._private.style.color.value[0]+","+M._private.style.color.value[1]+","+M._private.style.color.value[2]+","+(M._private.style["text-opacity"].value*M._private.style.opacity.value*Y)+")";I.strokeStyle="rgba("+M._private.style["text-outline-color"].value[0]+","+M._private.style["text-outline-color"].value[1]+","+M._private.style["text-outline-color"].value[2]+","+(M._private.style["text-opacity"].value*M._private.style.opacity.value*Y)+")";
if(W!=undefined){var Q=2*M._private.style["text-outline-width"].value;if(Q>0){I.lineWidth=Q;I.strokeText(W,P,O)}if(isNaN(P)){P=0}if(isNaN(O)){O=0}I.fillText(""+W,P,O);M._private.rstyle.labelWidth=I.measureText(W).width}};A.prototype.drawBackground=function(J,I,i,K,L){};A.prototype.findEdgeControlPoints=function(aa){var I={};var ad=this.data.cy;var N=[];var W;for(var ar=0;ar<aa.length;ar++){W=aa[ar]._private.data.source>aa[ar]._private.data.target?aa[ar]._private.data.target+"-"+aa[ar]._private.data.source:aa[ar]._private.data.source+"-"+aa[ar]._private.data.target;
if(I[W]==undefined){I[W]=[]}I[W].push(aa[ar]);N.push(W)}var ae,ak;for(var am=0;am<N.length;am++){W=N[am];ae=ad.getElementById(I[W][0]._private.data.source);ak=ad.getElementById(I[W][0]._private.data.target);var ac=(ae._private.position.x+ak._private.position.x)/2;var ab=(ae._private.position.y+ak._private.position.y)/2;var V,U;if(I[W].length>1){V=ak._private.position.y-ae._private.position.y;U=ae._private.position.x-ak._private.position.x;var T=Math.sqrt(V*V+U*U);V/=T;U/=T}var S;for(var ar=0;ar<I[W].length;
ar++){S=I[W][ar];var P=S._private.rscratch.lastEdgeIndex;var O=ar;var K=S._private.rscratch.lastNumEdges;var J=I[W].length;var aj=S._private.rscratch.lastSrcCtlPtX;var ai=ae._private.position.x;var Z=S._private.rscratch.lastSrcCtlPtY;var Y=ae._private.position.y;var au=S._private.rscratch.lastSrcCtlPtW;var at=ae.outerWidth();var ao=S._private.rscratch.lastSrcCtlPtH;var an=ae.outerHeight();var aq=S._private.rscratch.lastTgtCtlPtX;var ap=ak._private.position.x;var ag=S._private.rscratch.lastTgtCtlPtY;
var af=ak._private.position.y;var R=S._private.rscratch.lastTgtCtlPtW;var Q=ak.outerWidth();var M=S._private.rscratch.lastTgtCtlPtH;var L=ak.outerHeight();if(aj===ai&&Z===Y&&au===at&&ao===an&&aq===ap&&ag===af&&R===Q&&M===L&&P===O&&K===J){continue}else{var ah=S._private.rscratch;ah.lastSrcCtlPtX=ai;ah.lastSrcCtlPtY=Y;ah.lastSrcCtlPtW=at;ah.lastSrcCtlPtH=an;ah.lastTgtCtlPtX=ap;ah.lastTgtCtlPtY=af;ah.lastTgtCtlPtW=Q;ah.lastTgtCtlPtH=L;ah.lastEdgeIndex=O;ah.lastNumEdges=J}if(ae._private.data.id==ak._private.data.id){var al=S._private.style["control-point-step-size"].pxValue;
S._private.rscratch.edgeType="self";S._private.rscratch.cp2ax=ae._private.position.x;S._private.rscratch.cp2ay=ae._private.position.y-(1+Math.pow(this.getNodeHeight(ae),1.12)/100)*al*(ar/3+1);S._private.rscratch.cp2cx=ae._private.position.x-(1+Math.pow(this.getNodeWidth(ae),1.12)/100)*al*(ar/3+1);S._private.rscratch.cp2cy=ae._private.position.y;S._private.rscratch.selfEdgeMidX=(S._private.rscratch.cp2ax+S._private.rscratch.cp2cx)/2;S._private.rscratch.selfEdgeMidY=(S._private.rscratch.cp2ay+S._private.rscratch.cp2cy)/2
}else{if(I[W].length%2==1&&ar==Math.floor(I[W].length/2)){S._private.rscratch.edgeType="straight"}else{var al=S._private.style["control-point-step-size"].value;var X=(0.5-I[W].length/2+ar)*al;S._private.rscratch.edgeType="bezier";S._private.rscratch.cp2x=ac+V*X;S._private.rscratch.cp2y=ab+U*X}}}}return I};A.prototype.findEndpoints=function(K){var M;var i=K.source()[0];var Q=K.target()[0];var V=Math.max(this.getNodeWidth(i),this.getNodeHeight(i));var L=Math.max(this.getNodeWidth(Q),this.getNodeHeight(Q));
V=0;L/=2;var J=[K.source().position().x,K.source().position().y];var N=[K.target().position().x,K.target().position().y];if(K._private.rscratch.edgeType=="self"){var S=[K._private.rscratch.cp2cx,K._private.rscratch.cp2cy];M=r[this.getNodeShape(Q)].intersectLine(Q._private.position.x,Q._private.position.y,this.getNodeWidth(Q),this.getNodeHeight(Q),S[0],S[1],Q._private.style["border-width"].value/2);var I=this.shortenIntersection(M,S,b[K._private.style["target-arrow-shape"].value].spacing(K));var R=this.shortenIntersection(M,S,b[K._private.style["target-arrow-shape"].value].gap(K));
K._private.rscratch.endX=R[0];K._private.rscratch.endY=R[1];K._private.rscratch.arrowEndX=I[0];K._private.rscratch.arrowEndY=I[1];var S=[K._private.rscratch.cp2ax,K._private.rscratch.cp2ay];M=r[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),S[0],S[1],i._private.style["border-width"].value/2);var O=this.shortenIntersection(M,S,b[K._private.style["source-arrow-shape"].value].spacing(K));var P=this.shortenIntersection(M,S,b[K._private.style["source-arrow-shape"].value].gap(K));
K._private.rscratch.startX=P[0];K._private.rscratch.startY=P[1];K._private.rscratch.arrowStartX=O[0];K._private.rscratch.arrowStartY=O[1]}else{if(K._private.rscratch.edgeType=="straight"){M=r[this.getNodeShape(Q)].intersectLine(Q._private.position.x,Q._private.position.y,this.getNodeWidth(Q),this.getNodeHeight(Q),i.position().x,i.position().y,Q._private.style["border-width"].value/2);if(M.length==0){K._private.rscratch.noArrowPlacement=true}else{K._private.rscratch.noArrowPlacement=false}var I=this.shortenIntersection(M,[i.position().x,i.position().y],b[K._private.style["target-arrow-shape"].value].spacing(K));
var R=this.shortenIntersection(M,[i.position().x,i.position().y],b[K._private.style["target-arrow-shape"].value].gap(K));K._private.rscratch.endX=R[0];K._private.rscratch.endY=R[1];K._private.rscratch.arrowEndX=I[0];K._private.rscratch.arrowEndY=I[1];M=r[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),Q.position().x,Q.position().y,i._private.style["border-width"].value/2);if(M.length==0){K._private.rscratch.noArrowPlacement=true
}else{K._private.rscratch.noArrowPlacement=false}var O=this.shortenIntersection(M,[Q.position().x,Q.position().y],b[K._private.style["source-arrow-shape"].value].spacing(K));var P=this.shortenIntersection(M,[Q.position().x,Q.position().y],b[K._private.style["source-arrow-shape"].value].gap(K));K._private.rscratch.startX=P[0];K._private.rscratch.startY=P[1];K._private.rscratch.arrowStartX=O[0];K._private.rscratch.arrowStartY=O[1]}else{if(K._private.rscratch.edgeType=="bezier"){var S=[K._private.rscratch.cp2x,K._private.rscratch.cp2y];
var U=J[0]*0.25+N[0]*0.25+S[0]*0.5;var T=J[1]*0.25+N[1]*0.25+S[1]*0.5;M=r[this.getNodeShape(Q)].intersectLine(Q._private.position.x,Q._private.position.y,this.getNodeWidth(Q),this.getNodeHeight(Q),S[0],S[1],Q._private.style["border-width"].value/2);var I=this.shortenIntersection(M,S,b[K._private.style["target-arrow-shape"].value].spacing(K));var R=this.shortenIntersection(M,S,b[K._private.style["target-arrow-shape"].value].gap(K));K._private.rscratch.endX=R[0];K._private.rscratch.endY=R[1];K._private.rscratch.arrowEndX=I[0];
K._private.rscratch.arrowEndY=I[1];M=r[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),S[0],S[1],i._private.style["border-width"].value/2);var O=this.shortenIntersection(M,S,b[K._private.style["source-arrow-shape"].value].spacing(K));var P=this.shortenIntersection(M,S,b[K._private.style["source-arrow-shape"].value].gap(K));K._private.rscratch.startX=P[0];K._private.rscratch.startY=P[1];K._private.rscratch.arrowStartX=O[0];
K._private.rscratch.arrowStartY=O[1]}else{if(K._private.rscratch.isArcEdge){return}}}}};A.prototype.findEdges=function(M){var J=this.getCachedEdges();var L={};var I=[];for(var K=0;K<M.length;K++){L[M[K]._private.data.id]=M[K]}for(var K=0;K<J.length;K++){if(L[J[K]._private.data.source]||L[J[K]._private.data.target]){I.push(J[K])}}return I};A.prototype.intersectLineEllipse=function(R,Q,N,M,K,I){var J=N-R;var i=M-Q;J/=K;i/=I;var O=Math.sqrt(J*J+i*i);var L=O-1;if(L<0){return[]}var P=L/O;return[(N-R)*P+R,(M-Q)*P+Q]
};A.prototype.findCircleNearPoint=function(L,K,M,Q,P){var I=Q-L;var i=P-K;var J=Math.sqrt(I*I+i*i);var O=I/J;var N=i/J;return[L+O*M,K+N*M]};A.prototype.findMaxSqDistanceToOrigin=function(L){var I=0.000001;var K;for(var J=0;J<L.length/2;J++){K=L[J*2]*L[J*2]+L[J*2+1]*L[J*2+1];if(K>I){I=K}}return I};A.prototype.finiteLinesIntersect=function(M,T,K,R,I,Q,U,O,S){var N=(U-I)*(T-Q)-(O-Q)*(M-I);var P=(K-M)*(T-Q)-(R-T)*(M-I);var L=(O-Q)*(K-M)-(U-I)*(R-T);if(L!=0){var J=N/L;var i=P/L;if(0<=J&&J<=1&&0<=i&&i<=1){return[M+J*(K-M),T+J*(R-T)]
}else{if(!S){return[]}else{return[M+J*(K-M),T+J*(R-T)]}}}else{if(N==0||P==0){if([M,K,U].sort()[1]==U){return[U,O]}if([M,K,I].sort()[1]==I){return[I,Q]}if([I,U,K].sort()[1]==K){return[K,R]}return[]}else{return[]}}};A.prototype.boxIntersectEllipse=function(J,S,i,Q,R,I,V,N,L){if(i<J){var K=J;J=i;i=K}if(Q<S){var T=S;S=Q;Q=T}var U=[N-I/2-R,L];var P=[N+I/2+R,L];var M=[N,L-V/2-R];var O=[N,L+V/2+R];if(i<U[0]){return false}if(J>P[0]){return false}if(S>O[1]){return false}if(Q<M[1]){return false}if(J<=P[0]&&P[0]<=i&&S<=P[1]&&P[1]<=Q){return true
}if(J<=U[0]&&U[0]<=i&&S<=U[1]&&U[1]<=Q){return true}if(J<=M[0]&&M[0]<=i&&S<=M[1]&&M[1]<=Q){return true}if(J<=O[0]&&O[0]<=i&&S<=O[1]&&O[1]<=Q){return true}J=(J-N)/(I/2+R);i=(i-N)/(I/2+R);S=(S-L)/(V/2+R);Q=(Q-L)/(V/2+R);if(J*J+S*S<=1){return true}if(i*i+S*S<=1){return true}if(i*i+Q*Q<=1){return true}if(J*J+Q*Q<=1){return true}return false};A.prototype.boxIntersectPolygon=function(ae,O,ac,L,Q,X,W,U,S,ai,V){if(ac<ae){var ad=ae;ae=ac;ac=ad}if(L<O){var N=O;O=L;L=N}var T=new Array(Q.length);var af=Math.asin(ai[1]/(Math.sqrt(ai[0]*ai[0]+ai[1]*ai[1])));
if(ai[0]<0){af=af+Math.PI/2}else{af=-af-Math.PI/2}var J=Math.cos(-af);var I=Math.sin(-af);for(var ab=0;ab<T.length/2;ab++){T[ab*2]=X/2*(Q[ab*2]*J-Q[ab*2+1]*I);T[ab*2+1]=W/2*(Q[ab*2+1]*J+Q[ab*2]*I);T[ab*2]+=U;T[ab*2+1]+=S}var M=T[0];var R=T[0];var K=T[1];var P=T[1];for(var ab=1;ab<T.length/2;ab++){if(T[ab*2]>R){R=T[ab*2]}if(T[ab*2]<M){M=T[ab*2]}if(T[ab*2+1]>P){P=T[ab*2+1]}if(T[ab*2+1]<K){K=T[ab*2+1]}}if(ac<M-V){return false}if(ae>R+V){return false}if(L<K-V){return false}if(O>P+V){return false}var aa;
if(V>0){var aj=C.expandPolygon(T,-V);aa=C.joinLines(aj)}else{aa=T}for(var ab=0;ab<T.length/2;ab++){if(ae<=T[ab*2]&&T[ab*2]<=ac){if(O<=T[ab*2+1]&&T[ab*2+1]<=L){return true}}}for(var ab=0;ab<aa.length/2;ab++){var Z=aa[ab*2];var Y=aa[ab*2+1];var ah;var ag;if(ab<aa.length/2-1){ah=aa[(ab+1)*2];ag=aa[(ab+1)*2+1]}else{ah=aa[0];ag=aa[1]}if(C.finiteLinesIntersect(Z,Y,ah,ag,ae,O,ac,O,false).length>0){return true}if(C.finiteLinesIntersect(Z,Y,ah,ag,ae,L,ac,L,false).length>0){return true}if(C.finiteLinesIntersect(Z,Y,ah,ag,ae,O,ae,L,false).length>0){return true
}if(C.finiteLinesIntersect(Z,Y,ah,ag,ac,O,ac,L,false).length>0){return true}}return false};A.prototype.polygonIntersectLine=function(O,N,I,L,J,R,Q,P){var Z=[];var M;var K=new Array(I.length);for(var V=0;V<K.length/2;V++){K[V*2]=I[V*2]*R+L;K[V*2+1]=I[V*2+1]*Q+J}var U;if(P>0){var Y=C.expandPolygon(K,-P);U=C.joinLines(Y)}else{U=K}var T,S,X,W;for(var V=0;V<U.length/2;V++){T=U[V*2];S=U[V*2+1];if(V<U.length/2-1){X=U[(V+1)*2];W=U[(V+1)*2+1]}else{X=U[0];W=U[1]}M=this.finiteLinesIntersect(O,N,L,J,T,S,X,W);
if(M.length!=0){Z.push(M[0],M[1])}}return Z};A.prototype.shortenIntersection=function(M,L,J){var i=[M[0]-L[0],M[1]-L[1]];var K=Math.sqrt(i[0]*i[0]+i[1]*i[1]);var I=(K-J)/K;if(I<0){return[]}else{return[L[0]+I*i[0],L[1]+I*i[1]]}};b.arrow={_points:[-0.15,-0.3,0,0,0.15,-0.3],collide:function(N,K,J,I,i,P,L,M){var O=b.arrow._points;return c.pointInsidePolygon(N,K,O,J,I,i,P,L,M)},roughCollide:function(I,O,N,K,J,i,M,L){if(typeof(b.arrow._farthestPointSqDistance)=="undefined"){b.arrow._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.arrow._points)
}return c.checkInBoundingCircle(I,O,b.arrow._farthestPointSqDistance,0,J,i,N,K)},draw:function(J){var K=b.arrow._points;for(var I=0;I<K.length/2;I++){J.lineTo(K[I*2],K[I*2+1])}},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.triangle=b.arrow;b.none={collide:function(I,O,N,K,J,i,M,L){return false},roughCollide:function(I,O,N,K,J,i,M,L){return false},draw:function(i){},spacing:function(i){return 0},gap:function(i){return 0}};b.circle={_baseRadius:0.15,collide:function(Q,N,K,J,i,T,O,P){if(i!=T){var I=Math.asin(O[1]/(Math.sqrt(O[0]*O[0]+O[1]*O[1])));
var S=Math.cos(-I);var M=Math.sin(-I);var L=[Q*S-N*M,N*S+Q*M];var R=(T+P)/(i+P);N/=R;J/=R;return(Math.pow(K-Q,2)+Math.pow(J-N,2)<=Math.pow((i+P)*b.circle._baseRadius,2))}else{return(Math.pow(K-Q,2)+Math.pow(J-N,2)<=Math.pow((i+P)*b.circle._baseRadius,2))}},roughCollide:function(I,O,N,K,J,i,M,L){return true},draw:function(i){i.arc(0,0,b.circle._baseRadius,0,Math.PI*2,false)},spacing:function(i){return c.getArrowWidth(i._private.style.width.value)*b.circle._baseRadius},gap:function(i){return i._private.style.width.value*2
}};b.inhibitor={_points:[-0.25,0,-0.25,-0.1,0.25,-0.1,0.25,0],collide:function(N,K,J,I,i,P,L,M){var O=b.inhibitor._points;return c.pointInsidePolygon(N,K,O,J,I,i,P,L,M)},roughCollide:function(I,O,N,K,J,i,M,L){if(typeof(b.inhibitor._farthestPointSqDistance)=="undefined"){b.inhibitor._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.inhibitor._points)}return c.checkInBoundingCircle(I,O,b.inhibitor._farthestPointSqDistance,0,J,i,N,K)},draw:function(J){var K=b.inhibitor._points;for(var I=0;I<K.length/2;
I++){J.lineTo(K[I*2],K[I*2+1])}},spacing:function(i){return 4},gap:function(i){return 4}};b.square={_points:[-0.12,0,0.12,0,0.12,-0.24,-0.12,-0.24],collide:function(N,K,J,I,i,P,L,M){var O=b.square._points;return c.pointInsidePolygon(N,K,O,J,I,i,P,L,M)},roughCollide:function(I,O,N,K,J,i,M,L){if(typeof(b.square._farthestPointSqDistance)=="undefined"){b.square._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.square._points)}return c.checkInBoundingCircle(I,O,b.square._farthestPointSqDistance,0,J,i,N,K)
},draw:function(J){var K=b.square._points;for(var I=0;I<K.length/2;I++){J.lineTo(K[I*2],K[I*2+1])}},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.diamond={_points:[-0.14,-0.14,0,-0.28,0.14,-0.14,0,0],collide:function(N,K,J,I,i,P,L,M){var O=b.diamond._points;return c.pointInsidePolygon(N,K,O,J,I,i,P,L,M)},roughCollide:function(I,O,N,K,J,i,M,L){if(typeof(b.diamond._farthestPointSqDistance)=="undefined"){b.diamond._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.diamond._points)
}return c.checkInBoundingCircle(I,O,b.diamond._farthestPointSqDistance,0,J,i,N,K)},draw:function(i){i.lineTo(-0.14,-0.14);i.lineTo(0,-0.28);i.lineTo(0.14,-0.14);i.lineTo(0,0)},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.tee=b.inhibitor;A.prototype.getArrowWidth=function(i){return Math.max(Math.pow(i*13.37,0.9),29)};A.prototype.getArrowHeight=function(i){return Math.max(Math.pow(i*13.37,0.9),29)};A.prototype.drawArrowheads=function(I,K,L){if(L){return}var J,i;
var N=K._private.rscratch.arrowStartX;var M=K._private.rscratch.arrowStartY;J=N-K.source().position().x;i=M-K.source().position().y;I.fillStyle="rgba("+K._private.style["source-arrow-color"].value[0]+","+K._private.style["source-arrow-color"].value[1]+","+K._private.style["source-arrow-color"].value[2]+","+K._private.style.opacity.value+")";I.lineWidth=K._private.style.width.value;this.drawArrowShape(I,K._private.style["source-arrow-shape"].value,N,M,J,i);var P=K._private.rscratch.arrowEndX;var O=K._private.rscratch.arrowEndY;
J=P-K.target().position().x;i=O-K.target().position().y;I.fillStyle="rgba("+K._private.style["target-arrow-color"].value[0]+","+K._private.style["target-arrow-color"].value[1]+","+K._private.style["target-arrow-color"].value[2]+","+K._private.style.opacity.value+")";I.lineWidth=K._private.style.width.value;this.drawArrowShape(I,K._private.style["target-arrow-shape"].value,P,O,J,i)};A.prototype.drawArrowShape=function(M,I,i,O,L,J){var N=Math.asin(J/(Math.sqrt(L*L+J*J)));if(L<0){N=N+Math.PI/2}else{N=-(Math.PI/2+N)
}M.translate(i,O);M.moveTo(0,0);M.rotate(-N);var K=this.getArrowWidth(M.lineWidth);M.scale(K,K);M.beginPath();b[I].draw(M);M.closePath();M.fill();M.scale(1/K,1/K);M.rotate(N);M.translate(-i,-O)};var k=function(J,I){var O=1/J*2*Math.PI;var M=J%2==0?Math.PI/2+O/2:Math.PI/2;M+=I;var Q=new Array(J*2);var K;for(var L=0;L<J;L++){K=L*O+M;Q[2*L]=Math.cos(K);Q[2*L+1]=Math.sin(-K)}var S=0;var R=0;for(var L=0;L<Q.length/2;L++){if(Math.abs(Q[2*L]>S)){S=Math.abs(Q[2*L])}if(Math.abs(Q[2*L+1]>R)){R=Math.abs(Q[2*L+1])
}}var N=0.0005;var P=Math.max(S,R);for(var L=0;L<Q.length/2;L++){if(P>N){Q[2*L]*=(1/P);Q[2*L+1]*=(1/P)}}return Q};var C=c;r.ellipse={draw:function(I,L,K,J,i){r.ellipse.drawPath(I,L,K,J,i);I.fill()},drawPath:function(I,L,K,J,i){I.beginPath();I.translate(L,K);I.scale(J/2,i/2);I.arc(0,0,1,0,Math.PI*2*0.999,false);I.closePath();I.scale(2/J,2/i);I.translate(-L,-K)},intersectLine:function(M,L,K,J,I,O,N){var i=c.intersectLineEllipse(I,O,M,L,K/2+N,J/2+N);return i},intersectBox:function(J,O,i,M,I,P,L,K,N){return A.prototype.boxIntersectEllipse(J,O,i,M,N,I,P,L,K)
},checkPointRough:function(I,N,M,J,i,L,K){return true},checkPoint:function(I,N,M,J,i,L,K){I-=L;N-=K;I/=(J+M);N/=(i+M);return(Math.pow(I,2)+Math.pow(N,2)<=1)}};r.triangle={points:k(3,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.triangle.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.triangle.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.triangle.points,L,K,J/2,I/2,M)},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.triangle.points;
return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.triangle.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.triangle.points,L,K,J,i,[0,-1],M)}};r.square={points:k(4,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.square.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.square.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.square.points,L,K,J/2,I/2,M)
},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.square.points;return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.square.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.square.points,L,K,J,i,[0,-1],M)}};r.rectangle=r.square;r.octogon={};r.roundrectangle=r.square;r.roundrectangle2={roundness:4.99,draw:function(J,I,i){if(I<=roundness*2){return}C.drawPolygon(J._private.position.x,J._private.position.y,I,i,nodeSapes.roundrectangle2.points)
},intersectLine:function(K,J,I,i,L){return C.findPolygonIntersection(K,J,I,i,L,r.square.points)},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.square.points}};r.pentagon={points:k(5,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.pentagon.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.pentagon.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.pentagon.points,L,K,J/2,I/2,M)},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.pentagon.points;
return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.pentagon.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.pentagon.points,L,K,J,i,[0,-1],M)}};r.hexagon={points:k(6,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.hexagon.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.hexagon.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.hexagon.points,L,K,J/2,I/2,M)
},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.hexagon.points;return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.hexagon.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.hexagon.points,L,K,J,i,[0,-1],M)}};r.heptagon={points:k(7,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.heptagon.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.heptagon.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.heptagon.points,L,K,J/2,I/2,M)
},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.heptagon.points;return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.heptagon.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.heptagon.points,L,K,J,i,[0,-1],M)}};r.octagon={points:k(8,0),draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.octagon.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.octagon.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.octagon.points,L,K,J/2,I/2,M)
},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.octagon.points;return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.octagon.points,M,J,i,L,K)},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.octagon.points,L,K,J,i,[0,-1],M)}};var j=new Array(20);var g=k(5,0);var G=k(5,Math.PI/5);var o=0.5*(3-Math.sqrt(5));o*=1.57;for(var F=0;F<G.length/2;F++){G[F*2]*=o;G[F*2+1]*=o}for(var F=0;F<20/4;F++){j[F*4]=g[F*2];
j[F*4+1]=g[F*2+1];j[F*4+2]=G[F*2];j[F*4+3]=G[F*2+1]}r.star5={points:j,draw:function(I,L,K,J,i){C.drawPolygon(I,L,K,J,i,r.star5.points)},drawPath:function(I,L,K,J,i){C.drawPolygonPath(I,L,K,J,i,r.star5.points)},intersectLine:function(L,K,J,I,i,N,M){return C.polygonIntersectLine(i,N,r.star5.points,L,K,J/2,I/2,M)},intersectBox:function(J,O,i,M,I,Q,L,K,N){var P=r.star5.points;return C.boxIntersectPolygon(J,O,i,M,P,I,Q,L,K,[0,-1],N)},checkPointRough:function(I,N,M,J,i,L,K){return C.checkInBoundingBox(I,N,r.star5.points,M,J,i,L,K)
},checkPoint:function(I,N,M,J,i,L,K){return C.pointInsidePolygon(I,N,r.star5.points,L,K,J,i,[0,-1],M)}};A.prototype.expandPolygon=function(U,K){var L=new Array(U.length*2);var T,S,R,Q;for(var O=0;O<U.length/2;O++){T=U[O*2];S=U[O*2+1];if(O<U.length/2-1){R=U[(O+1)*2];Q=U[(O+1)*2+1]}else{R=U[0];Q=U[1]}var P=(Q-S);var N=-(R-T);var M=Math.sqrt(P*P+N*N);var J=P/M;var I=N/M;L[O*4]=T+J*K;L[O*4+1]=S+I*K;L[O*4+2]=R+J*K;L[O*4+3]=Q+I*K}return L};A.prototype.joinLines=function(R){var Q=new Array(R.length/2);var T,S,P,O;
var M,L,K,J;for(var N=0;N<R.length/4;N++){T=R[N*4];S=R[N*4+1];P=R[N*4+2];O=R[N*4+3];if(N<R.length/4-1){M=R[(N+1)*4];L=R[(N+1)*4+1];K=R[(N+1)*4+2];J=R[(N+1)*4+3]}else{M=R[0];L=R[1];K=R[2];J=R[3]}var I=this.finiteLinesIntersect(T,S,P,O,M,L,K,J,true);Q[N*2]=I[0];Q[N*2+1]=I[1]}return Q};A.prototype.pointInsidePolygon=function(T,R,N,Q,O,X,W,ad,V){var P=new Array(N.length);var ac=Math.asin(ad[1]/(Math.sqrt(ad[0]*ad[0]+ad[1]*ad[1])));if(ad[0]<0){ac=ac+Math.PI/2}else{ac=-ac-Math.PI/2}var K=Math.cos(-ac);
var I=Math.sin(-ac);for(var Z=0;Z<P.length/2;Z++){P[Z*2]=X*(N[Z*2]*K-N[Z*2+1]*I);P[Z*2+1]=W*(N[Z*2+1]*K+N[Z*2]*I);P[Z*2]+=Q;P[Z*2+1]+=O}var Y;if(V>0){var ae=C.expandPolygon(P,-V);Y=C.joinLines(ae)}else{Y=P}var ab,M,aa,L;var J;var S=0;var U=0;for(var Z=0;Z<Y.length/2;Z++){ab=Y[Z*2];M=Y[Z*2+1];if(Z+1<Y.length/2){aa=Y[(Z+1)*2];L=Y[(Z+1)*2+1]}else{aa=Y[(Z+1-Y.length/2)*2];L=Y[(Z+1-Y.length/2)*2+1]}if(ab==T&&aa==T){}else{if((ab>=T&&T>=aa)||(ab<=T&&T<=aa)){J=(T-ab)/(aa-ab)*(L-M)+M;if(J>R){S++}if(J<R){U++
}}else{continue}}}if(S%2==0){return false}else{return true}};A.prototype.drawPolygonPath=function(L,J,O,N,I,M){L.translate(J,O);L.scale(N/2,I/2);L.beginPath();L.moveTo(M[0],M[1]);for(var K=1;K<M.length/2;K++){L.lineTo(M[K*2],M[K*2+1])}L.closePath();L.scale(2/N,2/I);L.translate(-J,-O)};A.prototype.drawPolygon=function(J,I,M,L,i,K){this.drawPolygonPath(J,I,M,L,i,K);J.fill()};A.prototype.checkInBoundingCircle=function(I,O,J,N,K,i,M,L){I=(I-M)/(K+N);O=(O-L)/(i+N);return(I*I+O*O)<=J};A.prototype.checkInBoundingBox=function(S,Q,T,R,K,U,O,N){var M=T[0],L=T[1];
var J=T[0],I=T[1];for(var P=1;P<T.length/2;P++){if(T[P*2]<M){M=T[P*2]}else{if(T[P*2]>J){J=T[P*2]}}if(T[P*2+1]<L){L=T[P*2+1]}else{if(T[P*2+1]>I){I=T[P*2+1]}}}S-=O;Q-=N;S/=K;Q/=U;if(S<M){return false}else{if(S>J){return false}}if(Q<L){return false}else{if(Q>I){return false}}return true};A.prototype.boxInBezierVicinity=function(U,O,aa,R,W,N,V,M,S,K,T){var Q=0.25*W+0.5*V+0.25*S;var P=0.25*N+0.5*M+0.25*K;var ab=Math.min(U,aa)-T;var Y=Math.min(O,R)-T;var Z=Math.max(U,aa)+T;var X=Math.max(O,R)+T;if(W>=ab&&W<=Z&&N>=Y&&N<=X){return 1
}else{if(S>=ab&&S<=Z&&K>=Y&&K<=X){return 1}else{if(Q>=ab&&Q<=Z&&P>=Y&&P<=X){return 1}else{if(V>=ab&&V<=Z&&M>=Y&&M<=X){return 1}}}}var L=Math.min(W,Q,S);var J=Math.min(N,P,K);var I=Math.max(W,Q,S);var i=Math.max(N,P,K);if(L>Z||I<ab||J>X||i<Y){return 0}return 1};A.prototype.checkBezierInBox=function(M,L,U,R,K,Q,J,P,i,O,N){function I(Y,X,W,V){return(1-V)*(1-V)*Y+2*(1-V)*V*X+V*V*W}function T(W){var V=I(K,J,i,W);var X=I(Q,P,O,W);return M<=V&&V<=U&&L<=X&&X<=R}for(var S=0;S<=1;S+=0.25){if(!T(S)){return false
}}return true};A.prototype.checkStraightEdgeInBox=function(K,J,P,O,I,N,i,M,L){return K<=I&&I<=P&&K<=i&&i<=P&&J<=N&&N<=O&&J<=M&&M<=O};A.prototype.checkStraightEdgeCrossesBox=function(V,P,ac,S,X,L,W,I,U){var ad=Math.min(V,ac)-U;var aa=Math.min(P,S)-U;var ab=Math.max(V,ac)+U;var Z=Math.max(P,S)+U;var M=W-X;var R=X;var Y;var J=I-L;var Q=L;var T;if(Math.abs(M)<0.0001){return(X>=ad&&X<=ab&&Math.min(L,I)<=aa&&Math.max(L,I)>=Z)}var i=(ad-R)/M;if(i>0&&i<=1){Y=J*i+Q;if(Y>=aa&&Y<=Z){return true}}var O=(ab-R)/M;
if(O>0&&O<=1){Y=J*O+Q;if(Y>=aa&&Y<=Z){return true}}var K=(aa-Q)/J;if(K>0&&K<=1){T=M*K+R;if(T>=ad&&T<=ab){return true}}var N=(Z-Q)/J;if(N>0&&N<=1){T=M*N+R;if(T>=ad&&T<=ab){return true}}return false};A.prototype.checkBezierCrossesBox=function(aa,M,an,ac,U,ax,Q,au,O,aq,I){var W=Math.min(aa,an)-I;var T=Math.min(M,ac)-I;var aw=Math.max(aa,an)+I;var at=Math.max(M,ac)+I;if(U>=W&&U<=aw&&ax>=T&&ax<=at){return true}else{if(O>=W&&O<=aw&&aq>=T&&aq<=at){return true}}var Z=U-2*Q+O;var L=-2*U+2*Q;var aj=U;var ad=[];
if(Math.abs(Z)<0.0001){var ao=(W-U)/L;var ab=(aw-U)/L;ad.push(ao,ab)}else{var ag=L*L-4*Z*(aj-W);var X,V;if(ag>0){var ah=Math.sqrt(ag);X=(-L+ah)/(2*Z);V=(-L-ah)/(2*Z);ad.push(X,V)}var af=L*L-4*Z*(aj-aw);var S,P;if(af>0){var ah=Math.sqrt(af);S=(-L+ah)/(2*Z);P=(-L-ah)/(2*Z);ad.push(S,P)}}ad.sort(function(az,ay){return az-ay});var Y=ax-2*au+aq;var J=-2*ax+2*au;var ai=ax;var am=[];if(Math.abs(Y)<0.0001){var K=(T-ax)/J;var ak=(at-ax)/J;am.push(K,ak)}else{var R=J*J-4*Y*(ai-T);var i,av;if(R>0){var ah=Math.sqrt(R);
i=(-J+ah)/(2*Y);av=(-J-ah)/(2*Y);am.push(i,av)}var N=J*J-4*Y*(ai-at);var ar,ap;if(N>0){var ah=Math.sqrt(N);ar=(-J+ah)/(2*Y);ap=(-J-ah)/(2*Y);am.push(ar,ap)}}am.sort(function(az,ay){return az-ay});for(var ae=0;ae<ad.length;ae+=2){for(var al=1;al<am.length;al+=2){if(ad[ae]<am[al]&&am[al]>=0&&ad[ae]<=1&&ad[ae+1]>am[al-1]&&am[al-1]<=1&&ad[ae+1]>=0){return true}}}return false};A.prototype.inLineVicinity=function(R,P,M,K,L,J,N){var S=N;var I=Math.min(M,L);var i=Math.max(M,L);var Q=Math.min(K,J);var O=Math.max(K,J);
return I-S<=R&&R<=i+S&&Q-S<=P&&P<=O+S};A.prototype.inBezierVicinity=function(O,N,Y,J,X,I,W,i,ad){var Q=0.25*Y+0.5*X+0.25*W;var P=0.25*J+0.5*I+0.25*i;var L={x1:Math.min(Y,W,Q),x2:Math.max(Y,W,Q),y1:Math.min(J,i,P),y2:Math.max(J,i,P)};if(O<L.x1||O>L.x2||N<L.y1||N>L.y2){return false}else{}var T,S,ab,Z;var M,K,R;var ac=function(af,al,ag,ae,ak,aj,ah,ai){M=(aj-ae)*(af-ag)+(ag-ak)*(al-ae);K=M*M;sideSquared=(aj-ae)*(aj-ae)+(ag-ak)*(ag-ak);if(ai){if(M>0){return false}}else{if(M<0){return false}}return(K/sideSquared>ah)
};var V=(Q+X)/2;var U=(P+I)/2;var aa=true;if(ac(V,U,Y,J,X,I,0,aa)){aa=!aa}return(!ac(O,N,Y,J,X,I,ad,aa)&&!ac(O,N,X,I,W,i,ad,aa)&&!ac(O,N,W,i,Y,J,ad,aa))};A.prototype.solveCubic=function(P,O,N,M,T){O/=P;N/=P;M/=P;var R,I,i,K,S,Q,J,L;I=(3*N-(O*O))/9;i=-(27*M)+O*(9*N-2*(O*O));i/=54;R=I*I*I+i*i;T[1]=0;J=(O/3);if(R>0){S=i+Math.sqrt(R);S=((S<0)?-Math.pow(-S,(1/3)):Math.pow(S,(1/3)));Q=i-Math.sqrt(R);Q=((Q<0)?-Math.pow(-Q,(1/3)):Math.pow(Q,(1/3)));T[0]=-J+S+Q;J+=(S+Q)/2;T[4]=T[2]=-J;J=Math.sqrt(3)*(-Q+S)/2;
T[3]=J;T[5]=-J;return}T[5]=T[3]=0;if(R==0){L=((i<0)?-Math.pow(-i,(1/3)):Math.pow(i,(1/3)));T[0]=-J+2*L;T[4]=T[2]=-(L+J);return}I=-I;K=I*I*I;K=Math.acos(i/Math.sqrt(K));L=2*Math.sqrt(I);T[0]=-J+L*Math.cos(K/3);T[2]=-J+L*Math.cos((K+2*Math.PI)/3);T[4]=-J+L*Math.cos((K+4*Math.PI)/3);return};A.prototype.sqDistanceToQuadraticBezier=function(S,R,X,M,W,L,V,K){var ad=1*X*X-4*X*W+2*X*V+4*W*W-4*W*V+V*V+M*M-4*M*L+2*M*K+4*L*L-4*L*K+K*K;var ac=1*9*X*W-3*X*X-3*X*V-6*W*W+3*W*V+9*M*L-3*M*M-3*M*K-6*L*L+3*L*K;var aa=1*3*X*X-6*X*W+X*V-X*S+2*W*W+2*W*S-V*S+3*M*M-6*M*L+M*K-M*R+2*L*L+2*L*R-K*R;
var Z=1*X*W-X*X+X*S-W*S+M*L-M*M+M*R-L*R;B("coefficients: "+ad/ad+", "+ac/ad+", "+aa/ad+", "+Z/ad);var N=[];this.solveCubic(ad,ac,aa,Z,N);var I=1e-7;var ab=[];for(var O=0;O<6;O+=2){if(Math.abs(N[O+1])<I&&N[O]>=0&&N[O]<=1){ab.push(N[O])}}ab.push(1);ab.push(0);var J=-1;var Y;var Q,P,T;for(var U=0;U<ab.length;U++){Q=Math.pow(1-ab[U],2)*X+2*(1-ab[U])*ab[U]*W+ab[U]*ab[U]*V;P=Math.pow(1-ab[U],2)*M+2*(1-ab[U])*ab[U]*L+ab[U]*ab[U]*K;T=Math.pow(Q-S,2)+Math.pow(P-R,2);B("distance for param "+ab[U]+": "+Math.sqrt(T));
if(J>=0){if(T<J){J=T;Y=ab[U]}}else{J=T;Y=ab[U]}}B("given: ( "+S+", "+R+"), ( "+X+", "+M+"), ( "+W+", "+L+"), ( "+V+", "+K+")");B("roots: "+N);B("params: "+ab);B("closest param: "+Y);return J};A.prototype.sqDistanceToFiniteLine=function(P,M,I,O,i,N){var J=[P-I,M-O];var S=[i-I,N-O];var K=S[0]*S[0]+S[1]*S[1];var R=J[0]*J[0]+J[1]*J[1];var Q=J[0]*S[0]+J[1]*S[1];var L=Q*Q/K;if(Q<0){return R}if(L>K){return(P-i)*(P-i)+(M-N)*(M-N)}return(R-L)};var B=function(){};z("renderer","canvas",A)})(cytoscape);(function(a){var b={ready:function(){},stop:function(){}};
...
function b(d){this.options=a.util.extend({},c,d)}b.prototype.run=function(){var A=this.options;var h=A;var g=A.cy;var v=g.nodes();var f=g.edges();var r=g.container();var u=r.clientWidth;var t=r.clientHeight;if(t==0||u==0){v.positions(function(){return{x:0,y:0}})}else{var i=v.size();var o=Math.sqrt(i*t/u);var m=Math.round(o);var p=Math.round(u/t*o);function l(C){if(C==undefined){return Math.min(m,p)}else{var B=Math.min(m,p);if(B==m){m=C}else{p=C}}}function e(C){if(C==undefined){return Math.max(m,p)
}else{var B=Math.max(m,p);if(B==m){m=C}else{p=C}}}if(h.rows!=null&&h.columns!=null){m=h.rows;p=h.columns}else{if(h.rows!=null&&h.columns==null){m=h.rows;p=Math.ceil(i/m)}else{if(h.rows==null&&h.columns!=null){p=h.columns;m=Math.ceil(i/p)}else{if(p*m>i){var z=l();var s=e();if((z-1)*s>=i){l(z-1)}else{if((s-1)*z>=i){e(s-1)}}}else{while(p*m<i){var z=l();var s=e();if((s+1)*z>=i){e(s+1)}else{l(z+1)}}}}}}var w=u/p;var d=t/m;var k=0;var j=0;v.positions(function(D,C){if(C.locked()){return false}var B=j*w+w/2;
var E=k*d+d/2;j++;if(j>=p){j=0;k++}return{x:B,y:E}})}if(A.fit){g.reset()}g.one("layoutready",A.ready);g.trigger("layoutready");g.one("layoutstop",A.stop);g.trigger("layoutstop")};b.prototype.stop=function(){};a("layout","grid",b)})(cytoscape);(function(a){var c={fit:true,ready:undefined,stop:undefined,positions:undefined,zoom:undefined,pan:undefined,padding:30};function b(d){this.options=a.util.extend(true,{},c,d)}b.prototype.run=function(){var g=this.options;var i=g.cy;var f=i.nodes();var e=i.edges();
var d=i.container();function h(j){if(g.positions==null){return null}if(g.positions[j._private.data.id]==null){return null}return g.positions[j._private.data.id]}f.positions(function(k,l){var j=h(l);if(l.locked()||j==null){return false}return j});if(g.pan!=null){i.pan(g.pan)}if(g.zoom!=null){i.zoom(g.zoom)}i.one("layoutready",g.ready);i.trigger("layoutready");if(g.fit){i.fit(g.padding)}i.one("layoutstop",g.stop);i.trigger("layoutstop")};a("layout","preset",b);a("core","presetLayout",function(){var f=this;
var d={};var e={};f.nodes().each(function(g,h){e[h.data("id")]=h.position()});d.positions=e;d.name="preset";d.zoom=f.zoom();d.pan=f.pan();return d})})(cytoscape);(function(a){var b={liveUpdate:true,ready:undefined,stop:undefined,maxSimulationTime:4000,fit:true,padding:[50,50,50,50],ungrabifyWhileSimulating:true,repulsion:undefined,stiffness:undefined,friction:undefined,gravity:true,fps:undefined,precision:undefined,nodeMass:undefined,edgeLength:undefined,stepSize:1,stableEnergy:function(d){var f=d;
return(f.max<=0.5)||(f.mean<=0.3)}};function c(d){this.options=a.util.extend({},b,d)}c.prototype.run=function(){var k=this.options;var j=k.cy;var w=j.nodes();var i=j.edges();var r=j.container();var v=r.clientWidth;var t=r.clientHeight;if(j.nodes().size()<=1){if(k.fit){j.reset()}j.nodes().position({x:Math.round(v/2),y:Math.round(t/2)});j.one("layoutstop",k.stop);j.trigger("layoutstop");j.one("layoutstop",k.stop);j.trigger("layoutstop");return}var m=this.system=arbor.ParticleSystem(k.repulsion,k.stiffness,k.friction,k.gravity,k.fps,k.dt,k.precision);
this.system=m;if(k.liveUpdate&&k.fit){j.reset()}var u=250;var g;var p=false;var f=+new Date;var s={init:function(A){},redraw:function(){var B=m.energy();if(k.stableEnergy!=null&&B!=null&&B.n>0&&k.stableEnergy(B)){m.stop();return}clearTimeout(g);g=setTimeout(z,u);var C=[];m.eachNode(function(I,D){var H=I.name;var F=I.data;var E=F.element;if(E==null){return}var G=E._private.position;if(!E.locked()&&!E.grabbed()){G.x=D.x;G.y=D.y;C.push(E)}});var A=(+new Date-f)>=16;if(k.liveUpdate&&C.length>0&&A){new a.Collection(j,C).rtrigger("position");
f=+new Date}if(!p){p=true;j.one("layoutready",k.ready);j.trigger("layoutready")}}};m.renderer=s;m.screenSize(v,t);m.screenPadding(k.padding[0],k.padding[1],k.padding[2],k.padding[3]);m.screenStep(k.stepSize);function o(A,B){if(B==null){return undefined}else{if(typeof B==typeof function(){}){return B.apply(A,[A._private.data,{nodes:w.length,edges:i.length,element:A}])}else{return B}}}function d(E){var H=E.x;var F=E.y;var I=v;var C=t;var B=-2;var J=2;var G=-2;var A=2;var D=4;return{x:H/I*D+B,y:F/C*D+J}
}var l=function(B){grabbed=this;var C=m.fromScreen(this.position());var A=arbor.Point(C.x,C.y);this.scratch().arbor.p=A;switch(B.type){case"grab":this.scratch().arbor.fixed=true;break;case"dragstop":this.scratch().arbor.fixed=false;this.scratch().arbor.tempMass=1000;break}};w.bind("grab drag dragstop",l);w.each(function(C,D){var F=this._private.data.id;var B=o(this,k.nodeMass);var A=this._private.locked;var E=d({x:D.position().x,y:D.position().y});if(D.locked()){return}this.scratch().arbor=m.addNode(F,{element:this,mass:B,fixed:A,x:A?E.x:undefined,y:A?E.y:undefined})
});i.each(function(){var D=this.id();var B=this.source().id();var C=this.target().id();var A=o(this,k.edgeLength);this.scratch().arbor=m.addEdge(B,C,{length:A})});function h(A){if(k.fit){j.fit()}A()}var e=w.filter(":grabbable");if(k.ungrabifyWhileSimulating){e.ungrabify()}var z=function(){if(window.isIE){h(function(){A()})}else{A()}function A(){if(!k.liveUpdate){if(k.fit){j.reset()}j.nodes().rtrigger("position")}w.unbind("grab drag dragstop",l);if(k.ungrabifyWhileSimulating){e.grabify()}j.one("layoutstop",k.stop);
j.trigger("layoutstop")}};m.start();setTimeout(function(){m.stop()},k.maxSimulationTime)};c.prototype.stop=function(){if(this.system!=null){system.stop()}};a("layout","arbor",c)})(cytoscape);(function(a){var c={fit:true,ready:undefined,stop:undefined,rStepSize:10,padding:30,startAngle:3/2*Math.PI,counterclockwise:false};function b(d){this.options=a.util.extend({},c,d)}b.prototype.run=function(){var o=this.options;var z=o;var m=o.cy;var f=m.nodes();var p=m.edges();var g=m.container();var h=g.clientWidth;
var w=g.clientHeight;var e={x:h/2,y:w/2};var v=50;var j=z.startAngle;var t=2*Math.PI/f.length;var u=0;for(var s=0;s<f.length;s++){var l=f[s];u=Math.max(l.outerWidth(),l.outerHeight())}var d=h/2-u;function k(){var r=0;var i=t;var C={x:e.x+d*Math.cos(r),y:e.y+d*Math.sin(r)};var A={x:e.x+d*Math.cos(i),y:e.y+d*Math.sin(i)};var B=Math.sqrt((A.x-C.x)*(A.x-C.x)+(A.y-C.y)*(A.y-C.y));return B}while(k()<u){d+=z.rStepSize}var s=0;f.positions(function(){var i=this;var A=d*Math.cos(j);var r=d*Math.sin(j);var B={x:e.x+A,y:e.y+r};
s++;j=z.counterclockwise?j-t:j+t;return B});if(o.fit){m.fit(z.padding)}m.one("layoutready",o.ready);m.trigger("layoutready");m.one("layoutstop",o.stop);m.trigger("layoutstop")};b.prototype.stop=function(){};a("layout","circle",b)})(cytoscape);(function(a){var b={fit:true,ready:undefined,stop:undefined,directed:true,padding:30,circle:false,roots:undefined};function c(d){this.options=a.util.extend({},b,d)}c.prototype.run=function(){var O=this.options;var m=O;var j=O.cy;var G=j.nodes();var g=j.edges();
var B=j.container();var F=B.clientWidth;var D=B.clientHeight;var o;if(a.is.elementOrCollection(m.roots)){o=m.roots}else{if(a.is.array(m.roots)){var J=[];for(var K=0;K<m.roots.length;K++){var E=m.roots[K];var A=j.getElementById(E);o.push(A)}o=new a.Collection(j,J)}else{o=G.roots()}}var r=[];var l={};var p={};o.bfs(function(h,R){var w=this[0];if(!r[R]){r[R]=[]}r[R].push(w);l[w.id()]=true;p[w.id()]=R},m.directed);var N=[];for(var K=0;K<G.length;K++){var A=G[K];if(l[A.id()]){continue}else{N.push(A)}}var f=N.length*3;
var d=0;while(N.length!==0&&d<f){var I=N.shift();var H=I.neighborhood().nodes();var M=false;for(var K=0;K<H.length;K++){var Q=p[H[K].id()];if(Q!==undefined){r[Q].push(I);M=true;break}}if(!M){N.push(I)}d++}while(N.length!==0){var I=N.shift();var s=I.bfs();var M=false;for(var K=0;K<s.length;K++){var Q=p[s[K].id()];if(Q!==undefined){r[Q].push(I);M=true;break}}if(!M){if(r.length===0){r.push([])}r[0].push(I)}}function k(){for(var R=0;R<r.length;R++){var h=r[R];for(var w=0;w<h.length;w++){var S=h[w];S._private.scratch.BreadthFirstLayout={depth:R,index:w}
}}}k();var C=0;for(var K=0;K<G.length;K++){var z=G[K].outerWidth();var L=G[K].outerHeight();C=Math.max(C,z,L)}C*=1.75;var e={};function v(Z){if(e[Z.id()]){return e[Z.id()]}var R=Z._private.scratch.BreadthFirstLayout.depth;var Y=Z.neighborhood().nodes();var T=0;var U=0;for(var w=0;w<Y.length;w++){var W=Y[w];var V=W.edgesWith(Z);var S=W._private.scratch.BreadthFirstLayout.index;var h=W._private.scratch.BreadthFirstLayout.depth;var X=r[h].length;if(R>h||R===0){T+=S/X;U++}}U=Math.max(1,U);T=T/U;if(U===0){T=undefined
}e[Z.id()]=T;return T}for(var u=0;u<3;u++){for(var K=0;K<r.length;K++){var Q=K;var t=[];r[K]=r[K].sort(function(i,h){var w=v(i);var R=v(h);return w-R})}k()}var P={x:F/2,y:D/2};G.positions(function(){var W=this[0];var i=W._private.scratch.BreadthFirstLayout;var w=i.depth;var R=i.index;var V=Math.max(F/(r[w].length+1),C);var U=Math.max(D/(r.length+1),C);var T=Math.min(F/2/r.length,D/2/r.length);T=Math.max(T,C);if(m.circle){var S=T*w+T-(r.length>0&&r[0].length<=3?T/2:0);var h=2*Math.PI/r[w].length*R;
if(w===0&&r[0].length===1){S=1}return{x:P.x+S*Math.cos(h),y:P.y+S*Math.sin(h)}}else{return{x:(R+1)*V,y:(w+1)*U}}});if(O.fit){j.fit(m.padding)}j.one("layoutready",O.ready);j.trigger("layoutready");j.one("layoutstop",O.stop);j.trigger("layoutstop")};c.prototype.stop=function(){};a("layout","breadthfirst",c)})(cytoscape);