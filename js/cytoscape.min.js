...
})(cytoscape);(function(d){var h=function(){return Date.now()};var e={};var g={};var a=b.prototype;function b(j){this.data={select:[0,0,0,0,0],renderer:this,cy:j.cy,container:j.cy.container(),curTouch:[null,null,null,null,0],prevTouch:[null,null,null,null,0],canvases:[null,null,null,null,null,[],[],[],[],[],false,false,false,false,false],banvases:[null,null],};this.hoverData={down:null,last:null,downTime:null,triggerMode:null,dragging:false,initialPan:[null,null],capture:false};this.timeoutData={panTimeout:null};
this.dragData={possibleDragElements:[]};this.touchData={start:null,capture:false,now:[null,null,null,null,null,null],earlier:[null,null,null,null,null,null]};this.zoomData={freeToZoom:false,lastPointerX:null};this.redraws=0;this.init();for(var k=0;k<5;k++){this.data.canvases[k]=document.createElement("canvas");this.data.canvases[k].style.position="absolute";this.data.canvases[k].id="layer"+k;this.data.canvases[k].style.zIndex=String(-k);this.data.canvases[k].style.visibility="hidden";this.data.container.appendChild(this.data.canvases[k])
}for(var k=0;k<2;k++){this.data.banvases[k]=document.createElement("canvas");this.data.banvases[k].style.position="absolute";this.data.banvases[k].id="buffr"+k;this.data.banvases[k].style.zIndex=String(-k);this.data.banvases[k].style.visibility="visible";this.data.container.appendChild(this.data.banvases[k])}}b.prototype.notify=function(i){if(i.type=="add"||i.type=="remove"){this.updateNodesCache();this.updateEdgesCache()}if(i.type=="load"){this.load()}if(i.type=="viewport"){this.data.canvases[10+0]=true;
this.data.canvases[5+0].push("viewchange");this.data.canvases[10+2]=true;this.data.canvases[5+2].push("viewchange");this.data.canvases[10+4]=true;this.data.canvases[5+4].push("viewchange")}this.data.canvases[10+2]=true;this.data.canvases[5+2].push("x");this.data.canvases[10+4]=true;this.data.canvases[5+4].push("x");this.redraws++;this.redraw()};b.prototype.load=function(){var i=this;i.data.container.addEventListener("mousedown",function(u){i.hoverData.capture=true;var o=i.data.cy;var w=i.projectIntoViewport(u.pageX,u.pageY);
var x=i.data.select;var t=i.findNearestElement(w[0],w[1]);var y=i.hoverData.down;var v=i.dragData.possibleDragElements;if(u.button==0){if(t&&t._private.grabbable){if(t._private.group=="nodes"&&t._private.selected==false){t._private.grabbed=true;t.trigger(new d.Event(u,{type:"grab"}));var n=new d.Event(u,{type:"unselect"});var m=new d.Event(u,{type:"free"});for(var s=0;s<v.length;s++){var p=v[s];var z=false;if(p._private.selected||p._private.grabbed){z=true}if(p._private.selected){p._private.selected=false;
p.trigger(n)}if(p._private.grabbed){p._private.grabbed=false;p.trigger(m)}if(z){p.updateStyle(false)}}i.dragData.possibleDragElements=v=[];v.push(t);for(var s=0;s<t._private.edges.length;s++){t._private.edges[s]._private.grabbed=true}}if(t._private.group=="nodes"&&t._private.selected==true){var k=new d.Event(u,{type:"grab"});for(var s=0;s<v.length;s++){v[s]._private.grabbed=true;var l=v[s]._private.edges;for(var r=0;r<l.length;r++){l[r]._private.grabbed=true}v[s].trigger(k)}}i.data.canvases[10+2]=true;
i.data.canvases[5+2].push("Single node moved to drag layer");i.data.canvases[10+4]=true;i.data.canvases[5+4].push("Single node moved to drag layer")}if(t){t.trigger(new d.Event(u,{type:"mousedown"}))}i.hoverData.down=t;i.hoverData.downTime=(new Date()).getTime();if(t==null){x[4]=1}}else{if(u.button==1){i.hoverData.dragging=true;i.hoverData.initialPan=[o.pan().x,o.pan().y]}}x[0]=x[2]=w[0];x[1]=x[3]=w[1];i.redraw()},false);window.addEventListener("mousemove",function(s){var m=i.data.cy;var u=i.projectIntoViewport(s.pageX,s.pageY);
var v=i.data.select;var r=i.findNearestElement(u[0],u[1]);var x=i.hoverData.last;var w=i.hoverData.down;var y=[u[0]-v[2],u[1]-v[3]];var l=i.getCachedNodes();var n=i.getCachedEdges();var t=i.dragData.possibleDragElements;var z=i.hoverData.capture;if(!z){return}if(i.hoverData.dragging){m.panBy({x:y[0]*m.zoom(),y:y[1]*m.zoom()});u=i.projectIntoViewport(s.pageX,s.pageY);var k=new d.Event(s,{type:"mousemove"});if(r!=null){r.trigger(k)}else{if(r==null){m.trigger(k)}}}else{if(v[4]==1&&w==null&&(new Date()).getTime()-i.hoverData.downTime>200&&(Math.abs(v[3]-v[1])+Math.abs(v[2]-v[0])<4)){i.hoverData.dragging=true;
