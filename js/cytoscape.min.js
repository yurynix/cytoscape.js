...
var cytoscape;(function(){var a=cytoscape=function(){return cytoscape.init.apply(cytoscape,arguments)};a.init=function(b){if(a.is.plainObject(b)){return new a.Core(b)}else{if(a.is.string(b)){return a.extension.apply(a.extension,arguments)}}};a.fn={};if(typeof exports!=="undefined"){exports=module.exports=cytoscape}})();(function(a){a.is={string:function(b){return b!=null&&typeof b==typeof""},fn:function(b){return b!=null&&typeof b===typeof function(){}},array:function(b){return b!=null&&b instanceof Array
...
var d=a.is.element(K);var C=d;var G;if(f){G=r}else{G=new a.Event(r,{cyTarget:K,cy:A,namespace:r.namespace});var F=a.define.event.props;for(var U=0;U<F.length;U++){var x=F[U];G[x]=r[x]}}if(y){P=[{namespace:G.namespace,type:G.type,callback:y}]}for(var U=0;U<P.length;U++){var e=P[U];var N=!e.namespace||e.namespace===G.namespace;var w=e.type===G.type;var O=e.delegated?(K!==G.cyTarget&&a.is.element(G.cyTarget)&&G.cyTarget.is(e.selector)):(true);var H=N&&w&&O;if(H){var Q=[G];Q=Q.concat(g);if(e.data){G.data=e.data
}else{G.data=undefined}if(e.unbindSelfOnTrigger||e.unbindAllBindersOnTrigger){P.splice(U,1);U--}if(e.unbindAllBindersOnTrigger){var u=e.binders;for(var T=0;T<u.length;T++){var t=u[T];if(!t||t===K){continue}var B=t._private.listeners;for(var S=0;S<B.length;S++){var Z=B[S];if(Z===e){B.splice(S,1);S--}}}}var aa=e.delegated?G.cyTarget:K;var I=e.callback.apply(aa,Q);if(I===false||G.isPropagationStopped()){C=false;if(I===false){G.stopPropagation();G.preventDefault()}}}}if(C){var s=K.parent();var X=s.length!==0;
if(X){s=s[0];s.trigger(G)}else{A.trigger(G)}}}}return o}}}})(cytoscape);(function(a,b){a.Style=function(c){if(!(this instanceof a.Style)){return new a.Style(c)}if(!a.is.core(c)){a.util.error("A style must have a core reference");return}this._private={cy:c,coreStyle:{}};this.length=0;this.addDefaultStylesheet()};a.style=a.Style;a.styfn=a.Style.prototype;a.fn.style=function(c,d){for(var f in c){var e=c[f];a.Style.prototype=e}};a.stylesheet=a.Stylesheet=function(){if(!(this instanceof a.Stylesheet)){return new a.Stylesheet()
...
})(cytoscape);(function(W){var E=function(ag){if(false){console.log(ag)}};var O={minZoom:0.001,maxZoom:1000,maxPan:-1>>>1,minPan:(-(-1>>>1)-1),selectionToPanDelay:500,dragToSelect:true,dragToPan:true,};var I={};var U=[0,0,0,0,0];var g;var f;var p;var ad=false;var v=false;var b=false;var K;var h=[];var T=999;var aa;var H=[];var x=999;var M={};var o={};var af={};var G={};var A={};var C={};var i={};var j=false;var F=false;var m;var u=false;var c=[];var l=[];var B;var R;var r=new Array(2);var S=new Array(2);
var y;var k=new Array(2);var L=new Array(2);var n;var D=false;var N=0;var d=0;var z=undefined;var a=undefined;var Y=false;var Q=false;var t=undefined;var Z=undefined;var P=false;var e=false;var X=false;var s=undefined;var ae=undefined;var ac=0;var V=undefined;function ab(aj){this.options=$.extend(true,{},O,aj);this.cy=aj.cy;B=aj.cy;this.init();this.nodePairEdgeData={};var al=5;this.canvases=new Array(al);this.canvasContexts=new Array(al);var ai=2;this.bufferCanvases=new Array(ai);this.bufferCanvasContexts=new Array(ai);
this.canvasNeedsRedraw=new Array(al);this.redrawReason=new Array(al);var ag=this.options.cy.container();this.container=ag;setInterval(function(){ac++},450);for(var ak=0;ak<al+ai;ak++){var ah=document.createElement("canvas");ah.width=ag.clientWidth;ah.height=ag.clientHeight;ah.style.position="absolute";if(ak<al){ah.id="layer"+ak;ah.style.zIndex=String(-ak-ai);ah.style.visibility="hidden";this.canvases[ak]=ah;this.canvasContexts[ak]=ah.getContext("2d");this.canvasNeedsRedraw[ak]=false;this.redrawReason[ak]=new Array()
}else{ah.id="buffer"+(ak-al);ah.style.zIndex=-(ak-al);this.bufferCanvases[ak-al]=ah;this.bufferCanvasContexts[ak-al]=ah.getContext("2d")}ag.appendChild(ah)}this.bufferCanvases[0].style.visibility="visible";this.bufferCanvases[1].style.visibility="hidden";this.canvas=this.bufferCanvases[0];this.context=this.bufferCanvasContexts[0];this.center=[ag.clientWidth/2,ag.clientHeight/2];this.scale=[1,1];this.zoomLevel=0;R=this}ab.prototype.notify=function(ag){if(ag.type=="load"){this.load();this.canvasNeedsRedraw[2]=true;
this.redrawReason[2].push("Load");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Load");this.redraw()}else{if(ag.type=="viewport"){if(!D){this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Viewport change");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Viewport change");this.redraw()}else{D=false}}else{if(ag.type=="style"){w=true;this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Style change");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Style change");
this.redraw()}else{if(ag.type=="add"||ag.type=="remove"){this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Elements added/removed");this.redraw()}else{if(ag.type=="draw"){this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Draw call");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Draw call");this.redraw()}else{if(ag.type=="position"){this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Position call");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Position call");
this.redraw()}else{console.log("event: "+ag.type)}}}}}}};ab.prototype.projectMouse=function(aj){var ag,al;var ai=0;var ah=0;var ak;ak=B.container();while(ak!=null){if(typeof(ak.offsetLeft)=="number"){ai+=ak.offsetLeft;ah+=ak.offsetTop}ak=ak.parentNode}ag=aj.pageX-ai;al=aj.pageY-ah;ag-=B.pan().x;al-=B.pan().y;ag/=B.zoom();al/=B.zoom();return[ag,al]};ab.prototype.findEdgeMetrics=function(ag){this.nodePairEdgeData={};var aj,ai;for(var ah=0;ah<ag.length;ah++){aj=ag[ah];ai=aj._private.data.source<=aj._private.data.target?aj._private.data.source+aj._private.data.target:aj._private.data.target+aj._private.data.source;
if(this.nodePairEdgeData[ai]==undefined){this.nodePairEdgeData[ai]=1}else{this.nodePairEdgeData[ai]++}aj._private.rscratch.nodePairId=ai;aj._private.rscratch.nodePairEdgeNum=this.nodePairEdgeData[ai]}};ab.prototype.findEdges=function(ak){var ah=B.edges();var aj={};var ag=[];for(var ai=0;ai<ak.length;ai++){aj[ak[ai]._private.data.id]=ak[ai]}for(var ai=0;ai<ah.length;ai++){if(aj[ah[ai]._private.data.source]||aj[ah[ai]._private.data.target]){ag.push(ah[ai])}}return ag};ab.prototype.findEdgeControlPoints=function(am){var at={};
var ap;for(var an=0;an<am.length;an++){ap=am[an]._private.data.source>am[an]._private.data.target?am[an]._private.data.target+am[an]._private.data.source:am[an]._private.data.source+am[an]._private.data.target;if(at[ap]==undefined){at[ap]=[]}at[ap].push(am[an])}var ah,al;for(var ap in at){ah=B.getElementById(at[ap][0]._private.data.source);al=B.getElementById(at[ap][0]._private.data.target);var ar=(ah._private.position.x+al._private.position.x)/2;var aq=(ah._private.position.y+al._private.position.y)/2;
var ai,ag;if(at[ap].length>1){ai=al._private.position.y-ah._private.position.y;ag=ah._private.position.x-al._private.position.x;var ao=Math.sqrt(ai*ai+ag*ag);ai/=ao;ag/=ao}var aj;for(var an=0;an<at[ap].length;an++){aj=at[ap][an];if(ah._private.data.id==al._private.data.id){var au=aj._private.style["control-point-step-size"].pxValue;aj._private.rscratch.isSelfEdge=true;aj._private.rscratch.cp2ax=ah._private.position.x;aj._private.rscratch.cp2ay=ah._private.position.y-1.3*au*(an/3+1);aj._private.rscratch.cp2cx=ah._private.position.x-1.3*au*(an/3+1);
aj._private.rscratch.cp2cy=ah._private.position.y;aj._private.rscratch.selfEdgeMidX=(aj._private.rscratch.cp2ax+aj._private.rscratch.cp2cx)/2;aj._private.rscratch.selfEdgeMidY=(aj._private.rscratch.cp2ay+aj._private.rscratch.cp2cy)/2}else{if(at[ap].length%2==1&&an==Math.floor(at[ap].length/2)){aj._private.rscratch.isStraightEdge=true}else{var au=aj._private.style["control-point-step-size"].value;var ak=(0.5-at[ap].length/2+an)*au;aj._private.rscratch.isBezierEdge=true;aj._private.rscratch.cp2x=ar+ai*ak;
aj._private.rscratch.cp2y=aq+ag*ak}}}}return at};ab.prototype.findEdgeControlPoints2=function(ah){var ak={};var ag;for(var aj=0;aj<ah.length;aj++){if(ak[ah[aj]._private.data.id]==undefined){ag=ah[aj].parallelEdges();for(var ai=0;ai<ah.length;ai++){ak[ah[aj]._private.data.id]=true}W.styfn.calculateControlPoints(ag)}}};ab.prototype.checkRecordPinchCoordinates=function(ag){if(ag.touches.length>=2){prevTouch1=ag.touches[0];prevTouch2=ag.touches[1];prevTouch1.offsetX=prevTouch1.clientX+R.canvas.parentElement.offsetLeft;
prevTouch1.offsetY=prevTouch1.clientY+R.canvas.parentElement.offsetTop;prevTouch2.offsetX=prevTouch2.clientX+R.canvas.parentElement.offsetLeft;prevTouch2.offsetY=prevTouch2.clientY+R.canvas.parentElement.offsetTop}else{prevTouch1=undefined;prevTouch2=undefined}};ab.prototype.mouseDownHandler=function(ag){var ah=B.nodes();var ak=B.edges();var am=false;var ai=ag;if(ag.changedTouches){ag.preventDefault();am=true;if(ag.touches.length>=2){firstTouchFinger=ag.touches[0];secondTouchFinger=ag.touches[1];
var ap=[R.canvas.parentElement.offsetLeft,-R.canvas.parentElement.offsetTop];k[0]=ag.touches[0].clientX+ap[0];k[1]=ag.touches[0].clientY+ap[1];L[0]=ag.touches[1].clientX+ap[0];L[1]=ag.touches[1].clientY+ap[1];n=Math.sqrt(Math.pow(L[0]-k[0],2)+Math.pow(L[1]-k[1],2))}ag=ag.changedTouches[0];ag.button=0;ag.touch=1;R.mouseMoveHelper.hoverHandler(ah,ak,ag)}var an=ag;V=ac;clearTimeout(this.panTimeout);if(!aa&&!am){this.panTimeout=setTimeout(function(){if(V!==undefined&&!am&&ag.button===0){g=an.clientX;
f=an.clientY;ad=true;if(B.renderer().canvas.style.cursor!=B.style()._private.coreStyle["panning-cursor"].value){B.renderer().canvas.style.cursor=B.style()._private.coreStyle["panning-cursor"].value}U[4]=0;R.canvasNeedsRedraw[0]=true;R.redrawReason[0].push("selection boxed removed");V=undefined}},250)}if((!ag.touch&&an.button==1&&an.target==B.renderer().canvas)||(ag.touch&&aa==undefined&&K==undefined)){g=an.clientX;f=an.clientY;ad=true;if(B.renderer().canvas.style.cursor!=B.style()._private.coreStyle["panning-cursor"].value){B.renderer().canvas.style.cursor=B.style()._private.coreStyle["panning-cursor"].value
}}Y=true;Q=true;var aj=B.renderer().projectMouse(ag);U[0]=aj[0];U[1]=aj[1];if(an.button==0&&an.target==B.renderer().canvas&&aa==undefined&&K==undefined&&!am){U[4]=1}if(an.button==0){if(aa!=undefined&&aa.grabbable()){j=true;c=[];if(aa.selected()){F=true;for(var ao=0;ao<ah.length;ao++){if(ah[ao].selected()&&ah[ao].grabbable()){ah[ao]._private.rscratch.dragStartX=ah[ao]._private.position.x;ah[ao]._private.rscratch.dragStartY=ah[ao]._private.position.y;c.push(ah[ao]);ah[ao]._private.grabbed=true;ah[ao].trigger("grab")
}}}else{if(aa.grabbable()){F=false;m=aa;m._private.rscratch.dragStartX=m._private.position.x;m._private.rscratch.dragStartY=m._private.position.y;c.push(m);m._private.rscratch.layer2=true;m._private.grabbed=true;m.trigger("grab")}}l=R.findEdges(c);for(var al=0;al<l.length;al++){}u=false;if(am){aa.trigger("touchstart")}else{aa.trigger("mousedown")}z=aa}else{if(K!=undefined){if(am){K.trigger("touchstart")}else{K.trigger("mousedown")}a=K}else{if(am){B.trigger("touchstart")}else{B.trigger("mousedown")
}Y=true}}}B.renderer().redraw()};ab.prototype.mouseOverHandler=function(ag){e=true;P=true};ab.prototype.mouseOutHandler=function(ag){X=false;P=false;B.trigger("mouseout");s=undefined};ab.prototype.touchStartHandler=function(ag){};ab.prototype.touchStartHandler=function(ag){};ab.prototype.documentMouseMoveHandler=function(ah){var ai=false;var ag=ah;if(ah.touches){ai=true;ag=ah.touches[0]}};ab.prototype.mouseMoveHandler=function(az){P=true;N=setTimeout(function(){N=null},1000/100);var aw=az;var an=false;
if(az.touches){az.preventDefault();an=true;if(az.touches.length>=2){var ay=[R.canvas.parentElement.offsetLeft,-R.canvas.parentElement.offsetTop];r[0]=aw.touches[0].clientX+ay[0];r[1]=aw.touches[0].clientY+ay[1];S[0]=aw.touches[1].clientX+ay[0];S[1]=aw.touches[1].clientY+ay[1];y=Math.sqrt(Math.pow(L[0]-k[0],2)+Math.pow(L[1]-k[1],2));var aq=[r[0]-k[0],r[1]-k[1]];var ap=[S[0]-L[0],S[1]-L[1]];var ag=[(aq[0]+ap[0])/2,(ap[1]+ap[1])/2];var ak=y/n;if(ak>1){ak=(ak-1)*1.5+1}else{ak=1-(1-ak)*1.5}D=true;B.panBy({x:ag[0],y:ag[1]});
B.zoom({level:B.zoom()*ak,position:{x:(r[0]+S[0])/2,y:(r[1]+S[1])/2}});k[0]=r[0];k[1]=r[1];L[0]=S[0];L[1]=S[1];n=y;return}az=az.touches[0];az.button=0}var av=aw;var aj=R.mouseMoveHelper.dragHandler;var aB=R.mouseMoveHelper.checkBezierEdgeHover;var al=R.mouseMoveHelper.checkStraightEdgeHover;var aA=R.mouseMoveHelper.checkNodeHover;var ah=R.mouseMoveHelper.hoverHandler;var ai=B.edges();var at=B.nodes();V=undefined;if(ad){aj(az)}var ar=B.renderer().projectMouse(az);ae=az.screenX;if(s!==undefined&&Math.abs(s-ae)>1){X=true
}s=ae;U[2]=ar[0];U[3]=ar[1];if(!U[4]){ah(at,ai,az)}z=undefined;a=undefined;Q=false;if(aa!=undefined){if(B.renderer().canvas.style.cursor!=aa._private.style.cursor.value){B.renderer().canvas.style.cursor=aa._private.style.cursor.value}if(t!==aa){if(t!==undefined){if(an){}else{t.trigger("mouseout")}}t=aa;var ao=aa.grabbed();if(an&&!ao){}else{if(!an&&!ao){aa.trigger("mouseover")}}}else{if(an){aa.trigger("touchmove")}else{aa.trigger("mousemove")}}}else{if(K!=undefined){if(B.renderer().canvas.style.cursor!=K._private.style.cursor.value){B.renderer().canvas.style.cursor=K._private.style.cursor.value
}if(Z!==K){if(Z!==undefined){if(an){}else{Z.trigger("mouseout")}}Z=K;var au=K.grabbed();if(an&&!au){}else{if(!an&&!au){K.trigger("mouseover")}}}else{if(an){K.trigger("touchmove")}else{K.trigger("mousemove")}}}else{if(!aa&&!K&&B.renderer().canvas.style.cursor!="default"){B.renderer().canvas.style.cursor="default"}if(Z!==undefined){if(an){}else{Z.trigger("mouseout")}Z=undefined}if(t!==undefined){if(an){}else{t.trigger("mouseout")}t=undefined}if(e){if(an){}else{B.trigger("mouseover")}P=true;e=false}else{if(P){if(an){B.trigger("touchmove")
}else{B.trigger("mousemove")}}}}}if(j){if(!u){for(var ax=0;ax<c.length;ax++){c[ax]._private.rscratch.layer2=true}for(var ax=0;ax<l.length;ax++){l[ax]._private.rscratch.layer2=true}R.canvasNeedsRedraw[4]=true;R.redrawReason[4].push("nodes being dragged, moved to drag layer");R.canvasNeedsRedraw[2]=true;R.redrawReason[2].push("nodes being dragged, moved to drag layer");u=true}for(var am=0;am<at.length;am++){if((F&&at[am].selected())||(!F&&at[am]==m)){if(!at[am]._private.locked&&at[am]._private.grabbable){at[am]._private.position.x=at[am]._private.rscratch.dragStartX+(U[2]-U[0]);
at[am]._private.position.y=at[am]._private.rscratch.dragStartY+(U[3]-U[1]);at[am].trigger("drag");at[am].trigger("position")}}}R.canvasNeedsRedraw[2]=true;R.redrawReason[2].push("nodes being dragged")}if(U[4]){R.canvasNeedsRedraw[0]=true;R.redrawReason[0].push("selection boxed moved")}if(ad||j||U[4]){B.renderer().redraw()}};ab.prototype.mouseUpHandler=function(aw){var aq=undefined;if(aw.changedTouches){aw.preventDefault();aq=aw;aw=aw.changedTouches[0];aw.button=0;U[2]=R.projectMouse(aw)[0];U[3]=R.projectMouse(aw)[1]
}var av=aw;var ai=B.edges();var at=B.nodes();var ap=j&&(Math.abs(U[2]-U[0])+Math.abs(U[3]-U[1])>1);for(var ay=0;ay<c.length;ay++){c[ay]._private.rscratch.layer2=false;c[ay]._private.grabbed=false;c[ay].trigger("free")}c=[];for(var ay=0;ay<l.length;ay++){l[ay]._private.rscratch.layer2=false}var ax=undefined;if(aa!==undefined){ax=aa}else{if(K!==undefined){ax=K}}V=undefined;var ao=undefined;if(aq){ao="touchend"}else{ao="mouseup"}if(ax!=undefined){ax.trigger(ao);if(ax===z||ax===a){ax.trigger("click")
}}else{B.trigger(ao);if(Q){B.trigger("click")}}if(!v&&!ap){var am=B.collection();for(var ak=0;ak<at.length;ak++){at[ak]._private.rscratch.selected=false;if(at[ak]._private.selected){am=am.add(at[ak])}}for(var ak=0;ak<ai.length;ak++){ai[ak]._private.rscratch.selected=false;if(ai[ak]._private.selected){am=am.add(ai[ak])}}if(am.length>0){am.unselect()}}if(U[4]==1&&!j&&Math.abs(U[2]-U[0])+Math.abs(U[3]-U[1])>2){var an=2;var ag;var ar;var aA=B.collection();for(var ak=0;ak<ai.length;ak++){ag=ai[ak]._private.selected;
var aj;var ah=ai[ak]._private.rscratch;if(ai[ak]._private.rscratch.isStraightEdge){aj=W.math.boxInBezierVicinity(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,(ai[ak]._private.rscratch.startX+ai[ak]._private.rscratch.endX)/2,(ai[ak]._private.rscratch.startY+ai[ak]._private.rscratch.endY)/2,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)}else{if(ai[ak]._private.rscratch.isSelfEdge){aj=W.math.boxInBezierVicinity(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,ai[ak]._private.rscratch.cp2ax,ai[ak]._private.rscratch.cp2ay,ai[ak]._private.rscratch.selfEdgeMidX,ai[ak]._private.rscratch.selfEdgeMidY,an);
if(aj==0){aj=W.math.boxInBezierVicinity(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.selfEdgeMidX,ai[ak]._private.rscratch.selfEdgeMidY,ai[ak]._private.rscratch.cp2cx,ai[ak]._private.rscratch.cp2cy,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)}}else{aj=W.math.boxInBezierVicinity(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,ai[ak]._private.rscratch.cp2x,ai[ak]._private.rscratch.cp2y,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)
}}if(aj==2){ar=true}else{if(aj==1){if(ai[ak]._private.rscratch.isSelfEdge){ar=W.math.checkBezierCrossesBox(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,ai[ak]._private.rscratch.cp2ax,ai[ak]._private.rscratch.cp2ay,ai[ak]._private.rscratch.selfEdgeMidX,ai[ak]._private.rscratch.selfEdgeMidY,an);if(!ar){ar=W.math.checkBezierCrossesBox(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.selfEdgeMidX,ai[ak]._private.rscratch.selfEdgeMidY,ai[ak]._private.rscratch.cp2cx,ai[ak]._private.rscratch.cp2cy,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)
}}else{if(ai[ak]._private.rscratch.isStraightEdge){ar=W.math.checkStraightEdgeCrossesBox(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)}else{ar=W.math.checkBezierCrossesBox(U[0],U[1],U[2],U[3],ai[ak]._private.rscratch.startX,ai[ak]._private.rscratch.startY,ai[ak]._private.rscratch.cp2x,ai[ak]._private.rscratch.cp2y,ai[ak]._private.rscratch.endX,ai[ak]._private.rscratch.endY,an)}}}else{ar=false}}if(ar&&!ag){aA=aA.add(ai[ak])
}else{if(!ar&&ag){}}}var aE=Math.min(U[0],U[2]);var aD=Math.min(U[1],U[3]);var aC=Math.max(U[0],U[2]);var aB=Math.max(U[1],U[3]);var az,ar;var au,al;for(var ak=0;ak<at.length;ak++){az=at[ak]._private.selected;au=at[ak].position();al=at[ak]._private.data.weight/5;if(au.x>aE-al&&au.x<aC+al&&au.y>aD-al&&au.y<aB+al){ar=true;at[ak]._private.rscratch.selected=true}else{ar=false;at[ak]._private.rscratch.selected=false}if(ar&&!az){aA=aA.add(at[ak])}else{if(!ar&&az){}}}if(aA.length>0){aA.select()}}else{if(U[4]==0&&!ap){if(aa!=undefined){aa._private.rscratch.hovered=false;
aa._private.rscratch.selected=true;if(!aa._private.selected){aa.select()}}else{if(K!=undefined){K._private.rscratch.hovered=false;K._private.rscratch.selected=true;if(!K._private.selected){K.select()}}}}}ad=false;if(B.renderer().canvas.style.cursor!="default"){B.renderer().canvas.style.cursor="default"}U[4]=0;R.canvasNeedsRedraw[0]=true;R.redrawReason[0].push("Selection box gone");if(ap){R.canvasNeedsRedraw[2]=true;R.redrawReason[2].push("Node drag completed");R.canvasNeedsRedraw[4]=true;R.redrawReason[4].push("Node drag completed")
}j=false;B.renderer().redraw();if(aq&&aq.touches.length==1){g=aq.touches[0].clientX;f=aq.touches[0].clientY;ad=true;if(B.renderer().canvas.style.cursor!=B.style()._private.coreStyle["panning-cursor"].value){B.renderer().canvas.style.cursor=B.style()._private.coreStyle["panning-cursor"].value}}};ab.prototype.windowMouseDownHandler=function(ag){};ab.prototype.windowMouseMoveHandler=function(ag){};ab.prototype.windowMouseUpHandler=function(ag){};ab.prototype.mouseWheelHandler=function(ah){if(!X){return
}else{ah.preventDefault()}var ag=ah.wheelDeltaY;B.renderer().zoomLevel-=ag/5/500;B.renderer().scale[0]=Math.pow(10,-B.renderer().zoomLevel);B.renderer().scale[1]=Math.pow(10,-B.renderer().zoomLevel);var ai=B.renderer().projectMouse(ah);var aj=B.zoom()*Math.pow(10,ah.wheelDeltaY/500);aj=Math.min(aj,100);aj=Math.max(aj,0.01);B.zoom({level:aj,position:{x:ah.offsetX,y:ah.offsetY}})};ab.prototype.keyDownHandler=function(ag){if(ag.keyCode==16&&U[4]!=1){v=true}};ab.prototype.keyUpHandler=function(ag){if(ag.keyCode==16){U[4]=0;
v=false}};ab.prototype.mouseMoveHelper=function(){var aj=function(an){var am=an.clientX-g;var ao=an.clientY-f;B.panBy({x:am,y:ao});g=an.clientX;f=an.clientY};var ai=function(ap,an,ar){var ao=Math.pow(ar._private.style.width.value/2,2);var aq=false;if(W.math.inBezierVicinity(ap,an,ar._private.rscratch.startX,ar._private.rscratch.startY,ar._private.rscratch.cp2x,ar._private.rscratch.cp2y,ar._private.rscratch.endX,ar._private.rscratch.endY,ao)){var am=W.math.sqDistanceToQuadraticBezier(ap,an,ar._private.rscratch.startX,ar._private.rscratch.startY,ar._private.rscratch.cp2x,ar._private.rscratch.cp2y,ar._private.rscratch.endX,ar._private.rscratch.endY);
if(am<=ao){aq=true;if(am<T){K=ar;T=am}}}return aq};var ah=function(aq,ao,at){var ap=Math.pow(at._private.style.width.value/2,2);var ar=false;var am=false;if(W.math.inBezierVicinity(aq,ao,at._private.rscratch.startX,at._private.rscratch.startY,at._private.rscratch.cp2ax,at._private.rscratch.cp2ay,at._private.rscratch.selfEdgeMidX,at._private.rscratch.selfEdgeMidY)){var an=W.math.sqDistanceToQuadraticBezier(aq,ao,at._private.rscratch.startX,at._private.rscratch.startY,at._private.rscratch.cp2ax,at._private.rscratch.cp2ay,at._private.rscratch.selfEdgeMidX,at._private.rscratch.selfEdgeMidY);
if(an<ap){ar=true;if(an<T){K=at;T=an;am=true}}}if(!am&&W.math.inBezierVicinity(aq,ao,at._private.rscratch.selfEdgeMidX,at._private.rscratch.selfEdgeMidY,at._private.rscratch.cp2cx,at._private.rscratch.cp2cy,at._private.rscratch.endX,at._private.rscratch.endY)){var an=W.math.sqDistanceToQuadraticBezier(aq,ao,at._private.rscratch.selfEdgeMidX,at._private.rscratch.selfEdgeMidY,at._private.rscratch.cp2cx,at._private.rscratch.cp2cy,at._private.rscratch.endX,at._private.rscratch.endY);if(an<ap){ar=true;
if(an<T){K=at;T=an;am=true}}}return ar};var al=function(av,au,at,aD,ao,aC,am){var aE=Math.pow(at._private.style.width.value/2,2);var ap=av-aD;var an=au-ao;var az=av-aC;var ay=au-am;var aB=aC-aD;var aA=am-ao;var ax;var aw=false;if(ap*aB+an*aA<=0){ax=(Math.pow(aD-av,2)+Math.pow(ao-au,2))}else{if(az*aB+ay*aA>=0){ax=(Math.pow(aC-av,2)+Math.pow(am-au,2))}else{var ar=aA;var aq=-aB;ax=Math.pow(ap*ar+an*aq,2)/(ar*ar+aq*aq)}}if(ax<=aE){aw=true;if(ax<T){K=at;T=ax}}return aw};var ag=function(aq,ap,ar){var an=aq-ar.position().x;
var am=ap-ar.position().y;var ao=Math.pow(Math.max(ar._private.style.width.value,ar._private.style.height.value+ar._private.style["border-width"].value)/2,2);var at=an*an+am*am;if(ao>at){if(at<x){aa=ar;x=at;b=true}return true}return false};var ak=function(an,aq,am){var au=B.renderer().projectMouse(am);var ap=au[0];var ao=au[1];if(aa!=undefined){aa=undefined;x=99999}else{if(K!=undefined){K=undefined;T=99999}}b=false;for(var at=0;at<an.length;at++){ag(ap,ao,an[at])}var av=false;var ar=[];for(var at=0;
at<aq.length;at++){if(b){break}else{if(aq[at]._private.rscratch.isStraightEdge){av=al(ap,ao,aq[at],aq[at]._private.rscratch.startX,aq[at]._private.rscratch.startY,aq[at]._private.rscratch.endX,aq[at]._private.rscratch.endY)}else{if(aq[at]._private.rscratch.isSelfEdge){av=ah(ap,ao,aq[at])}else{av=ai(ap,ao,aq[at])}}}if(av){ar.push(aq[at])}av=false}if(ar.length>0){ar.sort(function(ax,aw){return aw._private.data.id.localeCompare(ax._private.data.id)});ar.sort(function(ax,aw){return aw._private.style["z-index"].value-ax._private.style["z-index"].value
});K=ar[0]}else{K=undefined}if(aa!=undefined){aa._private.rscratch.hovered=true}else{if(K!=undefined){K._private.rscratch.hovered=true}}};this.mouseMoveHelper.dragHandler=aj;this.mouseMoveHelper.checkBezierEdgeHover=ai;this.mouseMoveHelper.checkStraightEdgeHover=al;this.mouseMoveHelper.checkNodeHover=ag;this.mouseMoveHelper.hoverHandler=ak};ab.prototype.load=function(){var ag=this;this.mouseMoveHelper();document.addEventListener("keydown",this.keyDownHandler,false);document.addEventListener("keyup",this.keyUpHandler,false);
this.bufferCanvases[0].addEventListener("mousedown",this.mouseDownHandler,false);window.addEventListener("mouseup",this.mouseUpHandler,false);window.addEventListener("mousemove",this.mouseMoveHandler,false);this.bufferCanvases[0].addEventListener("mouseout",this.mouseOutHandler,false);this.bufferCanvases[0].addEventListener("mouseover",this.mouseOverHandler,false);window.addEventListener("mousedown",this.windowMouseDownHandler,false);window.addEventListener("mousemove",this.windowMouseMoveHandler,false);
window.addEventListener("mouseup",this.windowMouseUpHandler,false);this.bufferCanvases[0].addEventListener("mousewheel",this.mouseWheelHandler,false);this.bufferCanvases[0].addEventListener("touchstart",this.mouseDownHandler,true);this.bufferCanvases[0].addEventListener("touchmove",this.mouseMoveHandler,true);this.bufferCanvases[0].addEventListener("touchend",this.mouseUpHandler,true)};ab.prototype.init=function(){};ab.prototype.complexSqrt=function(al,ah,ai){var ak=Math.sqrt(al*al+ah*ah);var ag=Math.sqrt(0.5*(al+ak));
var aj=Math.sqrt(0.5*(ak-al));if(ah<-ai){aj=-aj}else{if(ah<ai){aj=0}}return[ag,aj]};ab.prototype.initStyle=function(){var aj=this.options.cy.nodes();var ai=this.options.cy.edges();var am;for(var ak=0;ak<aj.length;ak++){am=aj[ak];am._private.rscratch.override={};var ah=Math.max(Math.random(),0.6);am._private.rscratch.override.regularColor="rgba("+String(Math.floor(ah*100+125))+","+String(Math.floor(ah*100+125))+","+String(Math.floor(ah*100+125))+","+255+")";am._private.rscratch.override.regularBorderColor="rgba("+String(Math.floor(ah*70+160))+","+String(Math.floor(ah*70+160))+","+String(Math.floor(ah*70+160))+","+255+")";
var ag=Math.random();if(ag<10.35){am._private.rscratch.override.shape="ellipse"}else{if(ag<0.49){am._private.rscratch.override.shape="hexagon"}else{if(ag<0.76){am._private.rscratch.override.shape="square"}else{if(ag<0.91){am._private.rscratch.override.shape="pentagon"}else{am._private.rscratch.override.shape="octogon"}}}}am._private.rscratch.canvas=document.createElement("canvas")}var al;for(var ak=0;ak<ai.length;ak++){al=ai[ak];al._private.rscratch.cp2x=Math.random()*this.options.cy.container().width();
al._private.rscratch.cp2y=Math.random()*this.options.cy.container().height();al._private.rscratch.override={};if(Math.random()<0.45){al._private.rscratch.override.endShape="inhibitor"}al._private.rscratch.override.regularColor=al.source()[0]._private.rscratch.override.regularBorderColor||defaultNode.regularColor}};ab.prototype.findBezierIntersection=function(ao,aE){var aC=ao.source().position().x;var aA=ao.target().position().x;var al=ao.source().position().y;var ai=ao.target().position().y;var au;
var an=ao._private.rscratch.cp2x;var ak=ao._private.rscratch.cp2y;au=0.5+(0.5-0.5*aE/Math.sqrt(Math.pow(an-aA,2)+Math.pow(ak-ai,2)));var am=aC-2*an+aA;var at=-2*aC+2*an;var aD=aC;var aj=al-2*ak+ai;var ar=-2*al+2*ak;var aB=al;var aI=am*au*au+at*au+aD;var aH=aj*au*au+ar*au+aB;var ah=an-aC;var aq=aC;var ag=ak-al;var ap=al;var ay=aI-aA;var aG=aA;var ax=aH-ai;var aF=ai;var az;if(Math.abs(ah)>0.0001){az=(ag/ah*(aG-aq)+ap-aF)/(ax-(ag/ah)*ay)}else{az=(aq-aG)/(ay)}var aw=ay*az+aG;var av=ax*az+aF;ao._private.rscratch.newCp2x=aw;
ao._private.rscratch.newCp2y=av;ao._private.rscratch.newEndPointX=aI;ao._private.rscratch.newEndPointY=aH};ab.prototype.findIntersection=function(aj,ao,ah,an,ak){var ai=ah-aj;var ag=an-ao;var am=Math.sqrt(ai*ai+ag*ag);var al=am-ak;if(al<0){al=0}return[(al/am)*ai+aj,(al/am)*ag+ao]};ab.prototype.intersectLineEllipse=function(aq,ap,am,al,aj,ah){var ai=am-aq;var ag=al-ap;ai/=aj;ag/=ah;var an=Math.sqrt(ai*ai+ag*ag);var ak=an-1;if(ak<0){return[]}var ao=ak/an;return[(am-aq)*ao+aq,(al-ap)*ao+ap]};ab.prototype.findCircleNearPoint=function(ak,aj,al,ap,ao){var ah=ap-ak;
var ag=ao-aj;var ai=Math.sqrt(ah*ah+ag*ag);var an=ah/ai;var am=ag/ai;return[ak+an*al,aj+am*al]};ab.prototype.findNewEndPoints=function(ah,ag,ao,am,ap,an,ak,ai){var aj=this.findCircleNearPoint(ah,ag,ak,ao,am);var al=this.findCircleNearPoint(ap,an,ai,ao,am);return[aj[0],aj[1],al[0],al[1]]};ab.prototype.calculateNewEndPoints=function(){var ah=B.edges();var aj,ak;var ag;for(var ai=0;ai<ah.length;ai++){aj=ah[ai].source()[0];ak=ah[ai].target()[0];if(ah[ai]._private.rscratch.isStraightEdge){continue}ag=this.findNewEndPoints(aj.position().x,aj.position().y,ah[ai]._private.rscratch.controlPointX,ah[ai]._private.rscratch.controlPointY,ak.position().x,ak.position().y);
ah[ai]._private.rscratch.updatedStartX=ag[0];ah[ai]._private.rscratch.updatedStartY=ag[1];ah[ai]._private.rscratch.updatedEndX=ag[2];ah[ai]._private.rscratch.updatedEndY=ag[3]}};M.arrow={draw:function(ag){ag.lineTo(-0.15,-0.3);ag.lineTo(0,0);ag.lineTo(0.15,-0.3)},spacing:function(ag){return 0},gap:function(ag){return ag._private.style.width.value*2}};M.triangle=M.arrow;M.none={draw:function(ag){},spacing:function(ag){return 0},gap:function(ag){return 0}};M.circle={draw:function(ag){ag.translate(0,-0.15);
ag.arc(0,0,0.15,0,Math.PI*2,false)},spacing:function(ag){return 0},gap:function(ag){return ag._private.style.width.value*2}};M.inhibitor={draw:function(ag){ag.lineTo(-0.25,0);ag.lineTo(-0.25,-0.1);ag.lineTo(0.25,-0.1);ag.lineTo(0.25,0)},spacing:function(ag){return 4},gap:function(ag){return 4}};M.square={draw:function(ag){ag.lineTo(-0.12,0);ag.lineTo(0.12,0);ag.lineTo(0.12,-0.24);ag.lineTo(-0.12,-0.24)},spacing:function(ag){return 0},gap:function(ag){return ag._private.style.width.value*2}};M.diamond={draw:function(ag){ag.lineTo(-0.14,-0.14);
ag.lineTo(0,-0.28);ag.lineTo(0.14,-0.14);ag.lineTo(0,0)},spacing:function(ag){return 0},gap:function(ag){return ag._private.style.width.value*2}};M.tee=M.inhibitor;o.arrow=function(ag){ag.lineTo(-0.15,0.3);ag.lineTo(0,0);ag.lineTo(0.15,0.3)};af.arrow=0;G.arrow=4.5;o.triangle=o.arrow;af.triangle=af.arrow;G.triangle=G.arrow;o.none=function(ag){};af.none=0;G.none=0;o.circle=function(ag){ag.translate(0,-0.15);ag.arc(0,0,0.15,0,Math.PI*2,false)};af.circle=0;G.circle=0.3;o.inhibitor=function(ag){ag.lineTo(-0.25,0);
ag.lineTo(-0.25,-0.1);ag.lineTo(0.25,-0.1);ag.lineTo(0.25,0)};af.inhibitor=4;G.inhibitor=4;o.tee=o.inhibitor;af.tee=af.inhibitor;G.tee=G.inhibitor;ab.prototype.drawArrowShape=function(ah,ag,an,al,ai){var am=Math.asin(ai/(Math.sqrt(al*al+ai*ai)));if(al<0){am=am+Math.PI/2}else{am=-(Math.PI/2+am)}var ak=this.context;ak.save();ak.translate(ag,an);ak.moveTo(0,0);ak.rotate(-am);var aj=Math.max(Math.pow(ak.lineWidth*13.37,0.9),29);ak.scale(aj,aj);ak.beginPath();M[ah].draw(ak);ak.closePath();ak.fill();ak.restore()
};ab.prototype.findEndpoints=function(aj){var al;var ag=aj.source()[0];var ap=aj.target()[0];var av=Math.max(aj.source()[0]._private.style.width.value,aj.source()[0]._private.style.height.value);var ak=Math.max(aj.target()[0]._private.style.width.value,aj.target()[0]._private.style.height.value);av=0;ak/=2;var ai=[aj.source().position().x,aj.source().position().y];var am=[aj.target().position().x,aj.target().position().y];if(aj._private.rscratch.isSelfEdge){var ar=[aj._private.rscratch.cp2cx,aj._private.rscratch.cp2cy];
al=C[ap._private.style.shape.value](ap,ap._private.style.width.value,ap._private.style.height.value,ar[0],ar[1]);var ah=this.shortenIntersection(al,ar,M[aj._private.style["target-arrow-shape"].value].spacing(aj));var aq=this.shortenIntersection(al,ar,M[aj._private.style["target-arrow-shape"].value].gap(aj));aj._private.rscratch.endX=aq[0];aj._private.rscratch.endY=aq[1];aj._private.rscratch.arrowEndX=ah[0];aj._private.rscratch.arrowEndY=ah[1];var ar=[aj._private.rscratch.cp2ax,aj._private.rscratch.cp2ay];
al=C[ag._private.style.shape.value](ag,ag._private.style.width.value,ag._private.style.height.value,ar[0],ar[1]);var an=this.shortenIntersection(al,ar,M[aj._private.style["source-arrow-shape"].value].spacing(aj));var ao=this.shortenIntersection(al,ar,M[aj._private.style["source-arrow-shape"].value].gap(aj));aj._private.rscratch.startX=ao[0];aj._private.rscratch.startY=ao[1];aj._private.rscratch.arrowStartX=an[0];aj._private.rscratch.arrowStartY=an[1]}else{if(aj._private.rscratch.isStraightEdge){al=C[ap._private.style.shape.value](ap,ap._private.style.width.value,ap._private.style.height.value,ag.position().x,ag.position().y);
if(al.length==0){aj._private.rscratch.noArrowPlacement=true}else{aj._private.rscratch.noArrowPlacement=false}var ah=this.shortenIntersection(al,[ag.position().x,ag.position().y],M[aj._private.style["target-arrow-shape"].value].spacing(aj));var aq=this.shortenIntersection(al,[ag.position().x,ag.position().y],M[aj._private.style["target-arrow-shape"].value].gap(aj));aj._private.rscratch.endX=aq[0];aj._private.rscratch.endY=aq[1];aj._private.rscratch.arrowEndX=ah[0];aj._private.rscratch.arrowEndY=ah[1];
al=C[ag._private.style.shape.value](ag,ag._private.style.width.value,ag._private.style.height.value,ap.position().x,ap.position().y);if(al.length==0){aj._private.rscratch.noArrowPlacement=true}else{aj._private.rscratch.noArrowPlacement=false}var an=this.shortenIntersection(al,[ap.position().x,ap.position().y],M[aj._private.style["source-arrow-shape"].value].spacing(aj));var ao=this.shortenIntersection(al,[ap.position().x,ap.position().y],M[aj._private.style["source-arrow-shape"].value].gap(aj));aj._private.rscratch.startX=ao[0];
aj._private.rscratch.startY=ao[1];aj._private.rscratch.arrowStartX=an[0];aj._private.rscratch.arrowStartY=an[1]}else{if(aj._private.rscratch.isBezierEdge){var ar=[aj._private.rscratch.cp2x,aj._private.rscratch.cp2y];var au=ai[0]*0.25+am[0]*0.25+ar[0]*0.5;var at=ai[1]*0.25+am[1]*0.25+ar[1]*0.5;al=C[ap._private.style.shape.value](ap,ap._private.style.width.value,ap._private.style.height.value,ar[0],ar[1]);var ah=this.shortenIntersection(al,ar,M[aj._private.style["target-arrow-shape"].value].spacing(aj));
var aq=this.shortenIntersection(al,ar,M[aj._private.style["target-arrow-shape"].value].gap(aj));aj._private.rscratch.endX=aq[0];aj._private.rscratch.endY=aq[1];aj._private.rscratch.arrowEndX=ah[0];aj._private.rscratch.arrowEndY=ah[1];al=C[ag._private.style.shape.value](ag,ag._private.style.width.value,ag._private.style.height.value,ar[0],ar[1]);var an=this.shortenIntersection(al,ar,M[aj._private.style["source-arrow-shape"].value].spacing(aj));var ao=this.shortenIntersection(al,ar,M[aj._private.style["source-arrow-shape"].value].gap(aj));
aj._private.rscratch.startX=ao[0];aj._private.rscratch.startY=ao[1];aj._private.rscratch.arrowStartX=an[0];aj._private.rscratch.arrowStartY=an[1]}else{if(aj._private.rscratch.isArcEdge){return}}}}};ab.prototype.drawArrowhead=function(aj){var ai=aj._private.rscratch.override.endShape;ai=ai?ai:defaultEdge.endShape;var ah=aj.target().position().x-aj._private.rscratch.newEndPointX;var ag=aj.target().position().y-aj._private.rscratch.newEndPointY;this.drawArrowShape(aj,aj._private.rscratch.newEndPointX,aj._private.rscratch.newEndPointY,ah,ag)
};ab.prototype.drawArrowheads=function(am){var aj,ai;var ah=am._private.rscratch.arrowStartX;var ag=am._private.rscratch.arrowStartY;aj=ah-am.source().position().x;ai=ag-am.source().position().y;this.context.fillStyle="rgba("+am._private.style["source-arrow-color"].value[0]+","+am._private.style["source-arrow-color"].value[1]+","+am._private.style["source-arrow-color"].value[2]+","+am._private.style.opacity.value+")";this.context.lineWidth=am._private.style.width.value;this.drawArrowShape(am._private.style["source-arrow-shape"].value,ah,ag,aj,ai);
var al=am._private.rscratch.arrowEndX;var ak=am._private.rscratch.arrowEndY;aj=-(am.target().position().x-al);ai=-(am.target().position().y-ak);this.context.fillStyle="rgba("+am._private.style["target-arrow-color"].value[0]+","+am._private.style["target-arrow-color"].value[1]+","+am._private.style["target-arrow-color"].value[2]+","+am._private.style.opacity.value+")";this.context.lineWidth=am._private.style.width.value;this.drawArrowShape(am._private.style["target-arrow-shape"].value,al,ak,aj,ai)
};ab.prototype.drawStraightArrowhead=function(ai){var ah=ai.target().position().x-ai._private.rscratch.newStraightEndX;var ag=ai.target().position().y-ai._private.rscratch.newStraightEndY;this.drawArrowShape(ai,ai._private.rscratch.newStraightEndX,ai._private.rscratch.newStraightEndY,ah,ag)};ab.prototype.calculateEdgeMetrics=function(aj){if(aj._private.data.source==aj._private.data.target){aj._private.rscratch.selfEdge=true;return}var ak=aj._private.data.source<aj._private.data.target?aj.source()[0]:aj.target()[0];
var ao=aj._private.data.target<aj._private.data.source?aj.source()[0]:aj.target()[0];var aq=0.5*(ak._private.position.x+ao._private.position.x);var ap=0.5*(ak._private.position.y+ao._private.position.y);if(this.nodePairEdgeData[aj._private.rscratch.nodePairId]==1){aj._private.rscratch.straightEdge=true;aj._private.rscratch.cp2x=aq;aj._private.rscratch.cp2y=ap;return}var ah=aj._private.rscratch.nodePairEdgeNum-1;var al=this.nodePairEdgeData[aj._private.rscratch.nodePairId]-1;var ar=(ah/al-0.5);if(Math.abs(ar)<0.0001){aj._private.rscratch.straightEdge=true;
aj._private.rscratch.cp2x=aq;aj._private.rscratch.cp2y=ap;return}var ai=ao._private.position.x-ak._private.position.x;var ag=ao._private.position.y-ak._private.position.y;var an=ag*ar;var am=-ai*ar;aj._private.rscratch.cp2x=aq+an;aj._private.rscratch.cp2y=ap+am};A.ellipse=function(aj,ai,ag){var ah=R.context;ah.beginPath();ah.save();ah.translate(aj._private.position.x,aj._private.position.y);ah.scale(ai/2,ag/2);ah.arc(0,0,1,0,Math.PI*2,false);ah.closePath();ah.restore();ah.fill()};C.ellipse=function(ak,aj,ai,ah,al){var ag=R.intersectLineEllipse(ah,al,ak.position().x,ak.position().y,aj/2+ak._private.style["border-width"].value/2,ai/2+ak._private.style["border-width"].value/2);
return ag};var J=function(al,ah){var ag=1/al*2*Math.PI;var aj=al%2==0?Math.PI/2+ag/2:Math.PI/2;aj+=ah;var ak=new Array(al*2);var am;for(var ai=0;ai<al;ai++){am=ai*ag+aj;ak[2*ai]=Math.cos(am);ak[2*ai+1]=Math.sin(-am)}return ak};ab.prototype.findPolygonIntersection=function(aj,ai,ah,ag,an,am,ak){if(i[am]==undefined){i[am]=J(ak,0)}var al=R.polygonIntersectLine(ag,an,i[am],aj._private.position.x,aj._private.position.y,ai/2,ah/2,aj._private.style["border-width"].value/2);return R.findNearestIntersection(al,ag,an)
};ab.prototype.expandPolygon=function(at,ai){var aj=new Array(at.length*2);var ar,aq,ap,ao;for(var am=0;am<at.length/2;am++){ar=at[am*2];aq=at[am*2+1];if(am<at.length/2-1){ap=at[(am+1)*2];ao=at[(am+1)*2+1]}else{ap=at[0];ao=at[1]}var an=(ao-aq);var al=-(ap-ar);var ak=Math.sqrt(an*an+al*al);var ah=an/ak;var ag=al/ak;aj[am*4]=ar+ah*ai;aj[am*4+1]=aq+ag*ai;aj[am*4+2]=ap+ah*ai;aj[am*4+3]=ao+ag*ai}return aj};ab.prototype.joinLines=function(ap){var ao=new Array(ap.length/2);var ar,aq,an,am;var ak,aj,ai,ah;
for(var al=0;al<ap.length/4;al++){ar=ap[al*4];aq=ap[al*4+1];an=ap[al*4+2];am=ap[al*4+3];if(al<ap.length/4-1){ak=ap[(al+1)*4];aj=ap[(al+1)*4+1];ai=ap[(al+1)*4+2];ah=ap[(al+1)*4+3]}else{ak=ap[0];aj=ap[1];ai=ap[2];ah=ap[3]}var ag=this.finiteLinesIntersect(ar,aq,an,am,ak,aj,ai,ah,true);ao[al*2]=ag[0];ao[al*2+1]=ag[1]}return ao};A.triangle=function(ai,ah,ag){B.renderer().drawPolygon(ai._private.position.x,ai._private.position.y,ah,ag,"triangle",3)};C.triangle=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"triangle",3)
};A.square=function(ai,ah,ag){B.renderer().drawPolygon(ai._private.position.x,ai._private.position.y,ah,ag,"square",4)};C.square=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"square",4)};A.rectangle=A.square;C.rectangle=C.square;A.pentagon=function(ai,ah,ag){B.renderer().drawNgon(ai._private.position.x,ai._private.position.y,ah,ag,"pentagon",5)};C.pentagon=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"pentagon",5)};A.hexagon=function(ai,ah,ag){B.renderer().drawNgon(ai._private.position.x,ai._private.position.y,ah,ag,"hexagon",6)
};C.hexagon=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"hexagon",6)};A.heptagon=function(ai,ah,ag){B.renderer().drawNgon(ai._private.position.x,ai._private.position.y,ah,ag,"heptagon",7)};C.heptagon=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"heptagon",7)};A.octagon=function(ai,ah,ag){B.renderer().drawNgon(ai._private.position.x,ai._private.position.y,ah,ag,"octagon",8)};C.octagon=function(aj,ai,ah,ag,ak){return R.findPolygonIntersection(aj,ai,ah,ag,ak,"octagon",8)
};ab.prototype.findNearestIntersection=function(am,ag,an){var ai;var al;var aj;var ah;if(am.length==0){return[]}for(var ak=0;ak<am.length/2;ak++){ai=Math.pow(ag-am[ak*2],2)+Math.pow(an-am[ak*2+1],2);if(al==undefined||al>ai){al=ai;aj=am[ak*2];ah=am[ak*2+1]}}return[aj,ah]};ab.prototype.shortenIntersection=function(al,ak,ai){var ag=[al[0]-ak[0],al[1]-ak[1]];var aj=Math.sqrt(ag[0]*ag[0]+ag[1]*ag[1]);var ah=(aj-ai)/aj;if(ah<0){return[]}else{return[ak[0]+ah*ag[0],ak[1]+ah*ag[1]]}};ab.prototype.drawPolygon=function(am,al,ag,ao,aj,ai){if(i[aj]==undefined){i[aj]=J(ai,0)
}var an=i[aj];var ah=B.renderer().context;ah.save();ah.translate(am,al);ah.beginPath();ah.scale(ag/2,ao/2);ah.moveTo(an[0],an[1]);for(var ak=1;ak<an.length/2;ak++){ah.lineTo(an[ak*2],an[ak*2+1])}ah.closePath();ah.fill();ah.restore()};ab.prototype.pointInsidePolygon=function(ao,am,ak,aj,ai,ag,aq,at,an){var ar=new Array(ak.length);for(var al=0;al<ar.length/2;al++){ar[al*2]=ak[al*2]*ag*Math.cos(at)+aj;ar[al*2+1]=ak[al*2+1]*aq*Math.sin(at)+ai}var ah=this.expandPolygon(ar,-an);var ap=this.joinLines(ah)
};ab.prototype.polygonIntersectLine=function(am,al,ag,aj,ah,ap,ao,an){var ay=[];var ak;var ai=new Array(ag.length);for(var au=0;au<ai.length/2;au++){ai[au*2]=ag[au*2]*ap+aj;ai[au*2+1]=ag[au*2+1]*ao+ah}var ax=this.expandPolygon(ai,-an);var at=this.joinLines(ax);var ar,aq,aw,av;for(var au=0;au<at.length/2;au++){ar=at[au*2];aq=at[au*2+1];if(au<at.length/2-1){aw=at[(au+1)*2];av=at[(au+1)*2+1]}else{aw=at[0];av=at[1]}ak=this.finiteLinesIntersect(am,al,aj,ah,ar,aq,aw,av);if(ak.length!=0){ay.push(ak[0],ak[1])
}}return ay};ab.prototype.finiteLinesIntersect=function(al,at,aj,aq,ah,ap,au,an,ar){var am=(au-ah)*(at-ap)-(an-ap)*(al-ah);var ao=(aj-al)*(at-ap)-(aq-at)*(al-ah);var ak=(an-ap)*(aj-al)-(au-ah)*(aq-at);if(ak!=0){var ai=am/ak;var ag=ao/ak;if(0<=ai&&ai<=1&&0<=ag&&ag<=1){return[al+ai*(aj-al),at+ai*(aq-at)]}else{if(!ar){return[]}else{return[al+ai*(aj-al),at+ai*(aq-at)]}}}else{if(am==0||ao==0){if([al,aj,au].sort()[1]==au){return[au,an]}if([al,aj,ah].sort()[1]==ah){return[ah,ap]}if([ah,au,aj].sort()[1]==aj){return[aj,aq]
}return[]}else{return[]}}};ab.prototype.drawNgon=function(am,al,ai,ag,ao){var ah=B.renderer().context;ah.save();ah.translate(am,al);ah.beginPath();var an=1/ai*2*Math.PI;var ak=ai%2==0?Math.PI/2+an/2:Math.PI/2;ah.scale(ag/2,ao/2);ah.moveTo(Math.cos(ak),-Math.sin(ak));for(var aj=ak;aj<ak+2*Math.PI;aj+=an){ah.lineTo(Math.cos(aj),-Math.sin(aj))}ah.closePath();ah.fill();ah.restore()};ab.prototype.matchCanvasSize=function(ah){var ak=ah.clientWidth;var ag=ah.clientHeight;var ai;for(var aj=0;aj<this.canvases.length+this.bufferCanvases.length;
aj++){if(aj<this.canvases.length){ai=this.canvases[aj]}else{ai=this.bufferCanvases[aj-this.canvases.length]}if(ai.width!==ak||ai.height!==ag){ai.width=ak;ai.height=ag}}};var w=false;ab.prototype.redraw=function(al){R.matchCanvasSize(R.container);if(d){}d=setTimeout(function(){d=null;if(w&&!al){R.redraw(true);w=false}},1000/80);var ai=this.context;var aj=this.canvasContexts;var ah=this.options.cy.elements().toArray();var ap=[];var an=[];if(this.canvasNeedsRedraw[2]||this.canvasNeedsRedraw[4]){this.findEdgeControlPoints(this.options.cy.edges());
ah.sort(function(at,ar){var aq=at._private.style["z-index"].value-ar._private.style["z-index"].value;if(aq==0){if(at._private.group=="nodes"&&ar._private.group=="edges"){return 1}else{if(at._private.group=="edges"&&ar._private.group=="nodes"){return -1}}}return 0})}if(this.canvasNeedsRedraw[2]){ai=this.canvasContexts[2];this.context=ai;ai.setTransform(1,0,0,1,0,0);ai.clearRect(0,0,ai.canvas.width,ai.canvas.height);ai.translate(this.cy.pan().x,this.cy.pan().y);ai.scale(this.cy.zoom(),this.cy.zoom());
var ak;for(var am=0;am<ah.length;am++){ak=ah[am];if(ak._private.rscratch.layer2){if(ak._private.group=="nodes"){this.drawNode(ak)}else{if(ak._private.group=="edges"){this.drawEdge(ak)}}}}for(var am=0;am<ah.length;am++){ak=ah[am];if(ak._private.rscratch.layer2){if(ak._private.group=="nodes"){this.drawNodeText(ak)}else{if(ak._private.group=="edges"){this.drawEdgeText(ak)}}}}this.canvasNeedsRedraw[2]=false;this.redrawReason[2]=[]}if(this.canvasNeedsRedraw[4]){ai=this.canvasContexts[4];this.context=ai;
ai.setTransform(1,0,0,1,0,0);ai.clearRect(0,0,ai.canvas.width,ai.canvas.height);ai.translate(this.cy.pan().x,this.cy.pan().y);ai.scale(this.cy.zoom(),this.cy.zoom());var ak;for(var am=0;am<ah.length;am++){ak=ah[am];if(!ak._private.rscratch.layer2){if(ak._private.group=="nodes"){this.drawNode(ak)}else{if(ak._private.group=="edges"){this.drawEdge(ak)}}}}for(var am=0;am<ah.length;am++){ak=ah[am];if(!ak._private.rscratch.layer2){if(ak._private.group=="nodes"){this.drawNodeText(ak)}else{if(ak._private.group=="edges"){this.drawEdgeText(ak)
}}}}this.canvasNeedsRedraw[4]=false;this.redrawReason[4]=[]}if(this.canvasNeedsRedraw[0]){ai=this.canvasContexts[0];ai.setTransform(1,0,0,1,0,0);ai.clearRect(0,0,ai.canvas.width,ai.canvas.height);ai.translate(this.cy.pan().x,this.cy.pan().y);ai.scale(this.cy.zoom(),this.cy.zoom());if(U[4]==1){var ao=B.style()._private.coreStyle;var ag=ao["selection-box-border-width"].value;ai.lineWidth=ag;ai.fillStyle="rgba("+ao["selection-box-color"].value[0]+","+ao["selection-box-color"].value[1]+","+ao["selection-box-color"].value[2]+","+ao["selection-box-opacity"].value+")";
ai.fillRect(U[0]+ag/2,U[1]+ag/2,U[2]-U[0]-ag/2,U[3]-U[1]-ag/2);if(ag>0){ai.strokeStyle="rgba("+ao["selection-box-border-color"].value[0]+","+ao["selection-box-border-color"].value[1]+","+ao["selection-box-border-color"].value[2]+","+ao["selection-box-opacity"].value+")";ai.strokeRect(U[0]+ag/2,U[1]+ag/2,U[2]-U[0]-ag/2,U[3]-U[1]-ag/2)}}this.canvasNeedsRedraw[0]=false;this.redrawReason[0]=[]}this.bufferCanvasContexts[1].globalCompositeOperation="copy";this.bufferCanvasContexts[1].drawImage(this.canvases[4],0,0);
this.bufferCanvasContexts[1].globalCompositeOperation="source-over";this.bufferCanvasContexts[1].drawImage(this.canvases[2],0,0);this.bufferCanvasContexts[1].drawImage(this.canvases[0],0,0);this.bufferCanvasContexts[0].globalCompositeOperation="copy";this.bufferCanvasContexts[0].drawImage(this.bufferCanvases[1],0,0)};ab.prototype.drawEdge=function(al){var ak=R.context;var aj,ah;aj=al.source()[0];ah=al.target()[0];if(al._private.style.visibility.value!="visible"||aj._private.style.visibility.value!="visible"||ah._private.style.visibility.value!="visible"){return
}ak.strokeStyle="rgba("+al._private.style["line-color"].value[0]+","+al._private.style["line-color"].value[1]+","+al._private.style["line-color"].value[2]+","+al._private.style.opacity.value+")";if(al._private.style.width.value<=0){return}ak.lineWidth=al._private.style.width.value;this.findEndpoints(al);if(al._private.rscratch.isSelfEdge){ak.beginPath();ak.moveTo(al._private.rscratch.startX,al._private.rscratch.startY);ak.quadraticCurveTo(al._private.rscratch.cp2ax,al._private.rscratch.cp2ay,al._private.rscratch.selfEdgeMidX,al._private.rscratch.selfEdgeMidY);
ak.moveTo(al._private.rscratch.selfEdgeMidX,al._private.rscratch.selfEdgeMidY);ak.quadraticCurveTo(al._private.rscratch.cp2cx,al._private.rscratch.cp2cy,al._private.rscratch.endX,al._private.rscratch.endY);ak.stroke()}else{if(al._private.rscratch.isStraightEdge){var ai=ah._private.position.x-aj._private.position.x;var ag=ah._private.position.y-aj._private.position.y;var an=al._private.rscratch.endX-al._private.rscratch.startX;var am=al._private.rscratch.endY-al._private.rscratch.startY;if(ai*an+ag*am<0){al._private.rscratch.straightEdgeTooShort=true
}else{ak.beginPath();ak.moveTo(al._private.rscratch.startX,al._private.rscratch.startY);ak.lineTo(al._private.rscratch.endX,al._private.rscratch.endY);ak.stroke();al._private.rscratch.straightEdgeTooShort=false}}else{ak.beginPath();ak.moveTo(al._private.rscratch.startX,al._private.rscratch.startY);ak.quadraticCurveTo(al._private.rscratch.cp2x,al._private.rscratch.cp2y,al._private.rscratch.endX,al._private.rscratch.endY);ak.stroke()}}if(al._private.rscratch.noArrowPlacement!==true&&al._private.rscratch.startX!==undefined){this.drawArrowheads(al)
}};ab.prototype.drawEdgeText=function(al){var ai=R.context;if(al._private.style.visibility.value!="visible"){return}ai.textAlign="center";ai.textBaseline="middle";var ah,ag;var ak,aj;if(al._private.rscratch.isSelfEdge){ak=al._private.rscratch.selfEdgeMidX;aj=al._private.rscratch.selfEdgeMidY}else{if(al._private.rscratch.isStraightEdge){ak=(al._private.rscratch.startX+al._private.rscratch.endX)/2;aj=(al._private.rscratch.startY+al._private.rscratch.endY)/2}else{if(al._private.rscratch.isBezierEdge){ak=Math.pow(1-0.5,2)*al._private.rscratch.startX+2*(1-0.5)*0.5*al._private.rscratch.cp2x+(0.5*0.5)*al._private.rscratch.endX;
aj=Math.pow(1-0.5,2)*al._private.rscratch.startY+2*(1-0.5)*0.5*al._private.rscratch.cp2y+(0.5*0.5)*al._private.rscratch.endY}}}ah=ak;ag=aj;this.drawText(al,ah,ag)};ab.prototype.drawNode=function(ai){var ah=R.context;var ag,aj;if(ai._private.style.visibility.value!="visible"){return}ah.fillStyle="rgba("+ai._private.style["background-color"].value[0]+","+ai._private.style["background-color"].value[1]+","+ai._private.style["background-color"].value[2]+","+(ai._private.style["background-opacity"].value*ai._private.style.opacity.value)+")";
ah.strokeStyle="rgba("+ai._private.style["border-color"].value[0]+","+ai._private.style["border-color"].value[1]+","+ai._private.style["border-color"].value[2]+","+(ai._private.style["border-opacity"].value*ai._private.style.opacity.value)+")";ag=ai._private.style.width.value;aj=ai._private.style.height.value;A[ai._private.style.shape.value](ai,ag,aj);ah.lineWidth=ai._private.style["border-width"].value;if(ai._private.style["border-width"].value>0){ah.stroke()}};ab.prototype.drawNodeText=function(am){var ak=R.context;
if(am._private.style.visibility.value!="visible"){return}var ai,ag;var ah=am._private.style.width.value;var an=am._private.style.height.value;var al=am._private.style["text-halign"].strValue;if(al=="left"){ak.textAlign="right";ai=am._private.position.x-ah/2}else{if(al=="right"){ak.textAlign="left";ai=am._private.position.x+ah/2}else{if(al=="center"){ak.textAlign="center";ai=am._private.position.x}else{ak.textAlign="center";ai=am._private.position.x}}}var aj=am._private.style["text-valign"].strValue;
if(aj=="top"){ak.textBaseline="bottom";ag=am._private.position.y-an/2}else{if(aj=="bottom"){ak.textBaseline="top";ag=am._private.position.y+an/2}else{if(aj=="middle"||aj=="center"){ak.textBaseline="middle";ag=am._private.position.y}else{ak.textBaseline="middle";ag=am._private.position.y}}}this.drawText(am,ai,ag)};ab.prototype.drawText=function(ak,am,al){var ag=R.context;var ap=ak._private.style["font-style"].strValue;var aj=ak._private.style["font-size"].strValue;var ar=ak._private.style["font-family"].strValue;
var ao=ak._private.style["font-variant"].strValue;var ah=ak._private.style["font-weight"].strValue;ag.font=ap+" "+ao+" "+ah+" "+aj+" "+ar;var aq=String(ak._private.style.content.value);var ai=ak._private.style["text-transform"].value;if(ai=="none"){}else{if(ai=="uppercase"){aq=aq.toUpperCase()}else{if(ai=="lowercase"){aq=aq.toLowerCase()}}}ag.fillStyle="rgba("+ak._private.style.color.value[0]+","+ak._private.style.color.value[1]+","+ak._private.style.color.value[2]+","+(ak._private.style["text-opacity"].value*ak._private.style.opacity.value)+")";
ag.strokeStyle="rgba("+ak._private.style["text-outline-color"].value[0]+","+ak._private.style["text-outline-color"].value[1]+","+ak._private.style["text-outline-color"].value[2]+","+(ak._private.style["text-opacity"].value*ak._private.style.opacity.value)+")";ag.fillText(aq,am,al);var an=ak._private.style["text-outline-width"].value;if(an>0){ag.lineWidth=an;ag.strokeText(aq,am,al)}};ab.prototype.zoom=function(ag){if(ag!=undefined&&ag.level!=undefined){this.scale[0]=ag.level;this.scale[1]=ag.level
}console.log("zoom call");console.log(ag)};ab.prototype.fit=function(ag){console.log("fit call");console.log(ag)};ab.prototype.pan=function(ag){console.log("pan call");console.log(ag);if(this.context!=undefined){}};ab.prototype.panBy=function(ag){this.center[0]-=ag.x;this.center[1]-=ag.y;this.redraw();console.log("panBy call");console.log(ag)};W("renderer","canvas",ab)})(cytoscape);(function(a){var b={ready:function(){},stop:function(){}};function c(d){this.options=a.util.extend(true,{},b,d)}c.prototype.run=function(){var d=this.options;
...
this.system=m;if(k.liveUpdate&&k.fit){j.reset()}var t=250;var g;var o=false;var f=+new Date;var r={init:function(x){},redraw:function(){var y=m.energy();if(k.stableEnergy!=null&&y!=null&&y.n>0&&k.stableEnergy(y)){m.stop();return}clearTimeout(g);g=setTimeout(w,t);var z=j.collection();m.eachNode(function(F,A){var E=F.name;var C=F.data;var B=C.element;if(B==null){return}var D=B.position();if(!B.locked()&&!B.grabbed()){D.x=A.x;D.y=A.y;z=z.add(B)}});var x=(+new Date-f)>=16;if(k.liveUpdate&&z.size()>0&&x){z.rtrigger("position");
