...
})}}}function f(){var h=g._private.options.layout.ready;var j=g._private.options.layout.stop;g.layout(b.extend({},g._private.options.layout,{ready:function(){g.notifications(true);g.notify({type:"load",collection:g.elements(),style:g._private.style});if(a.is.fn(h)){h.apply(g,[g])}if(a.is.fn(e)){e.apply(g,[g])}g.trigger("load");g.trigger("layoutready")},stop:function(){if(a.is.fn(j)){j.apply(g,[g])}if(a.is.fn(c)){c.apply(g,[g])}g.trigger("layoutdone")}}))}if(window.chrome){setTimeout(function(){f()
},30)}else{f()}return this}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({startAnimationLoop:function(){var h=this;var n=this._private;var g=10;var o=false;var k=true;n.animation.elements=new a.CyCollection(h);var l=window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame;if(l==null||!k){l=function(p){window.setTimeout(function(){p(+new Date)},g)}}var d=h.container()[0];function m(){function p(){l(function(q){e(q);
m()},d)}if(o){setTimeout(function(){p()},g)}else{p()}}m();function e(p){n.animation.elements.each(function(t,w){try{w=w.element();var y=w._private.animation.current;var r=w._private.animation.queue;if(y.length==0){var v=r;var u=v.length>0?v.shift():null;if(u!=null){u.callTime=+new Date;y.push(u)}}b.each(y,function(z,q){f(w,q,p)});var s=[];for(var t=0;t<y.length;t++){if(y[t].done){s.push(y[t].params.complete);y.splice(t,1);t--}}b.each(s,function(q,z){if(a.is.fn(complete)){complete.apply(w)}})}catch(x){}});
if(n.animation.elements.size()>0){h.notify({type:"draw",collection:n.animation.elements})}n.animation.elements=n.animation.elements.filter(function(){var r=this;var q=r._private.animation.queue;var s=r._private.animation.current;return s.length>0||q.length>0})}function f(p,u,q){var r=u.properties;var v=u.params;var s=u.callTime;var t;if(v.duration==0){t=1}else{t=Math.min(1,(q-s)/v.duration)}function w(y){if(y.end!=null){var z=y.start;var x=y.end;b.each(x,function(A,B){if(c(z[A],x[A])){p._private[y.field][A]=j(z[A],x[A],t)
...
function d(g){var f={target:"cy",one:false};g=e.extend({},f,g);return function(h,l,k){var m=this;var j=m._private.listeners;if(k===undefined){k=l;l=undefined}h=h.split(/\s+/);e.each(h,function(n,o){if(a.is.emptyString(o)){return}if(j[g.target]==null){j[g.target]={}}if(j[g.target][o]==null){j[g.target][o]=[]}j[g.target][o].push({callback:k,data:l,one:g.one})});return m}}function c(g){var f={target:"cy"};g=e.extend({},f,g);return function(h,k){var l=this;var j=l._private.listeners;if(j[g.target]==null){return
}h=h.split(/\s+/);e.each(h,function(m,n){if(a.is.emptyString(n)){return}if(k===undefined){delete j[g.target][n];return}else{for(var m=0;m<j[g.target][n].length;m++){var o=j[g.target][n][m];if(o.callback==k){j[g.target][n].splice(m,1);m--}}}});return l}}function b(g){var f={target:"cy"};g=e.extend({},f,g);return function(k,l){var n=this;var j=n._private.listeners;var h;var m=g.target;if(a.is.plainObject(k)){h=[k.type]}else{h=k.split(/\s+/)}e.each(h,function(q,r){if(j[m]==null||j[m][r]==null){return
}for(var p=0;p<j[m][r].length;p++){var u=j[m][r][p];var s;if(a.is.plainObject(k)){s=k;k=s.type}else{s=e.Event(k)}s.data=u.data;s.cy=s.cytoscapeweb=n;var o=[s];if(l!=null){e.each(l,function(v,t){o.push(t)})}if(u.one){j[m][r].splice(p,1);p--}u.callback.apply(n,o)}})}}})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({exportTo:function(f){var e=f.name;var d=a.extension("exporter",e);if(d==null){a.console.error("No exporter with name `%s` found; did you remember to register it?",e)}else{var c=new d({cy:cy,renderer:this.renderer()});
return c.run()}}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({layout:function(c){var d=this;if(c==null){c=this._private.options.layout}this.initLayout(c);d.trigger("layoutstart");this._private.layout.run(b.extend({},c,{renderer:d._private.renderer,cy:d}));return this},initLayout:function(e){if(e==null){a.console.error("Layout options must be specified to run a layout");return}if(e.name==null){a.console.error("A `name` must be specified to run a layout");return}var d=e.name;var c=a.extension("layout",d);
if(c==null){a.console.error("Can not apply layout: No such layout `%s` found; did you include its JS file?",d);return}this._private.layout=new c();this._private.options.layout=e}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({notify:function(f){if(!this._private.notificationsEnabled){return}var e=this.renderer();if(a.is.element(f.collection)){var c=f.collection;f.collection=new a.CyCollection(cy,[c])}else{if(a.is.array(f.collection)){var d=f.collection;f.collection=new a.CyCollection(cy,d)
}}if(this.getContinuousMapperUpdates().length!=0){f.updateMappers=true;this.clearContinuousMapperUpdates()}e.notify(f)},notifications:function(c){var d=this._private;if(c===undefined){return d.notificationsEnabled}else{d.notificationsEnabled=c?true:false}},noNotifications:function(c){this.notifications(false);c();this.notifications(true)}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({renderer:function(){return this._private.renderer},initRenderer:function(d){var e=this;var c=a.extension("renderer",d.name);
if(c==null){a.console.error("Can not initialise: No such renderer `$s` found; did you include its JS file?",d.name);return}this._private.renderer=new c(b.extend({},d,{cy:e,style:e._private.style,styleCalculator:{calculate:function(l,u){if(a.is.plainObject(u)){var p;if(u.customMapper!=null){p=u.customMapper.apply(l,[l.data()])}else{if(u.passthroughMapper!=null){var r=u.passthroughMapper;p=l._private.data[r]}else{if(u.discreteMapper!=null){var r=u.discreteMapper.attr;var m=u.discreteMapper.entries;
var s=l.data(r);b.each(m,function(x,y){var w=y.attrVal;var z=y.mappedVal;if(w==s){p=z}})}else{if(u.continuousMapper!=null){var f=u.continuousMapper;if(f.attr.name==null||typeof f.attr.name!=typeof""){a.console.error("For style.%s.%s, `attr.name` must be defined as a string since it's a continuous mapper",l.group(),styleName);return}var g=e._private.continuousMapperBounds[l._private.group][f.attr.name];g={min:g==null?0:g.min,max:g==null?0:g.max};if(f.attr.min!=null){g.min=f.attr.min}if(f.attr.max!=null){g.max=f.attr.max
}if(g!=null){var k=l.data(f.attr.name);var o=(k-g.min)/(g.max-g.min);if(g.max==g.min){o=1}if(o>1){o=1}else{if(o<0||k==null||isNaN(o)){o=0}}if(k==null&&u.defaultValue!=null){p=u.defaultValue}else{if(a.is.number(f.mapped.min)&&a.is.number(f.mapped.max)){p=o*(f.mapped.max-f.mapped.min)+f.mapped.min}else{if((f.mapped.min)&&(f.mapped.max)){var n=b.Color(f.mapped.min).fix().toRGB();var v=b.Color(f.mapped.max).fix().toRGB();var h=Math.round(n.red()*(1-o)+v.red()*o);var j=Math.round(n.green()*(1-o)+v.green()*o);
var t=Math.round(n.blue()*(1-o)+v.blue()*o);p=b.Color([h,j,t],"RGB").toHEX().toString()}else{a.console.error("Unsupported value used in mapper for `style.%s.%s` with min mapped value `%o` and max `%o` (neither number nor colour)",l.group(),f.styleName,f.mapped.min,f.mapped.max);return}}}}else{a.console.error("Attribute values for `%s.%s` must be numeric for continuous mapper `style.%s.%s` (offending %s: `%s`)",l.group(),f.attr.name,l.group(),styleName,l.group(),l.data("id"));return}}}}}var q=u.defaultValue;
if(p==null){p=q}}else{p=u}return p}}}))}})})(jQuery,jQuery.cytoscapeweb);(function($,$$){$$.fn.core({scratch:function(name,value){if(value===undefined){return eval("this._private.scratch."+name)}else{eval("this._private.scratch."+name+" = "+value+";");return this}},removeScratch:function(name){if(name===undefined){structs.scratch={}}else{eval("delete this._private.scratch."+name+";")}return this}})})(jQuery,jQuery.cytoscapeweb);(function(c,a){a.fn.core({collection:function(){return new a.CyCollection(this)
},getElementById:function(d){return this._private.nodes[d]||this._private.edges[d]||new a.CyCollection(this)},nodes:b({addLiveFunction:true,group:"nodes"}),edges:b({addLiveFunction:true,group:"edges"}),elements:function(){return this.$.apply(this,arguments)},$:b({addLiveFunction:true}),filter:function(d){if(a.is.string(d)){return this.$(d)}else{if(a.is.fn(d)){return this.$(d).filter(d)}}}});function b(f){var e={group:undefined,addLiveFunction:false};f=c.extend({},e,f);var d=[];if(f.group==null){d=["nodes","edges"]
}else{d=[f.group]}return function(g){var k=this;var h=[];if(g==null){g=""}c.each(d,function(l,m){c.each(k._private[m],function(o,n){h.push(n)})});var j=new a.CyCollection(k,h);var g;if(f.group!=null){g=new a.CySelector(k,f.group,g)}else{g=new a.CySelector(k,g)}return g.filter(j,f.addLiveFunction)}}})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({style:function(d){var c;if(d===undefined){c=a.util.copy(this._private.style)}else{this._private.style=a.util.copy(d);c=this;this.notify({type:"style",style:this._private.style})
}return c},getContinuousMapperUpdates:function(){var d=this;var c=d._private;return c.continuousMapperUpdates},clearContinuousMapperUpdates:function(){var d=this;var c=d._private;c.continuousMapperUpdates=[]},addContinuousMapperBounds:function(k,d,e){var f=this;var m=f._private;var n=k._private.group;if(a.is.number(e)){if(m.continuousMapperBounds[n][d]==null){m.continuousMapperBounds[n][d]={min:e,max:e,vals:[]}}var c=m.continuousMapperBounds[n][d];var l=c.vals;var h=false;var o=null,g=null;if(l.length>0){o=l[0];
g=l[l.length-1]}for(var j=0;j<l.length;j++){if(e<=l[j]){l.splice(j,0,e);h=true;break}}if(!h){l.push(e)}c.min=l[0];c.max=l[l.length-1];if(o!=c.min||g!=c.max){m.continuousMapperUpdates.push({group:k.group(),element:k})}}},updateContinuousMapperBounds:function(h,d,k,e){var f=this;var l=f._private;var m=h._private.group;var c=l.continuousMapperBounds[m][d];if(c==null){this.addContinuousMapperBounds(h,d,e);return}var j=c.vals;var n=null,g=null;if(j.length>0){n=j[0];g=j[j.length-1]}this.removeContinuousMapperBounds(h,d,k);
this.addContinuousMapperBounds(h,d,e);if(n!=c.min||g!=c.max){l.continuousMapperUpdates.push({group:h.group(),element:h})}},removeContinuousMapperBounds:function(j,d,e){var f=this;var l=f._private;var m=j._private.group;var c=l.continuousMapperBounds[m][d];if(c==null){return}var n=null,g=null;var k=c.vals;if(k.length>0){n=k[0];g=k[k.length-1]}for(var h=0;h<k.length;h++){if(e==k[h]){k.splice(h,1);break}}if(k.length>0){c.min=k[0];c.max=k[k.length-1]}else{c.min=null;c.max=null}if(n!=c.min||g!=c.max){l.continuousMapperUpdates.push({group:j.group(),element:j})
}}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({panning:function(c){if(c!==undefined){this._private.panEnabled=c?true:false}else{return this._private.panEnabled}return this},zooming:function(c){if(c!==undefined){this._private.zoomEnabled=c?true:false}else{return this._private.zoomEnabled}return this},pan:function(d){var c=this.renderer().pan(d);if(c==null){this.trigger("pan");return this}return c},panBy:function(d){var c=this.renderer().panBy(d);if(c==null){this.trigger("pan");return this
}return c},fit:function(d){var c=this.renderer().fit({elements:d,zoom:true});if(c==null){this.trigger("zoom");this.trigger("pan");return this}return c},zoom:function(d){var c=this.renderer().zoom(d);if(c!=null){return c}else{this.trigger("zoom");return this}},center:function(c){this.renderer().fit({elements:c,zoom:false});this.trigger("pan");return this},centre:function(){return this.center.apply(cy,arguments)},reset:function(){this.renderer().pan({x:0,y:0});this.renderer().zoom(1);this.trigger("zoom");
this.trigger("pan");return this}})})(jQuery,jQuery.cytoscapeweb);(function(d,c){c.fn.collection=function(f,h){for(var g in f){c.CyCollection.prototype[g]=f[g];c.CyElement.prototype[g]=function(){var j=this.collection();return j[g].apply(j,arguments)}}};var b={prefix:{nodes:"n",edges:"e"},id:{nodes:0,edges:0},generate:function(g,k){var h=g._private.group;var f=g._private.cy._private;var j=k!=null?k:this.prefix[h]+this.id[h];while(f[h][j]!=null){j=this.prefix[h]+(++this.id[h])}return j}};function e(h,g){var f=this;
...
return}else{if(o.nodes[j]==null){c.console.error("Can not create edge with id `%s` since it specifies non-existant node as its `%s` attribute with id `%s`",this._private.data.id,n,j);return}}}var h=this.cy().getElementById(this._private.data.source);var k=this.cy().getElementById(this._private.data.target);function q(u,s,t){var r=s.element()._private.data.id;var v=t.element()._private.data.id;if(u._private.edges[r]==null){u._private.edges[r]={}}u._private.edges[r][v]={edge:t,source:h,target:k}}q(h,k,this);
q(k,h,this)}this._private.removed=false;o[this._private.group][this._private.data.id]=this;var p=this;d.each(this._private.data,function(r,s){p.cy().addContinuousMapperBounds(p,r,s)});g=g.add(this)});if(g.size()>0){if(f){g.rtrigger("add")}else{g.trigger("add")}}return this}});c.fn.collection({removed:function(){return this.element()._private.removed}});c.fn.collection({remove:function(h){var k=[];var g={};var f={};if(h===undefined){h=true}this.each(function(){if(this.isNode()){var l=this.element();
f[l.id()]=this;d.each(l._private.edges,function(m,n){d.each(n,function(p,o){g[p]=o.edge})})}if(this.isEdge()){g[this.id()]=this}});d.each([g,f],function(l,m){d.each(m,function(s,n){var o=n.element();var p=o._private.group;o._private.removed=true;delete o.cy()._private[o.group()][o.id()];d.each(o._private.data,function(t,u){o.cy().removeContinuousMapperBounds(o,t,u)});k.push(o);if(o.isEdge()){var q=o.source().element();var r=o.target().element();delete q._private.edges[r.id()][o.id()];delete r._private.edges[q.id()][o.id()]
}})});var j=new c.CyCollection(this.cy(),k);if(j.size()>0){if(h){this.cy().notify({type:"remove",collection:j})}j.trigger("remove")}return this}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({animated:function(){return this.element()._private.animation.current.length>0}});a.fn.collection({clearQueue:function(){return this.each(function(){this.element()._private.animation.queue=[]})}});a.fn.collection({delay:function(c){return this.animate({delay:c},{duration:c})}});a.fn.collection({animate:function(d,e){var c=+new Date;
...
var c=this;var d=[];b.each(e,function(h,g){if(g==null||g==""){return}c.each(function(){var j=this._private.classes[g]!==undefined;delete this._private.classes[g];if(j){d.push(this)}})});if(d.length>0){var f=new a.CyCollection(c.cy(),d);f.rtrigger("class")}return c}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({allAre:function(c){return this.filter(c).size()==this.size()}});a.fn.collection({is:function(c){return new a.CySelector(this.cy(),c).filter(this).size()>0}});a.fn.collection({same:function(c){return this.element()===c.element()
...
var f=a.util.copy({data:g.data,position:g.position,group:g.group,bypass:g.bypass,removed:g.removed,selected:g.selected,locked:g.locked,grabbed:g.grabbed,grabbable:g.grabbable,classes:"",scratch:g.scratch});var e=[];d.each(g.classes,function(h,j){e.push(h)});d.each(e,function(j,h){f.classes+=h+(j<e.length-1?" ":"")});return f}});function b(e){var f={attr:"foo",allowBinding:false,bindingEvent:"foo",settingTriggersEvent:false,settingEvent:"foo",validKey:{forGet:function(h){return true},forSet:function(h){return true
...
r.data=s.data;r.cy=r.cytoscapeweb=cy;var q=[r];if(g!=null){d.each(g,function(u,t){q.push(t)})}s.callback.apply(k,q)}}if(n!=null){d.each(n,function(q,r){m(r)});for(var l=0;l<n.length;l++){function h(){n.splice(l,1);l--}if(n[l].one){h()}else{if(n[l].once){var p=n[l];p.collection.each(function(q,r){if(!r.same(k)){r.unbind(o,p.callback)}});h()}}}}if(j.live[o]!=null){d.each(j.live[o],function(s,t){var q=new c.CySelector(k.cy(),s);var r=q.filter(k.collection());if(r.size()>0){d.each(t,function(u,v){m(v)
...
}else{return this.rtrigger(g.event,j)}}}}function a(g){var f={listener:function(){},after:function(){}};g=d.extend({},f,g);return function(j,k,l){var h=this;if(l===undefined){l=k;k=undefined}d.each(j.split(/\s+/),function(m,n){if(c.is.emptyString(n)){return}h.each(function(){if(this._private.listeners[n]==null){this._private.listeners[n]=[]}var o=g.listener.apply(this,[h,this]);o.callback=l;o.data=k;this._private.listeners[n].push(o)});g.after.apply(h,[h,l,k])});return this}}})(jQuery,jQuery.cytoscapeweb);
...
})})}return this}}})(jQuery,jQuery.cytoscapeweb);(function(d,b){function c(e){return function(){var f=arguments;if(f.length==2){this.bind(e.event,f[0],f[1])}else{if(f.length==1){this.bind(e.event,f[0])}else{if(f.length==0){this.each(function(){this.element()._private[e.field]=e.value});this.rtrigger(e.event)}}}return this}}function a(f){function e(h,j){var g={};g[h]=j;return g}b.fn.collection(e(f.field,function(){return this.element()._private[f.field]}));b.fn.collection(e(f.on,c({event:f.on,field:f.field,value:true})));
...
}var f=[];e=e.collection();this.each(function(j,h){var g=e._private.ids[h.id()];if(!g){f.push(h.element())}});return new b.CyCollection(this.cy(),f)}}});b.fn.collection({intersect:function(l){var n=this;if(b.is.string(l)){var h=l;return this.filter(h)}if(b.is.element(l)){l=l.collection()}var e=[];var k=this;var j=l;var m=this.size()<l.size();var g=m?k._private.ids:j._private.ids;var f=m?j._private.ids:k._private.ids;d.each(g,function(o){if(f[o]){e.push(n.cy().getElementById(o))}});return new b.CyCollection(this.cy(),e)
}});b.fn.collection({add:function(g){var f=this;if(g==null){return this}if(b.is.string(g)){var e=g;g=this.cy().elements(e)}g=g.collection();var j=[];var h={};function k(m){if(m==null){return}if(h[m.id()]==null){j.push(m);h[m.id()]=true}}this.each(function(n,m){k(m)});var l=g.collection();l.each(function(n,m){k(m)});return new b.CyCollection(this.cy(),j)}});b.fn.collection({neighborhood:function(e){var f=[];this.nodes().each(function(g,h){h.connectedEdges().each(function(k,m){var l=m.connectedNodes().not(h).element();
f.push(l);f.push(m.element())})});return this.connectedNodes().add(new b.CyCollection(this.cy(),f)).filter(e)}});b.fn.collection({neighbourhood:function(e){return this.neighborhood(e)}});b.fn.collection({closedNeighborhood:function(e){return new b.CySelector(this.cy(),e).filter(this.neighborhood().add(this))}});b.fn.collection({closedNeighbourhood:function(e){return this.closedNeighborhood(e)}});b.fn.collection({openNeighborhood:function(e){return this.neighborhood(e)}});b.fn.collection({openNeighbourhood:function(e){return this.openNeighborhood(e)
...
}});b.fn.collection({edgesWith:a()});b.fn.collection({edgesTo:a({include:function(g,f,e){return e.target.same(f)}})});b.fn.collection({edgesFrom:a({include:function(g,f,e){return e.source.same(g)}})});function a(f){var e={include:function(j,h,g){return true}};f=d.extend(true,{},e,f);return function(h){var g=[];this.nodes().each(function(j,k){h.nodes().each(function(l,m){d.each(k.element()._private.edges[m.id()],function(o,n){if(f.include(k,m,n)){g.push(n.edge)}})})});return new b.CyCollection(this.cy(),g)
}}b.fn.collection({connectedEdges:function(e){var f=[];this.nodes().each(function(g,h){d.each(h.element()._private.edges,function(j,k){d.each(k,function(m,l){f.push(l.edge)})})});return new b.CyCollection(this.cy(),f).filter(e)}});b.fn.collection({connectedNodes:function(e){var f=[];this.edges().each(function(g,h){f.push(h.source().element());f.push(h.target().element())});return new b.CyCollection(this.cy(),f).filter(e)}});b.fn.collection({parallelEdges:c()});b.fn.collection({codirectedEdges:c({include:function(f,g,e){return e.source
}})});function c(f){var e={include:function(h,j,g){return true}};f=d.extend(true,{},e,f);return function(g){var h=[];this.edges().each(function(j,k){var l=k.source().element();var m=k.target().element();d.each(l._private.edges[m.id()],function(o,n){if(f.include(l,m,n)){h.push(n.edge)}})});return new b.CyCollection(this.cy(),h).filter(g)}}})(jQuery,jQuery.cytoscapeweb);(function(b){function a(c){this.options=c;this.cy=c.cy;this.renderer=c.renderer}a.prototype.run=function(){var c={};this.cy.elements().each(function(d,e){var f=e.group();
...
},centerPoint:{x:0.5,y:0.5}});p({name:"circle",svg:function(v,x,y,u,w){return v.circle(x,0.5,0.5,0.5)},centerPoint:{x:0.5,y:0.5}});var n=function(u){j.cytoscapeweb("renderer","svg","nodeshape",u.name,u);delete u.name};n({name:"ellipse",svg:function(v,x,y,u,w){return v.ellipse(x,u.x,u.y,w.width,w.height)},update:function(v,x,y,u,w){v.change(y.renderer("svg"),{cx:u.x,cy:u.y,rx:w.width/2,ry:w.height/2})},intersectionShape:Ellipse});n({name:"rectangle",svg:function(v,x,y,u,w){return v.rect(x,u.x-w.width/2,u.y-w.height/2,w.width,w.height)
},update:function(v,x,y,u,w){v.change(y.renderer("svg"),{x:u.x-w.width/2,y:u.y-w.height/2,width:w.width,height:w.height})},intersectionShape:Rectangle});n({name:"roundrectangle",svg:function(v,x,y,u,w){return v.rect(x,u.x-w.width/2,u.y-w.height/2,w.width,w.height,w.width/4,w.height/4)},update:function(v,x,y,u,w){v.change(y.renderer("svg"),{x:u.x-w.width/2,y:u.y-w.height/2,width:w.width,height:w.height})},intersectionShape:Rectangle});n({name:"triangle",svg:function(v,x,y,u,w){return v.polygon(x,[[u.x,u.y-w.height/2],[u.x+w.width/2,u.y+w.height/2],[u.x-w.width/2,u.y+w.height/2]])
},update:function(v,x,y,u,w){v.change(y.renderer("svg"),{points:[[u.x,u.y-w.height/2],[u.x+w.width/2,u.y+w.height/2],[u.x-w.width/2,u.y+w.height/2]]})},intersectionShape:Polygon});function r(u){if(u!=null&&typeof u==typeof""&&(u=="hidden"||u=="visible")){return u}else{m.console.error("SVG renderer does not recognise %o as a valid visibility",u)}}function b(u){if(u!=null&&typeof u==typeof 1&&!isNaN(u)&&0<=u&&u<=1){return u}else{m.console.error("SVG renderer does not recognise %o as a valid percent (should be between 0 and 1)",u)
}}function q(u){if(u!=null&&typeof u==typeof""&&j.Color(u)!=""){return j.Color(u).toHEX()}else{m.console.error("SVG renderer does not recognise %o as a valid colour",u)}}function c(u){if(u!=null&&typeof u==typeof 1&&!isNaN(u)){return u}else{m.console.error("SVG renderer does not recognise %o as a valid number",u)}}function o(v){var u=j.cytoscapeweb("renderer","svg","nodeshape",v);if(u==null){m.console.error("SVG renderer does not recognise %s as a valid node shape",v)}return u}function e(v){var u=j.cytoscapeweb("renderer","svg","linestyle",v);
if(u==null){m.console.error("SVG renderer does not recognise %s as a valid line style",v)}return u}function k(u){if(u=="none"||u==null){return null}return j.cytoscapeweb("renderer","svg","edgearrowshape",u)}function s(u){if(u!=null&&typeof u==typeof""&&(u=="left"||u=="right"||u=="middle")){return u}else{m.console.error("SVG renderer does not recognise %o as a valid label horizonal alignment",u)}}function g(u){if(u!=null&&typeof u==typeof""&&(u=="top"||u=="bottom"||u=="middle")){return u}else{m.console.error("SVG renderer does not recognise %o as a valid label vertical alignment",u)
}}function h(u){if(u=="grab"){if(j.browser.webkit){return"-webkit-grab"}else{if(j.browser.mozilla){return"-moz-grab"}else{return"move"}}}else{if(u=="grabbing"){if(j.browser.webkit){return"-webkit-grabbing"}else{if(j.browser.mozilla){return"-moz-grabbing"}else{return"move"}}}else{return u}}}function f(u){m.console.debug("Creating SVG renderer with options (%o)",u);this.options=j.extend({},l,u);this.setStyle(u.style);this.cy=u.cy;m.console.debug("SVG renderer is using style (%o)",this.style)}f.prototype.init=function(x){var w=this;
this.cy=this.options.cy;var u=this.cy.container();var v=u.svg("get");this.container=u;this.svg=v;if(v!=null){u.svg("destroy")}u.css({padding:"0 !important"});u.svg({onLoad:function(y){if(w.scale==null){w.scale=1}if(w.translation==null){w.translation={x:0,y:0}}u.find("svg").css("overflow","hidden");v=y;w.svg=v;w.svg.change();w.svgBg=v.rect(0,0,"100%","100%",{fill:"white",opacity:0});w.edgesGroup=v.group();w.nodesGroup=v.group();w.svgRoot=j(w.nodesGroup).parents("svg:first")[0];w.selectedElements=w.cy.collection();
w.touchingNodes=w.cy.collection();w.defs=w.svg.defs();w.makeBackgroundInteractive();x()},settings:{height:"100%",width:"100%"}})};f.prototype.offsetFix=function(v){var u=this;if(v.offsetX==null||v.offsetY==null){v.offsetX=v.clientX-u.cy.container().offset().left;v.offsetY=v.clientY-u.cy.container().offset().top}};f.prototype.makeBackgroundInteractive=function(){var D=this;var M=D.svgRoot;var O=D.options.selectionToPanDelay;D.shiftDown=false;j(window).bind("keydown keyup",function(R){D.shiftDown=R.shiftKey
});function v(R){return R.target==M||j(R.target).parents("g:last")[0]==D.edgesGroup||j(R.target)[0]==D.svgBg}j(M).bind("mousedown",function(T){if(T.button!=0){return}if(v(T)){T.preventDefault();D.offsetFix(T);var U=null;var S={};var V=false;var X=true;var ac=T.pageX;var aa=T.pageY;var R=T.offsetX;var ae=T.offsetY;var ad=0;var ab=0;var W=setTimeout(function(){if(!D.cy.panning()){return}V=true;X=false;D.svg.change(M,{cursor:h(D.style.global.panCursor)});D.cy.container().scrollLeft(100)},O);var Y=function(ak){clearTimeout(W);
var ai=ak.pageX-ac;var ah=ak.pageY-aa;ac=ak.pageX;aa=ak.pageY;ad+=ai;ab+=ah;if(V){D.translation.x+=ai;D.translation.y+=ah;D.pan(D.translation)}if(X){if(U==null){U=D.svg.rect(R,ae,0,0,{fill:q(D.style.global.selectionFillColor),opacity:b(D.style.global.selectionOpacity),stroke:q(D.style.global.selectionBorderColor),strokeWidth:c(D.style.global.selectionBorderWidth)})}else{var aj=Math.abs(ad);var ag=Math.abs(ab);var af=ad>=0?R:R+ad;var al=ab>=0?ae:ae+ab;S={x1:af,y1:al,x2:af+aj,y2:al+ag};D.svg.change(U,{x:af,y:al,width:aj,height:ag})
}}};j(window).bind("mousemove",Y);var Z=function(af){if(af.type=="mouseup"&&af.button!=0){return}clearTimeout(W);j(window).unbind("mousemove",Y);j(window).unbind("mouseup",Z);j(window).unbind("blur",Z);j(M).unbind("mouseup",Z);if(V){D.svg.change(M,{cursor:null})}if(X){if(U!=null&&S.x1!=null&&!isNaN(S.x1)){D.selectElementsFromIntersection(U,S);D.svgRemove(U)}else{if(!D.shiftDown){D.unselectAll()}}}};j(window).bind("mouseup",Z);j(window).bind("blur",Z);j(M).bind("mouseup",Z)}}).bind("mousewheel",function(W,X,T,S){if(!D.cy.panning()||!D.cy.zooming()){return
...
};f.prototype.zoom=function(w){var x=this.cy;if(!x.zooming()){return}if(w===undefined){return this.scale}else{if(typeof w==typeof{}){var v=w;var u=v.position;if(v.renderedPosition!==undefined){u=this.modelPoint(v.renderedPosition)}if(u!==undefined){this.zoomAboutPoint(u,w.level)}else{this.transform({scale:v.level})}}else{this.transform({scale:w})}}};f.prototype.fit=function(D){var u=D.elements;var M=D.zoom;var C=this.cy;if(!C.panning()||(M!==undefined&&!C.zooming())){return}if(u==null||u.size()==0){u=this.cy.elements()
}if(u.is(":removed")){m.console.debug("SVG renderer does not take into account removed elements when fitting");u=u.filter(":inside")}m.console.debug("Fit SVG renderer to view bounds");var A=this.nodesGroup.getBBox();var G=this.edgesGroup.getBBox();var y,J,v,I;function E(P){var O=P.x;var z=O+P.width;var N=P.y;var w=N+P.height;if(O<y||y==null){y=O}if(z>v||v==null){v=z}if(N<J||J==null){J=N}if(w>I||I==null){I=w}}u.nodes().each(function(){var w=this.renderer("svgGroup").getBBox();E(w)});u.edges().each(function(){var z=this.source().id();
var N=this.target().id();var w=this.renderer("svg").getBBox();if(z==N){if(w.x==G.x){y=Math.min(w.x+w.width*0.4,A.x)}if(w.y==G.y){J=Math.min(w.y+w.height*0.4,A.y)}}else{w=this.renderer("svgGroup").getBBox();E(w)}});var K=v-y;var F=I-J;var x=this.cy.container().width();var L=this.cy.container().height();var B=Math.min(x/K,L/F);if(M){this.transform({translation:{x:-y*B-(K*B-x)/2,y:-J*B-(F*B-L)/2},scale:B})}else{var H=this.scale;this.transform({translation:{x:-y*H+x/2-(v-y)/2*H,y:-J*H+L/2-(I-J)/2*H}})
}};f.prototype.panBy=function(u){if(!this.cy.panning()){return}m.console.debug("Relatively pan SVG renderer with position (%o)",u);this.transform({translation:{x:this.translation.x+c(u.x),y:this.translation.y+c(u.y)}})};f.prototype.pan=function(u){if(!this.cy.panning()){return}m.console.debug("Pan SVG renderer with position (%o)",u);if(u===undefined){return{x:this.translation.x,y:this.translation.y}}if(u==null||typeof u!=typeof{}){m.console.error("You can not pan without specifying a proper position object; `%o` is invalid",u);
return}this.transform({translation:{x:c(u.x),y:c(u.y)}})};f.prototype.capTransformation=function(z){var u=z.translation;var y=z.scale;var C=this;var v=C.options.maxZoom;var x=C.options.minZoom;var B=C.options.minPan;var w=C.options.maxPan;var A=true;var D=true;if(u!=null){if(u.x<B){u.x=B;D=false}else{if(u.x>w){u.x=w;D=false}}if(u.y<B){u.y=B;D=false}else{if(u.y>w){u.y=w;D=false}}}else{u=C.translation}if(y!=null){if(y>v){y=v;A=false}else{if(y<x){y=x;A=false}}}else{y=C.scale}return{scale:y,translation:u,valid:A&&D,validScale:A,validTranslation:D}
};f.prototype.transform=function(x){var v=this;var u=v.capTransformation(x);if(u.valid){v.translation=u.translation;v.scale=u.scale}else{if(x.capScale){m.console.debug("Capping zoom level %o to %o",v.scale,u.scale);v.scale=u.scale}if(x.capTranslation){m.console.debug("Capping translation %o to %o",v.translation,u.translation);v.translation=u.translation}}function w(y){if(v.svg==null||y==null){return}v.svg.change(y,{transform:"translate("+v.translation.x+","+v.translation.y+") scale("+v.scale+")"})
}w(v.nodesGroup);w(v.edgesGroup)};f.prototype.calculateStyleField=function(w,A){var u=this;var y=u.options.styleCalculator;var v=u.style.selectors;var x=undefined;var z=w.bypass(false)[A];if(z!==undefined){x=z}else{j.each(v,function(B,C){var D=C[A];if(D!=null&&w.is(B)){x=D}})}return y.calculate(w,x)};f.prototype.calculateStyle=function(x){var u=this;var B=u.options.styleCalculator;var w=u.style.selectors;var y={};j.each(w,function(C,D){if(x.is(C)){y=j.extend(y,D)}});y=j.extend(y,x.bypass(false));
j.each(y,function(C,D){y[C]=B.calculate(x,D)});x._private.style=y;if(x.isEdge()){var z=x.source();var A=x.target();function v(){if(z.style("visibility")=="visible"&&A.style("visibility")=="visible"){return r(y.visibility)}else{return"hidden"}}y.visibility=v()}return y};f.prototype.svgRemove=function(u){var v=j(u);var x=j(this.svgRoot);function w(z){var A=j(z);var y=false;if(A.parent().size()==0){return false}A.parents().each(function(){if(this==x[0]){y=true}});return y}if(u==null||!w(u)){return}this.svg.remove(u)
};f.prototype.updateNodePositionFromShape=function(v){var x=v.style(false);var w=v.renderer("svgGroup");var u=v.position(false);o(x.shape).update(this.svg,w,v,u,x)};f.prototype.makeSvgEdgeInteractive=function(y){var v=y.renderer("svg");var z=y.renderer("svgTargetArrow");var x=y.renderer("svgSourceArrow");var w=j(v).parents("svg:first")[0];var u=this;j(v).add(z).add(x).bind("mouseup mousedown click touchstart touchmove touchend mouseover mousemove mouseout",function(A){if(u.edgeEventIsValid(y,A)){y.trigger(A)
}}).bind("click touchend",function(A){u.selectElement(y)})};f.prototype.makeSvgNodeLabelInteractive=function(u){};f.prototype.makeSvgNodeInteractive=function(x){var v=x.renderer("svg");var w=j(v).parents("svg:first")[0];var u=this;var y=null;j(v).bind("mousedown touchstart",function(A){y=false;x.trigger(A);if(x.grabbed()||x.locked()||!x.grabbable()){A.preventDefault();return}if(A.type=="touchstart"&&A.originalEvent.touches.length>1){return}x._private.grabbed=true;x.trigger(j.extend({},A,{type:"grab"}));
u.touchingNodes=u.touchingNodes.add(x);var I,G;if(A.type=="touchstart"){var D=A.originalEvent.touches;var C=D[D.length-1];I=C.pageX;G=C.pageY}else{I=A.pageX;G=A.pageY}var H=true;var E=function(M){y=true;var P,O;if(M.type=="touchmove"){var N=A.originalEvent.touches;var Q=N[N.length-1];P=Q.pageX;O=Q.pageY}else{P=M.pageX;O=M.pageY}var K=(P-I)/u.zoom();var J=(O-G)/u.zoom();I=P;G=O;var L;if(x.selected()){L=u.selectedElements.add(x)}else{L=x.collection()}L.each(function(R,S){S.element()._private.position.x+=K;
S.element()._private.position.y+=J});u.updatePosition(L);if(H){if(M.type!="touchmove"){u.moveToFront(x)}H=false}else{x.trigger(j.extend({},M,{type:"position"}));x.trigger(j.extend({},M,{type:"drag"}))}};j(window).bind("mousemove touchmove",E);var B=false;var z=0;var F=function(J){if(J.type=="touchend"&&J.originalEvent.touches.length!=0){return}if(!B){B=true}else{return}j(window).unbind("mousemove touchmove",E);j(window).unbind("mouseup touchend blur",F);j(v).unbind("mouseup touchend",F);x._private.grabbed=false;
u.touchingNodes=u.touchingNodes.not(x);x.trigger(j.extend({},J,{type:"free"}))};j(window).bind("mouseup touchend blur",F);j(v).bind("mouseup touchend",F);A.preventDefault()}).bind("mouseup touchend",function(z){x.trigger(j.extend({},z));if(y==false){y=null;x.trigger(j.extend({},z,{type:"click"}));u.selectElement(x)}}).bind("mouseover mouseout mousemove",function(z){if(u.nodeEventIsValid(x,z)){x.trigger(j.extend({},z))}})};f.prototype.edgeEventIsValid=function(w,x){var u=j(x.relatedTarget);var v=this;
switch(x.type){case"mouseover":case"mouseout":return u.parent().parent().size()>0;default:return true}};f.prototype.nodeEventIsValid=function(w,x){var u=j(x.relatedTarget);var v=this;switch(x.type){case"mouseover":case"mouseout":return u.parent().parent().size()>0;default:return true}};f.prototype.modelPoint=function(w){var v=this;var u={};if(w.x!==undefined){u.x=(w.x-v.pan().x)/v.zoom()}if(w.y!==undefined){u.y=(w.y-v.pan().y)/v.zoom()}return u};f.prototype.renderedPoint=function(v){var u=this;var w={};
if(v.x!==undefined){w.x=v.x*u.zoom()+u.pan().x}if(v.y!==undefined){w.y=v.y*u.zoom()+u.pan().y}return w};f.prototype.renderedPosition=function(v){var u=this;return u.renderedPoint(v.position(false))};f.prototype.hideElements=function(u){u.each(function(w,v){v.bypass(false).visibility="hidden"});this.updateBypass(u)};f.prototype.showElements=function(w){var v=this;var u=this.cy.collection();w.each(function(y,x){x.bypass(false).visibility="visible"});this.updateBypass(w)};f.prototype.elementIsVisible=function(u){return u.style(false).visibility!="hidden"
};f.prototype.renderedDimensions=function(v){var u=this;if(v.isNode()){return{height:v.style("height")*u.zoom(),width:v.style("width")*u.zoom()}}else{return{width:v.style("width")*u.zoom()}}};f.prototype.unselectElements=function(u){u=u.collection();u.unselect();this.selectedElements=this.selectedElements.not(u)};f.prototype.selectElementsFromIntersection=function(x,z){var v=this;var w=this.cy.collection();var u=this.cy.collection();function y(F){if(!v.elementIsVisible(F)){return false}var I=o(F.style("shape")).intersectionShape;
var M=v.modelPoint({x:z.x1,y:z.y1});var J=v.modelPoint({x:z.x2,y:z.y2});var D=v.svg.rect(M.x,M.y,J.x-M.x,J.y-M.y);var B=Intersection.intersectShapes(new Rectangle(D),new I(F.renderer("svg")));v.svgRemove(D);var Q=v.zoom();var O=F.renderedPosition().x;var L=F.renderedPosition().y;var P=F.renderedDimensions().width+F.style("borderWidth")*Q;var G=F.renderedDimensions().height+F.style("borderWidth")*Q;var C=z.x1;var N=z.y1;var A=z.x2;var K=z.y2;var E=C<=O&&O<=A&&N<=L&&L<=K;var H=B.points.length>0;return E||H
}this.cy.elements().each(function(B,A){if(A.isNode()){if(y(A)){w=w.add(A)}}else{if(v.elementIsVisible(A)&&y(A.source())&&y(A.target())){w=w.add(A)}}});if(!v.shiftDown){u=u.add(this.cy.elements().filter(function(A,B){return B.selected()&&!w.allSame(B)}))}u.unselect();w.select();v.selectedElements=v.selectedElements.not(u);v.selectedElements=v.selectedElements.add(w)};f.prototype.selectElement=function(w){var v=this;var u=v.cy.collection();var x=v.cy.collection();if(!v.shiftDown){u=u.add(v.cy.elements().filter(function(y,z){return z.selected()&&!w.same(z)
}))}if(v.shiftDown){if(w.selected()){u=u.add(w)}else{x=x.add(w)}}else{if(!w.selected()){x=x.add(w)}}u.unselect();x.select();v.selectedElements=v.selectedElements.not(u);v.selectedElements=v.selectedElements.add(x);v.moveToFront(x)};f.prototype.moveToFront=function(v){v=v.collection();var u=this;v.each(function(x,w){u.svgRemove(w.renderer("svgGroup"));u.makeSvgElement(w);u.updatePosition(v.closedNeighborhood().edges())})};f.prototype.unselectAll=function(){this.unselectElements(this.cy.elements())
};f.prototype.makeSvgNode=function(w){var y=w.position(false);var u=this;if(y.x==null||y.y==null){m.console.debug("SVG renderer is ignoring creating of node `%s` with position (%o, %o)",w.id(),y.x,y.y);return}var v;var x=this.calculateStyle(w);var z=this.svg.group(this.nodesGroup);w.renderer("svgGroup",z);v=o(x.shape).svg(this.svg,z,w,y,x);w.renderer("svg",v);this.makeSvgNodeLabel(w);w.renderer("svg",v);m.console.debug("SVG renderer made node `%s` with position (%i, %i)",w.id(),y.x,y.y);this.makeSvgNodeInteractive(w);
this.updateElementStyle(w,x);return v};f.prototype.makeSvgNodeLabel=function(w){var v=this;var u=w.position("x");var z=w.position("y");w.renderer().svgLabelGroup=v.svg.group(w.renderer().svgGroup);w.renderer().svgLabelOutline=v.svg.text(w.renderer().svgLabelGroup,u,z,"label init");w.renderer().svgLabel=v.svg.text(w.renderer().svgLabelGroup,u,z,"label init")};f.prototype.positionSvgNodeLabel=function(w){var v=this;var u=w.position("x");var z=w.position("y");v.svg.change(w.renderer().svgLabel,{x:u,y:z});
v.svg.change(w.renderer().svgLabelOutline,{x:u,y:z})};f.prototype.makeSvgEdgePath=function(K){var G=this;var ai=K.target();var Q=K.source();var z=ai.data("id")==Q.data("id");var ad;var M=Q.position("x");var av=Q.position("y");var L=ai.position("x");var au=ai.position("y");if(M==L&&av==au){L++;au++}var J=K.parallelEdges();var D=J.size();var aa;var C;var ac=20;var aj=20;var W,T,S;var Z=G.getDistance({x:M,y:av},{x:L,y:au});var al=200;if(!z&&F){ac=Math.min(20+4000/Z,al)}J.each(function(aw,ax){if(ax==K){aa=aw
}});function v(){var aw=C!=0;var ax=G.svg.createPath();if(ad!=null){G.svgRemove(ad)}if(z){ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).curveC(T.x,T.y,S.x,S.y,L,au))}else{if(aw){ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).curveQ(W.x,W.y,L,au))}else{ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).line(L,au))}}}if(z){var O=Q.style("height");var I=Q.style("width");ac+=Math.max(I,O);C=aa;ac+=aj*(C);var ap=ac;T={x:M,y:av-O/2-ap};S={x:M-I/2-ap,y:av};v()}else{var ae=D%2==0;if(ae){C=aa-D/2+(aa<D/2?0:1);
if(C>0){C-=0.5}else{C+=0.5}}else{C=aa-Math.floor(D/2)}var F=C!=0;if(Q.id()>ai.id()){C*=-1}if(F){W=T=S=G.getOrthogonalPoint({x:M,y:av},{x:L,y:au},ac*C)}else{W=T={x:L,y:au};S={x:M,y:av}}v()}var u=G.calculateStyleField(K,"width");var af=G.calculateStyleField(ai,"shape");var X=G.calculateStyleField(Q,"shape");var w=G.calculateStyleField(K,"targetArrowShape");var ar=G.calculateStyleField(K,"sourceArrowShape");var an=3;var A=10;while(an*u<A){an++}var aq=an;var x=aq*u;var af=o(af).intersectionShape;var X=o(X).intersectionShape;
var y=Intersection.intersectShapes(new Path(ad),new af(ai.renderer("svg")));var ag=y.points[y.points.length-1];y=Intersection.intersectShapes(new Path(ad),new X(Q.renderer("svg")));var H=y.points[0];var ab=aq*u;var E=-1*(this.getAngle(T,{x:M,y:av})-90);var V=-1*(this.getAngle(S,{x:L,y:au})-90);if(ag!=null){if(w!="none"){var N=G.getPointAlong(ag,S,x/2,ag);L=N.x;au=N.y}else{if(ag!=null){L=ag.x;au=ag.y}}}if(H!=null){if(ar!="none"){var U=G.getPointAlong(H,T,x/2,H);M=U.x;av=U.y}else{M=H.x;av=H.y}}v();
if(K.renderer("svgTargetArrow")!=null){this.svgRemove(K.renderer("svgTargetArrow"))}if(w!="none"){var am=k(w);var Y={x:L-am.centerPoint.x*ab,y:au-am.centerPoint.y*ab,};var B=am.centerPoint;var ao=am==null?null:am.svg(this.svg,K.renderer("svgGroup"),K,K.position(false),K.style(false));K.renderer("svgTargetArrow",ao);this.svg.change(ao,{transform:"translate("+Y.x+" "+Y.y+") scale("+ab+") rotate("+V+" "+B.x+" "+B.y+")"})}if(K.renderer("svgSourceArrow")!=null){this.svgRemove(K.renderer("svgSourceArrow"))
}if(ar!="none"){var R=k(ar);var P={x:M-R.centerPoint.x*ab,y:av-R.centerPoint.y*ab,};var ak=R.centerPoint;var ah=R==null?null:R.svg(this.svg,K.renderer("svgGroup"),K,K.position(false),K.style(false));K.renderer().svgSourceArrow=ah;this.svg.change(ah,{transform:"translate("+P.x+" "+P.y+") scale("+ab+") rotate("+E+" "+ak.x+" "+ak.y+")"})}var at;if(z){at={x:(T.x+S.x)/2*0.85+ai.position("x")*0.15,y:(T.y+S.y)/2*0.85+ai.position("y")*0.15}}else{if(F){at={x:(W.x+(M+L)/2)/2,y:(W.y+(av+au)/2)/2}}else{at={x:(M+L)/2,y:(av+au)/2}
}}K.renderer("svgLabelGroup",G.svg.group(K.renderer().svgGroup));K.renderer("svgLabelOutline",G.svg.text(K.renderer().svgLabelGroup,at.x,at.y,"label init"));K.renderer("svgLabel",G.svg.text(K.renderer().svgLabelGroup,at.x,at.y,"label init"));K.renderer().svg=ad;return ad};f.prototype.markerDrawFix=function(){this.forceRedraw()};f.prototype.forceRedraw=function(){this.svg.change(this.svgRoot,{opacity:0});this.svg.change(this.svgRoot,{opacity:Math.random()});this.svg.change(this.svgRoot,{opacity:1});
var u=this.svg.rect(0,0,this.cy.container().width(),this.cy.container().height());this.svgRemove(u)};f.prototype.getAngle=function(A,z){var v=function(B){return B*180/Math.PI};var y=this.getDistance(A,z);var x=z.x-A.x;var u=-1*(z.y-A.y);var w=v(Math.acos(x/y));if(u<0){return 360-w}else{return w}};f.prototype.getOrthogonalPoint=function(z,y,v){var x={x:z.x-y.x,y:z.y-y.y};var w=this.getNormalizedPoint({x:x.y,y:-x.x},1);var u={x:(z.x+y.x)/2,y:(z.y+y.y)/2};return{x:u.x+w.x*v,y:u.y+w.y*v}};f.prototype.getPointAlong=function(y,x,w,z){var u={x:x.x-y.x,y:x.y-y.y};
var v=this.getNormalizedPoint({x:u.x,y:u.y},1);if(z==null){z=x}return{x:z.x+v.x*w,y:z.y+v.y*w}};f.prototype.getNormalizedPoint=function(x,w){var v=Math.sqrt(x.x*x.x+x.y*x.y);var u=w/v;return{x:x.x*u,y:x.y*u}};f.prototype.getDistance=function(v,u){return Math.sqrt((u.x-v.x)*(u.x-v.x)+(u.y-v.y)*(u.y-v.y))};f.prototype.makeSvgEdge=function(v){var u=this;var x=v.source().element();var z=v.target().element();if(x==null||z==null){m.console.debug("SVG renderer is ignoring creating of edge `%s` with missing nodes");
return}var B=x.position(false);var y=z.position(false);if(B.x==null||B.y==null||y.x==null||y.y==null){m.console.debug("SVG renderer is ignoring creating of edge `%s` with position (%o, %o, %o, %o)",v.id(),B.x,B.y,y.x,y.y);return}var w=this.calculateStyle(v);var A=this.svg.group(this.edgesGroup);v.renderer().svgGroup=A;this.svg.change(A);this.makeSvgEdgePath(v);m.console.debug("SVG renderer made edge `%s` with position (%i, %i, %i, %i)",v.id(),B.x,B.y,y.x,y.y);this.makeSvgEdgeInteractive(v);this.updateElementStyle(v,w);
return v.renderer().svg};f.prototype.makeSvgElement=function(v){var u;if(v.group()=="nodes"){u=this.makeSvgNode(v)}else{if(v.group()=="edges"){u=this.makeSvgEdge(v)}}return u};f.prototype.getSvgElement=function(u){if(u.renderer().svg!=null){return u.renderer().svg}else{return this.makeSvgElement(u)}};f.prototype.updateSelection=function(u){this.updateElementsStyle(u)};f.prototype.updateClass=function(u){this.updateElementsStyle(u)};f.prototype.updateData=function(v,u){this.updateElementsStyle(v);
if(u){this.updateMapperBounds(v)}};f.prototype.updateMapperBounds=function(v){var u=this.cy.elements();if(v.nodes().size()>0&&v.edges().size()>0){}else{u=u.filter(function(){return this.group()==v.eq(0).group()})}u=u.not(v);this.updateElementsStyle(u)};f.prototype.updateElementsStyle=function(v){var u=this;v=v.collection();v.nodes().each(function(x,w){u.updateElementStyle(w)});v.edges().each(function(x,w){u.updateElementStyle(w)});v.nodes().neighborhood().edges().not(v.edges()).each(function(x,w){u.updatePosition(w)
...
}}};f.prototype.updateNodeStyle=function(x,w){var B=x.style(false).shape;x._private.style=w!=null?w:this.calculateStyle(x);var u=x.style(false);var C=x.style(false).shape;if(x.renderer().svg==null){m.console.error("SVG renderer can not update style for node `%s` since it has no SVG element",x.id());return}if(C!=B){this.svgRemove(x.renderer().svgGroup);this.makeSvgNode(x);return}this.svg.change(x.renderer().svg,{"pointer-events":"visible",fill:q(u.fillColor),fillOpacity:b(u.fillOpacity),stroke:q(u.borderColor),strokeWidth:c(u.borderWidth),strokeDashArray:e(u.borderStyle).array,strokeOpacity:b(u.borderOpacity),cursor:h(u.cursor),visibility:r(u.visibility)});
this.svg.change(x.renderer().svgGroup,{opacity:b(u.opacity)});var y={visibility:r(u.visibility),"pointer-events":"none",fill:q(u.labelFillColor),"font-family":u.labelFontFamily,"font-weight":u.labelFontWeight,"font-style":u.labelFontStyle,"text-decoration":u.labelFontDecoration,"font-variant":u.labelFontVariant,"font-size":u.labelFontSize};this.svg.change(x.renderer().svgLabelGroup,{opacity:b(u.labelOpacity)});this.svg.change(x.renderer().svgLabelOutline,{stroke:q(u.labelOutlineColor),strokeWidth:c(u.labelOutlineWidth)*2,fill:"none",opacity:b(u.labelOutlineOpacity)});
this.svg.change(x.renderer().svgLabelOutline,y);this.svg.change(x.renderer().svgLabel,y);var v=u.labelText==null?"":u.labelText;x.renderer().svgLabel.textContent=v;x.renderer().svgLabelOutline.textContent=v;var A=g(u.labelValign);var z=s(u.labelHalign);this.svg.change(x.renderer().svgGroup,{fillOpacity:b(u.fillOpacity)});o(u.shape).update(this.svg,this.nodesGroup,x,x.position(false),u);this.updateLabelPosition(x,A,z);m.console.debug("SVG renderer collapsed mappers and updated style for node `%s` to %o",x.id(),u)
};f.prototype.updateLabelPosition=function(x,C,z){var A=3;var I=0;var F=0;var E=0;var u=0;var D=x.renderer().svgLabel.textContent;this.positionSvgNodeLabel(x);var w;var B;var v;if(D==null||D==""){return}if(x.isNode()){E=x.style(false).height;u=x.style(false).width}if(z=="middle"){w={"text-anchor":"middle"}}else{if(z=="right"){w={"text-anchor":"start"};I=u/2+A}else{if(z=="left"){w={"text-anchor":"end"};I=-u/2-A}}}var H=parseInt(window.getComputedStyle(x.renderer().svgLabel)["fontSize"]);var G=j.browser.msie?H/3:0;
if(C=="middle"){B={style:"alignment-baseline: central; dominant-baseline: central;"};F=0+G}else{if(C=="top"){B={style:"alignment-baseline: normal; dominant-baseline: normal;"};F=-E/2-A}else{if(C=="bottom"){B={style:"alignment-baseline: normal; dominant-baseline: normal;"};F=E/2+H}}}v={transform:"translate("+I+","+F+")"};var y=j.extend({},w,B,v);this.svg.change(x.renderer().svgLabelOutline,y);this.svg.change(x.renderer().svgLabel,y)};f.prototype.updateEdgeStyle=function(A,x){var G=A.style(false).targetArrowShape;
var y=A.style(false).sourceArrowShape;A._private.style=x!=null?x:this.calculateStyle(A);var u=A.style(false);if(A.renderer().svg==null){m.console.error("SVG renderer can not update style for edge `%s` since it has no SVG element",A.id());return}var z=A.source().style();var E=A.renderer().oldSourceStyle||z;var w=A.target().style();var H=A.renderer().oldTargetStyle||w;var I=A.style(false).targetArrowShape;var C=A.style(false).sourceArrowShape;var F=z.height!=E.height||z.width!=E.width||w.height!=H.height||w.width!=H.width||z.shape!=E.shape||w.shape!=H.shape||z.borderWidth!=E.borderWidth||w.borderWidth!=H.borderWidth;
var D=A.renderer().oldStyle==null||A.renderer().oldStyle.width!=u.width;A.renderer().oldSourceStyle=z;A.renderer().oldTargetStyle=w;A.renderer().oldStyle=u;if(I!=G||C!=y||F||D){this.svgRemove(A.renderer().svgGroup);this.makeSvgEdge(A);return}this.svg.change(A.renderer().svg,{"pointer-events":"visibleStroke",stroke:q(u.lineColor),strokeWidth:c(u.width),strokeDashArray:e(u.style).array,"stroke-linecap":"butt",cursor:h(u.cursor),fill:"none",visibility:r(u.visibility)});this.svg.change(A.renderer().svgGroup,{opacity:b(u.opacity)});
this.svg.change(A.renderer().svgTargetArrow,{fill:q(u.targetArrowColor),cursor:h(u.cursor),visibility:r(u.visibility)});this.svg.change(A.renderer().svgSourceArrow,{fill:q(u.sourceArrowColor),cursor:h(u.cursor),visibility:r(u.visibility)});var B={visibility:r(u.visibility),"pointer-events":"none",fill:q(u.labelFillColor),"font-family":u.labelFontFamily,"font-weight":u.labelFontWeight,"font-style":u.labelFontStyle,"text-decoration":u.labelFontDecoration,"font-variant":u.labelFontVariant,"font-size":u.labelFontSize};
this.svg.change(A.renderer().svgLabel,B);this.svg.change(A.renderer().svgLabelOutline,j.extend({},B,{fill:"none",stroke:q(u.labelOutlineColor),strokeWidth:c(u.labelOutlineWidth)*2,opacity:b(u.labelOutlineOpacity),}));this.svg.change(A.renderer().svgLabelGroup,{opacity:b(u.labelOpacity)});var v=u.labelText==null?"":u.labelText;A.renderer().svgLabel.textContent=v;A.renderer().svgLabelOutline.textContent=v;this.updateLabelPosition(A,"middle","middle");m.console.debug("SVG renderer collapsed mappers and updated style for edge `%s` to %o",A.id(),u)
...
v.updateNodePositionFromShape(A);v.positionSvgNodeLabel(A);m.console.debug("SVG renderer is moving node `%s` to position (%o, %o)",A.id(),C.x,C.y)});v.positionEdges(x.closedNeighborhood().edges())};f.prototype.positionEdges=function(v){var u=this;v.each(function(w,x){if(x.renderer().svgGroup!=null){u.svgRemove(x.renderer().svgGroup)}u.makeSvgEdge(x);var z=x.source().position(false);var y=x.target().position(false);m.console.debug("SVG renderer is moving edge `%s` to position (%o, %o, %o, %o)",x.id(),z.x,z.y,y.x,y.y)
})};f.prototype.drawElements=function(v){var u=this;u.updateElementsStyle(v)};f.prototype.removeElements=function(y,x){m.console.debug("SVG renderer is removing elements");var u=this.cy.container();var w=u.svg("get");var z=this.options.cy;var v=this;y.each(function(B,A){if(A.renderer().svgGroup!=null){w.remove(A.renderer().svgGroup);A.removeRenderer("svg");A.removeRenderer("svgGroup");A.removeRenderer("svgSourceArrow");A.removeRenderer("svgTargetArrow")}else{m.console.debug("Element with group `%s` and ID `%s` has no associated SVG element",A.group(),A.id())
}});if(v.selectedElements!=null){v.selectedElements=v.selectedElements.not(y)}if(x){this.updateMapperBounds(y)}};f.prototype.notify=function(w){var u=this.options.cy.container();m.console.debug("Notify SVG renderer with params (%o)",w);if(w.type==null){m.console.error("The SVG renderer should be notified with a `type` field");return}var v=this;switch(w.type){case"load":v.init(function(){v.addElements(w.collection)});break;case"add":this.addElements(w.collection,w.updateMappers);break;case"remove":this.removeElements(w.collection,w.updateMappers);
break;case"position":this.updatePosition(w.collection);break;case"style":this.updateStyle(w.style);break;case"bypass":this.updateBypass(w.collection);break;case"class":this.updateClass(w.collection);break;case"data":this.updateData(w.collection,w.updateMappers);break;case"select":case"unselect":this.updateSelection(w.collection);break;case"draw":this.drawElements(w.collection);break;default:m.console.debug("The SVG renderer doesn't consider the `%s` event",w.type);break}};function a(u){this.options=u;
this.cy=u.cy;this.renderer=u.renderer;if(this.renderer.name()!="svg"){m.console.error("The SVG exporter can be used only if the SVG renderer is used")}}a.prototype.run=function(){return this.options.cy.container().svg("get").toSVG()};j.cytoscapeweb("renderer","svg",f);j.cytoscapeweb("exporter","svg",a)})(jQuery,jQuery.cytoscapeweb);(function(b,a){function c(){b.cytoscapeweb("debug","Creating null layout")}c.prototype.run=function(e){b.cytoscapeweb("debug","Running null layout with options (%o)",e);
var f=e.cy;f.nodes().positions(function(){return{x:0,y:0}});function d(g){if(g!=null&&typeof g==typeof function(){}){g()}}f.trigger("layoutready");d(e.ready);f.trigger("layoutstop");d(e.stop)};b.cytoscapeweb("layout","null",c)})(jQuery,jQuery.cytoscapeweb);(function(c,a){var d={fit:true};function b(){c.cytoscapeweb("debug","Creating random layout with options")}b.prototype.run=function(k){var n=c.extend(true,{},d,k);var j=k.cy;var e=j.nodes();var l=j.edges();var f=j.container();c.cytoscapeweb("debug","Running random layout with options (%o)",k);
var g=f.width();var m=f.height();c.cytoscapeweb("debug","Random layout found (w, h, d) = (%i, %i)",g,m);e.positions(function(p,o){if(o.locked()){return false}return{x:Math.round(Math.random()*g),y:Math.round(Math.random()*m)}});function h(o){if(o!=null&&typeof o==typeof function(){}){o()}}j.trigger("layoutready");h(k.ready);if(n.fit){j.fit()}j.trigger("layoutstop");h(k.stop)};c.cytoscapeweb("layout","random",b)})(jQuery,jQuery.cytoscapeweb);(function(b,a){var d={fit:true,rows:undefined,columns:undefined};
function c(){b.cytoscapeweb("debug","Creating grid layout")}c.prototype.run=function(z){var j=b.extend(true,{},d,z);var h=z.cy;var v=h.nodes();var g=h.edges();var r=h.container();b.cytoscapeweb("debug","Running grid layout with options (%o)",j);var u=r.width();var t=r.height();b.cytoscapeweb("debug","Running grid layout on container of size (w, h) = (%i, %i) with %i nodes",u,t,v.size());if(t==0||u==0){b.cytoscapeweb("warn","Running grid layout on container of size 0");v.positions(function(){return{x:0,y:0}
})}else{var k=v.size();var p=Math.sqrt(k*t/u);var o=Math.round(p);var q=Math.round(u/t*p);b.cytoscapeweb("debug","Grid layout decided on initial (cols, rows) = (%i, %i)",q,o);function n(B){if(B==undefined){return Math.min(o,q)}else{var A=Math.min(o,q);if(A==o){o=B;b.cytoscapeweb("debug","Grid layout set small number of rows to %i",o)}else{q=B;b.cytoscapeweb("debug","Grid layout set small number of columns to %i",q)}}}function f(B){if(B==undefined){return Math.max(o,q)}else{var A=Math.max(o,q);if(A==o){o=B;
b.cytoscapeweb("debug","Grid layout set large number of rows to %i",o)}else{q=B;b.cytoscapeweb("debug","Grid layout set large number of columns to %i",q)}}}if(j.rows!=null&&j.columns!=null){o=j.rows;q=j.columns}else{if(j.rows!=null&&j.columns==null){o=j.rows;q=Math.ceil(k/o)}else{if(j.rows==null&&j.columns!=null){q=j.columns;o=Math.ceil(k/q)}else{if(q*o>k){var y=n();var s=f();b.cytoscapeweb("debug","Grid layout is looking to make a reduction");if((y-1)*s>=k){n(y-1)}else{if((s-1)*y>=k){f(s-1)}}}else{b.cytoscapeweb("debug","Grid layout is looking to make an increase");
while(q*o<k){var y=n();var s=f();if((s+1)*y>=k){f(s+1)}else{n(y+1)}}}}}}b.cytoscapeweb("debug","Grid layout split area into cells (cols, rows) = (%i, %i)",q,o);var x=u/q;var e=t/o;var m=0;var l=0;v.positions(function(C,B){if(B.locked()){return false}var A=l*x+x/2;var D=m*e+e/2;l++;if(l>=q){l=0;m++}return{x:A,y:D}})}if(j.fit){h.reset()}function w(A){if(A!=null&&typeof A==typeof function(){}){A()}}h.trigger("layoutready");w(z.ready);h.trigger("layoutstop");w(z.stop)};b.cytoscapeweb("layout","grid",c)
})(jQuery,jQuery.cytoscapeweb);(function(c,a){var d={fit:true};function b(){c.cytoscapeweb("debug","Creating preset layout with options")}b.prototype.run=function(m){var k=c.extend(true,{},d,m);c.cytoscapeweb("debug","Running preset layout with options (%o)",k);var n=m.cy;var j=n.nodes();var h=n.edges();var g=n.container();function l(o){if(k.positions==null){return null}if(k.positions[o._private.data.id]==null){return null}return k.positions[o._private.data.id]}j.positions(function(p,q){var o=l(q);
if(q.locked()||o==null){return false}return o});function f(o){if(o!=null&&typeof o==typeof function(){}){o()}}n.trigger("layoutready");f(m.ready);if(k.fit){n.fit()}n.trigger("layoutstop");f(m.stop)};c.cytoscapeweb("layout","preset",b);function e(f){this.options=f;this.cy=f.cy;this.renderer=f.renderer}e.prototype.run=function(){var f={};this.cy.elements().each(function(g,h){f[h.data("id")]=h.position()});return f};c.cytoscapeweb("exporter","preset",e)})(jQuery,jQuery.cytoscapeweb);(jQuery.color||(function(a){a.color={isEqual:function(b,d){if(b.length!==d.length){return false
}var c=b.length;while(c--){if(b[c]!==d[c]){return false}}return true},fix:function(b,d){var c=d.length;while(c--){if(typeof b[c]==="number"){switch(d.charAt(c)){case"i":b[c]=Math.round(b[c]);break;case"o":b[c]=Math.min(255,Math.max(0,Math.round(b[c])));break;case"1":b[c]=Math.min(1,Math.max(0,b[c]));break}}}return b},self:function(b){return b},alpha:function(b){return b===undefined?1:b},palette:{},fns:[]}})(jQuery));(jQuery.color&&(function(a){a.color.RGB={fix:function(b){b=a.color.fix(b,"ooo1");
return b},toRGB:a.color.self,toHEX:function(b){return"#"+(16777216+b[0]*65536+b[1]*256+b[2]).toString(16).slice(-6)},toCSS:function(b){if(a.color.alpha(b[3])===0){return"transparent"}if(a.color.alpha(b[3])<1){return"rgba("+b.join(",")+")"}return"rgb("+Array.prototype.slice.call(b,0,3).join(",")+")"},red:function(b){return b[0]},green:function(b){return b[1]},blue:function(b){return b[2]},alpha:function(b){return a.color.alpha(b[3])}};a.color.RGB.toString=a.color.RGB.toHEX;a.color.fns.push("RGB.toRGB","RGB.toHEX","RGB.toCSS","RGB.red","RGB.green","RGB.blue","RGB.alpha")
