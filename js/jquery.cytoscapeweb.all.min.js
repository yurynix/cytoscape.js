...
/**
 * This file is part of Cytoscape Web 2.0-prerelease-snapshot-2012.05.24-13.30.02.
 * 
...
g.push(n)}});for(var j=0;j<g.length;j++){this[j]=g[j]}this.length=g.length;this._private={cy:m,ids:k};if(h){this.restore()}}c.CyCollection=a;a.prototype.cy=function(){return this._private.cy};a.prototype.element=function(){return this[0]};a.prototype.collection=function(){return this};c.fn.collection({json:function(){var h=this.element();if(h==null){return undefined}var j=h._private;var g=c.util.copy({data:j.data,position:j.position,group:j.group,bypass:j.bypass,removed:j.removed,selected:j.selected,selectable:j.selectable,locked:j.locked,grabbed:j.grabbed,grabbable:j.grabbable,classes:"",scratch:j.scratch});
var f=[];d.each(j.classes,function(k,l){f.push(k)});d.each(f,function(l,k){g.classes+=k+(l<f.length-1?" ":"")});return g}});c.fn.collection({restore:function(h){var k=[];if(h===undefined){h=true}function j(q,n){if(!q.removed()){return}var l=q.cy()._private;var p=q._private.data;var r=q.cy();if(p.id==null){p.id=b.generate(r,q)}else{if(q.cy().getElementById(p.id).size()!=0){c.console.error("Can not create element: an element in the visualisation already has ID `%s`",p.id);return}}if(c.is.fn(n)){var o=n(q);
...
return this}});a.fn.collection({visible:function(){var c=this.element();if(c!=null){return this.cy().renderer().elementIsVisible(c)}}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({addClass:function(d){d=d.split(/\s+/);var c=this;b.each(d,function(f,e){if(a.is.emptyString(e)){return}c.each(function(){this._private.classes[e]=true})});c.rtrigger("class");return c}});a.fn.collection({hasClass:function(c){var d=this.element();return d!=null&&d._private.classes[c]==true}});a.fn.collection({toggleClass:function(j,d){var f=j.split(/\s+/);
...
default:return true}}},onSet:function(g,e,f){this.cy().updateContinuousMapperBounds(this,g,e,f)}})});a.fn.collection({removeData:c({attr:"data",event:"data",triggerEvent:true,onRemove:function(e,f){this.cy().removeContinuousMapperBounds(this,e,f)},validKey:function(e){switch(e){case"id":case"source":case"target":return false;default:return true}},essentialKeys:["id","source","target","parent"]})});a.fn.collection({id:function(){var e=this.element();if(e!=null){return e._private.data.id}}});a.fn.collection({position:b({attr:"position",allowBinding:true,bindingEvent:"position",settingTriggersEvent:true,settingEvent:"position",validKey:{forSet:function(e){return this.isNode()
...
},forGet:function(e){return this.isNode()}},validValue:function(e,f){return true},override:{forSet:function(f,j){var h={};h[f]=j;var e=this.cy().renderer().modelPoint(h);var g=this.element();if(g!=null){g._private.position[f]=e[f]}},forGet:function(f){var e=this.position(false);var g=this.cy().renderer().renderedPoint(e);return g[f]},forObjectGet:function(){return this.cy().renderer().renderedPosition(this.element())}}})});a.fn.collection({renderedStyle:function(h){var g=this.element();if(g==null){return undefined
}var f=g.cy().renderer();var e=f.renderedStyle(g);if(h===undefined){return e}else{return e[h]}}});a.fn.collection({style:function(e){var f=this.element();if(f==null){return undefined}if(e===undefined){return a.util.copy(f._private.style)}else{if(e===false){return f._private.style}else{if(a.is.string(e)){return a.util.copy(f._private.style[e])}}}}});a.fn.collection({bypass:b({attr:"bypass",allowBinding:true,bindingEvent:"bypass",settingTriggersEvent:true,settingEvent:"bypass"})});a.fn.collection({removeBypass:c({attr:"bypass",event:"bypass",triggerEvent:true})});
function b(e){var f={attr:"foo",allowBinding:false,bindingEvent:"foo",settingTriggersEvent:false,settingEvent:"foo",validKey:{forGet:function(h){return true},forSet:function(h){return true}},override:{forSet:null,forGet:null,forObjectGet:null},validValue:function(h,j){return true},onSet:null,onGet:null,};var g=d.extend(true,{},f,e);return function(r,h){var s=this.element();var t=this;function p(v){if(s==null){return undefined}if(g.validKey.forGet.apply(s,[v])){var u;if(a.is.fn(g.override.forGet)){u=g.override.forGet.apply(s,[v])
}else{u=a.util.copy(s._private[g.attr][v])}if(a.is.fn(g.onGet)){g.onGet.apply(s,[v,u])}return u}else{}}function k(u,v){t.each(function(){if(g.validKey.forSet.apply(this,[u])&&g.validValue.apply(this,[u,v])){var w=this.element()._private[g.attr][u];if(a.is.fn(g.override.forSet)){g.override.forSet.apply(this,[u,v])}else{this.element()._private[g.attr][u]=a.util.copy(v)}if(a.is.fn(g.onSet)){g.onSet.apply(s,[u,w,v])}}else{}})}function o(u,v){if(v===undefined){t.bind(g.bindingEvent,u)}else{t.bind(g.bindingEvent,v,u)
}}function j(){if(g.settingTriggersEvent){t.rtrigger(g.settingEvent)}}function n(w){if(s==null){return undefined}var u;var v=s._private[g.attr];if(a.is.fn(g.override.forObjectGet)){u=g.override.forObjectGet.apply(s,[])}else{u=v}if(w||w===undefined){u=a.util.copy(u)}return u}if(r===undefined){return n()}else{if(r===false){return n(false)}else{if(h===undefined){if(a.is.string(r)){return p(r)}else{if(a.is.plainObject(r)){var m=r;d.each(m,function(u,v){k(u,v)});j()}else{if(g.allowBinding&&a.is.fn(r)){var q=r;
o(q)}else{a.console.warn("Invalid first parameter for `%s()` for collection with element `%s` : expect a key string or an object"+(g.allowBinding?" or a handler function for binding":""),g.attr,s._private.data.id)}}}}else{if(g.allowBinding&&a.is.plainObject(r)&&a.is.fn(h)){var l=r;var q=h;o(q,l)}else{if(a.is.string(r)){k(r,h);j()}else{a.console.warn("Invalid parameters for `%s()` for collection with element `%s` : expect a key string and a value"+(g.allowBinding?" or a data object and a handler function for binding":""),g.attr,s._private.data.id)
...
delete q._private[g.attr][p];if(a.is.fn(g.onRemove)){g.onRemove.apply(q,[p,r])}}})}function l(){if(g.triggerEvent){j.rtrigger(g.event)}}if(n===undefined){m();l()}else{if(a.is.string(n)){var k=n.split(/\s+/);d.each(k,function(q,p){if(a.is.emptyString(p)){return}h(p)});l()}else{a.console.warn("Invalid parameters to `%s()` for collection with element `%s` : %o",g.attr,o._private.data.id,arguments)}}return this}}})(jQuery,jQuery.cytoscapeweb);(function(d,b){function a(e){return function(){var f=this.element();
if(f==null){return undefined}if(f.isNode()&&!f.removed()){var h=0;var g=this;g.connectedEdges().each(function(j,k){h+=e(g,k)});return h}else{return undefined}}}b.fn.collection({degree:a(function(f,e){if(e.source().same(e.target())){return 2}else{return 1}})});b.fn.collection({indegree:a(function(f,e){if(e.target().same(f)){return 1}else{return 0}})});b.fn.collection({outdegree:a(function(f,e){if(e.source().same(f)){return 1}else{return 0}})});function c(f,e){return function(){var g=null;this.nodes().each(function(h,j){var k=j[f]();
if(k!=null&&(g==null||e(k,g))){g=k}});return g}}b.fn.collection({minDegree:c("degree",function(f,e){return f<e})});b.fn.collection({maxDegree:c("degree",function(f,e){return f>e})});b.fn.collection({minIndegree:c("indegree",function(f,e){return f<e})});b.fn.collection({maxIndegree:c("indegree",function(f,e){return f>e})});b.fn.collection({minOutdegree:c("outdegree",function(f,e){return f<e})});b.fn.collection({maxOutdegree:c("outdegree",function(f,e){return f>e})});b.fn.collection({totalDegree:function(){var e=0;
this.nodes().each(function(f,g){e+=g.degree()});return e}})})(jQuery,jQuery.cytoscapeweb);(function(d,c){c.fn.collection({trigger:function(f,g){this.each(function(){var k=this;var o=c.is.plainObject(f)?f.type:f;var j=this.cy()._private;var n=this._private.listeners[o];function m(s,r){if(s!=null&&c.is.fn(s.callback)){var r=c.is.plainObject(f)?f:d.Event(o);r.data=s.data;r.cy=r.cytoscapeweb=cy;var q=[r];if(g!=null){if(!c.is.array(g)){g=[g]}d.each(g,function(u,t){q.push(t)})}s.callback.apply(k,q)}}if(n!=null){d.each(n,function(q,r){m(r)
});for(var l=0;l<n.length;l++){function h(){n.splice(l,1);l--}if(n[l].one){h()}else{if(n[l].once){var p=n[l];p.collection.each(function(q,r){if(!r.same(k)){r.unbind(o,p.callback)}});h()}}}}if(j.live[o]!=null){d.each(j.live[o],function(s,t){var q=new c.CySelector(k.cy(),s);var r=q.filter(k.collection());if(r.size()>0){d.each(t,function(u,v){m(v)})}})}k.cy().trigger(f,g)});return this}});c.fn.collection({rtrigger:function(f,g){this.cy().notify({type:f,collection:this.filter(function(){return !this.removed()
})});this.trigger(f,g)}});c.fn.collection({live:function(){c.console.warn("`live()` can be called only on collections made from top-level selectors");return this}});c.fn.collection({die:function(){c.console.warn("`die()` can be called only on collections made from top-level selectors");return this}});c.fn.collection({bind:a({listener:function(){return{}}})});c.fn.collection({one:a({listener:function(){return{one:true}}})});c.fn.collection({once:a({listener:function(g,f){return{once:true,collection:g}
},after:function(g,h,f){}})});c.fn.collection({on:function(f,g,h){return this.bind(f,g,h)}});c.fn.collection({off:function(f,g){return this.unbind(f,g)}});c.fn.collection({unbind:function(g,h){var f=(g||"").split(/\s+/);this.each(function(){var j=this;if(g===undefined){j._private.listeners={};return}d.each(f,function(k,n){if(c.is.emptyString(n)){return this}var m=j._private.listeners[n];if(m!=null){for(var l=0;l<m.length;l++){var o=m[l];if(h==null||h==o.callback){m.splice(l,1);l--}}}})});return this
}});var b="mousedown mouseup click mouseover mouseout mousemove touchstart touchmove touchend grab drag free";d.each(b.split(/\s+/),function(h,g){var f={};f[g]=e({event:g});c.fn.collection(f)});function e(g){var f={event:""};g=d.extend({},f,g);return function(j,k){if(c.is.fn(k)){return this.bind(g.event,j,k)}else{if(c.is.fn(j)){var h=j;return this.bind(g.event,h)}else{return this.rtrigger(g.event,j)}}}}function a(g){var f={listener:function(){},after:function(){}};g=d.extend({},f,g);return function(j,k,l){var h=this;
if(l===undefined){l=k;k=undefined}if(l==null){return this}d.each(j.split(/\s+/),function(m,n){if(c.is.emptyString(n)){return}h.each(function(){if(this._private.listeners[n]==null){this._private.listeners[n]=[]}var o=g.listener.apply(this,[h,this]);o.callback=l;o.data=k;this._private.listeners[n].push(o)});g.after.apply(h,[h,l,k])});return this}}})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({isNode:function(){return this.group()=="nodes"}});a.fn.collection({isEdge:function(){return this.group()=="edges"
}});a.fn.collection({isLoop:function(){return this.isEdge()&&this.source().id()==this.target().id()}});a.fn.collection({group:function(){var c=this.element();if(c!=null){return c._private.group}}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({each:function(e){if(a.is.fn(e)){for(var d=0;d<this.size();d++){var f=this.eq(d).element();var c=e.apply(f,[d,f]);if(c===false){break}}}return this},toArray:function(){var d=[];for(var c=0;c<this.size();c++){d.push(this.eq(c).element())}return d
},slice:function(f,c){var e=[];if(c==null){c=this.size()}if(f<0){f=this.size()+f}for(var d=f;d>=0&&d<c&&d<this.size();d++){e.push(this.eq(d))}return new a.CyCollection(this.cy(),e)},size:function(){return this.length},eq:function(c){return this[c]},empty:function(){return this.size()==0},nonempty:function(){return !this.empty()}})})(jQuery,jQuery.cytoscapeweb);(function($,$$){$$.fn.collection({scratch:defineAccessor({attr:"scratch"})});$$.fn.collection({removeScratch:defineRemover({attr:"scratch"})});
$$.fn.collection({renscratch:defineAccessor({attr:"renscratch"})});$$.fn.collection({removeRenscratch:defineRemover({attr:"renscratch"})});function defineAccessor(params){var defaults={attr:"scratch"};params=$.extend(true,{},defaults,params);return function(name,val){var self=this;if(name===undefined){return self.element()._private[params.attr]}var fields=name.split(".");function set(){self.each(function(){var self=this;var obj=self._private[params.attr];$.each(fields,function(i,field){if(i==fields.length-1){return
...
})})}return this}}})(jQuery,jQuery.cytoscapeweb);(function(d,b){function c(e){return function(){var f=arguments;if(f.length==2){this.bind(e.event,f[0],f[1])}else{if(f.length==1){this.bind(e.event,f[0])}else{if(f.length==0){var g=new b.CyCollection(this.cy());this.each(function(){if(e.ableField==null||this.element()._private[e.ableField]){this.element()._private[e.field]=e.value;g=g.add(this)}});g.rtrigger(e.event)}}}return this}}function a(f){function e(h,j){var g={};g[h]=j;return g}b.fn.collection(e(f.field,function(){var g=this.element();
if(g!=null){return g._private[f.field]}}));b.fn.collection(e(f.on,c({event:f.on,field:f.field,ableField:f.ableField,value:true})));b.fn.collection(e(f.off,c({event:f.off,field:f.field,ableField:f.ableField,value:false})))}a({field:"locked",on:"lock",off:"unlock"});a({field:"grabbable",on:"grabify",off:"ungrabify"});a({field:"selected",ableField:"selectable",on:"select",off:"unselect"});a({field:"selectable",on:"selectify",off:"unselectify"});b.fn.collection({grabbed:function(){var e=this.element();
if(e!=null){return e._private.grabbed}}})})(jQuery,jQuery.cytoscapeweb);(function(e,b){b.fn.collection({nodes:function(f){return this.filter(function(h,g){return g.isNode()}).filter(f)}});b.fn.collection({edges:function(f){return this.filter(function(h,g){return g.isEdge()}).filter(f)}});b.fn.collection({filter:function(f){var h=this.cy();if(b.is.fn(f)){var g=[];this.each(function(k,j){j=j.element();if(f.apply(j,[k,j])){g.push(j)}});return new b.CyCollection(this.cy(),g)}else{if(b.is.string(f)||b.is.elementOrCollection(f)){return new b.CySelector(this.cy(),f).filter(this)
}else{if(f===undefined){return this}}}b.console.warn("You must pass a function or a selector to cy.filter() et al");return new b.CyCollection(this.cy())}});b.fn.collection({not:function(f){if(f==null){return this}else{if(b.is.string(f)){f=this.filter(f)}var g=[];f=f.collection();this.each(function(k,j){var h=f._private.ids[j.id()];if(!h){g.push(j.element())}});return new b.CyCollection(this.cy(),g)}}});b.fn.collection({intersect:function(m){var o=this;if(b.is.string(m)){var j=m;return this.filter(j)
}if(b.is.element(m)){m=m.collection()}var f=[];var l=this;var k=m;var n=this.size()<m.size();var h=n?l._private.ids:k._private.ids;var g=n?k._private.ids:l._private.ids;e.each(h,function(p){if(g[p]){f.push(o.cy().getElementById(p))}});return new b.CyCollection(this.cy(),f)}});b.fn.collection({add:function(h){var g=this;if(h==null){return this}if(b.is.string(h)){var f=h;h=this.cy().elements(f)}h=h.collection();var k=[];var j={};function l(n){if(n==null){return}if(j[n.id()]==null){k.push(n);j[n.id()]=true
}}this.each(function(o,n){l(n)});var m=h.collection();m.each(function(o,n){l(n)});return new b.CyCollection(this.cy(),k)}});b.fn.collection({neighborhood:function(f){var g=[];this.nodes().each(function(h,j){j.connectedEdges().each(function(k,m){var l=m.connectedNodes().not(j);if(l.size()>0){g.push(l.element())}g.push(m.element())})});return this.connectedNodes().add(new b.CyCollection(this.cy(),g)).filter(f)}});b.fn.collection({neighbourhood:function(f){return this.neighborhood(f)}});b.fn.collection({closedNeighborhood:function(f){return new b.CySelector(this.cy(),f).filter(this.neighborhood().add(this))
}});b.fn.collection({closedNeighbourhood:function(f){return this.closedNeighborhood(f)}});b.fn.collection({openNeighborhood:function(f){return this.neighborhood(f)}});b.fn.collection({openNeighbourhood:function(f){return this.openNeighborhood(f)}});b.fn.collection({source:d({attr:"source"})});b.fn.collection({target:d({attr:"target"})});function d(f){return function(g){var h=[];this.edges().each(function(){var j=this.cy().getElementById(this.data(f.attr)).element();h.push(j)});return new b.CyCollection(this.cy(),h).filter(g)
}}b.fn.collection({edgesWith:a()});b.fn.collection({edgesTo:a({include:function(h,g,f){return f.target.same(g)}})});function a(g){var f={include:function(k,j,h){return true}};g=e.extend(true,{},f,g);return function(j){var h=[];if(b.is.string(j)){j=this.cy().$(j)}this.nodes().each(function(k,l){j.nodes().each(function(n,o){var m=l.element()._private.edges[o.id()];if(m!=null){e.each(m,function(q,p){if(g.include(l,o,p)){h.push(p.edge)}})}})});return new b.CyCollection(this.cy(),h)}}b.fn.collection({connectedEdges:function(f){var g=[];
this.nodes().each(function(h,j){e.each(j.element()._private.edges,function(k,l){e.each(l,function(n,m){g.push(m.edge)})})});return new b.CyCollection(this.cy(),g).filter(f)}});b.fn.collection({connectedNodes:function(f){return this.source().add(this.target()).filter(f)}});b.fn.collection({parallelEdges:c()});b.fn.collection({codirectedEdges:c({include:function(g,h,f){return f.source.same(g)&&f.target.same(h)}})});function c(g){var f={include:function(j,k,h){return true}};g=e.extend(true,{},f,g);return function(h){var j=[];
this.edges().each(function(k,l){var m=l.source().element();var n=l.target().element();e.each(m._private.edges[n.id()],function(p,o){if(g.include(m,n,o)){j.push(o.edge)}})});return new b.CyCollection(this.cy(),j).filter(h)}}b.fn.collection({parent:function(f){var g=[];this.each(function(){var h=this.cy().getElementById(this.data("parent"));if(h.size()>0){g.push(h.element())}});return new b.CyCollection(this.cy(),g).filter(f)},parents:function(g){var h=[];var f=this.parent();while(f.nonempty()){f.each(function(){h.push(this.element())
});f=f.parent()}return new b.CyCollection(this.cy(),h).filter(g)},children:function(f){var g=[];this.each(function(){var h=this.element();e.each(h._private.children,function(k,j){g.push(j)})});return new b.CyCollection(this.cy(),g).filter(f)},siblings:function(f){return this.parent().children().not(this).filter(f)},descendants:function(f){var g=[];function h(j){j.each(function(){g.push(this.element());if(this.children().nonempty()){h(j.children())}})}h(this.children());return new b.CyCollection(this.cy(),g).filter(f)
}})})(jQuery,jQuery.cytoscapeweb);(function($,$$){function CySelector(cy,onlyThisGroup,selector){if(cy===undefined||!(cy instanceof $$.CyCore)){$$.console.error("A selector must have a reference to the core");return}if(selector===undefined&&onlyThisGroup!==undefined){selector=onlyThisGroup;onlyThisGroup=undefined}var self=this;self._private={selectorText:null,invalid:true,cy:cy};function newQuery(){return{classes:[],colonSelectors:[],data:[],group:null,ids:[],meta:[],collection:null,filter:null,parent:null,ancestor:null,subject:null,child:null,descendant:null}
}if(selector==null||($$.is.string(selector)&&selector.match(/^\s*$/))){if(onlyThisGroup==null){self.length=0}else{self[0]=newQuery();self[0].group=onlyThisGroup;self.length=1}}else{if($$.is.element(selector)){var collection=new $$.CyCollection(self.cy(),[selector]);self[0]=newQuery();self[0].collection=collection;self.length=1}else{if($$.is.collection(selector)){self[0]=newQuery();self[0].collection=selector;self.length=1}else{if($$.is.fn(selector)){self[0]=newQuery();self[0].filter=selector;self.length=1
}else{if($$.is.string(selector)){var metaChar="[\\!\\\"\\#\\$\\%\\&\\'\\(\\)\\*\\+\\,\\.\\/\\:\\;\\<\\=\\>\\?\\@\\[\\]\\^\\`\\{\\|\\}\\~]";var variable="(?:[\\w-]|(?:\\\\"+metaChar+"))+";var comparatorOp="=|\\!=|>|>=|<|<=|\\$=|\\^=|\\*=";var boolOp="\\?|\\!|\\^";var string="\"(?:\\\\\"|[^\"])+\"|'(?:\\\\'|[^'])+'";var number="\\d*\\.\\d+|\\d+|\\d*\\.\\d+[eE]\\d+";var value=string+"|"+number;var meta="degree|indegree|outdegree";var separator="\\s*,\\s*";var className=variable;var descendant="\\s+";
var child="\\s+>\\s+";var subject="\\$";var id=variable;function cleanMetaChars(str){return str.replace(new RegExp("\\\\("+metaChar+")","g"),"\1")}$.each(comparatorOp.split("|"),function(i,op){comparatorOp+="|@"+op});var currentSubject=null;var exprs={group:{query:true,regex:"(node|edge|\\*)",populate:function(group){this.group=group=="*"?group:group+"s"}},state:{query:true,regex:"(:selected|:unselected|:locked|:unlocked|:visible|:hidden|:grabbed|:free|:removed|:inside|:grabbable|:ungrabbable|:animated|:unanimated|:selectable|:unselectable|:parent|:child)",populate:function(state){this.colonSelectors.push(state)
...
}},dataBool:{query:true,regex:"\\[\\s*("+boolOp+")\\s*("+variable+")\\s*\\]",populate:function(boolOp,variable){this.data.push({field:cleanMetaChars(variable),operator:boolOp})}},metaCompare:{query:true,regex:"\\{\\s*("+meta+")\\s*("+comparatorOp+")\\s*("+number+")\\s*\\}",populate:function(meta,comparatorOp,number){this.meta.push({field:cleanMetaChars(meta),operator:comparatorOp,value:number})}},nextQuery:{separator:true,regex:separator,populate:function(){self[++i]=newQuery();currentSubject=null
}},child:{separator:true,regex:child,populate:function(){var childQuery=newQuery();childQuery.parent=this;childQuery.subject=currentSubject;self[i]=childQuery}},descendant:{separator:true,regex:descendant,populate:function(){var descendantQuery=newQuery();descendantQuery.ancestor=this;descendantQuery.subject=currentSubject;self[i]=descendantQuery}},subject:{modifier:true,regex:subject,populate:function(){if(currentSubject!=null&&this.subject!=this){$$.console.error("Redefinition of subject in selector `%s`",selector);
return false}currentSubject=this;this.subject=this},}};self._private.selectorText=selector;var remaining=selector;var i=0;function consumeExpr(expectation){var expr;var match;var name;$.each(exprs,function(n,e){if($$.is.fn(expectation)&&!expectation(n,e)){return}var m=remaining.match(new RegExp("^"+e.regex));if(m!=null){match=m;expr=e;name=n;var consumed=m[0];remaining=remaining.substring(consumed.length);return false}});return{expr:expr,match:match,name:name}}function consumeWhitespace(){var match=remaining.match(/^\s+/);
if(match){var consumed=match[0];remaining=remaining.substring(consumed.length)}}self[0]=newQuery();consumeWhitespace();for(;;){var check=consumeExpr();if(check.expr==null){$$.console.error("The selector `%s` is invalid",selector);return}else{var args=[];for(var j=1;j<check.match.length;j++){args.push(check.match[j])}var ret=check.expr.populate.apply(self[i],args);if(ret===false){return}}if(remaining.match(/^\s*$/)){break}}self.length=i+1;for(j=0;j<self.length;j++){var query=self[j];if(query.subject!=null){for(;
;){if(query.subject==query){break}if(query.parent!=null){var parent=query.parent;var child=query;child.parent=null;parent.child=child;query=parent}else{if(query.ancestor!=null){var ancestor=query.ancestor;var descendant=query;descendant.ancestor=null;ancestor.descendant=descendant;query=ancestor}else{$$.console.error("When adjusting references for the selector `%s`, neither parent nor ancestor was found");break}}}self[j]=query.subject}}if(onlyThisGroup!=null){for(var j=0;j<self.length;j++){if(self[j].group!=null&&self[j].group!=onlyThisGroup){$$.console.error("Group `%s` conflicts with implicit group `%s` in selector `%s`",self[j].group,onlyThisGroup,selector);
return}self[j].group=onlyThisGroup}}}else{$$.console.error("A selector must be created from a string; found %o",selector);return}}}}}self._private.invalid=false}$$.CySelector=CySelector;CySelector.prototype.cy=function(){return this._private.cy};CySelector.prototype.size=function(){return this.length};CySelector.prototype.eq=function(i){return this[i]};CySelector.prototype.find=function(){};CySelector.prototype.filter=function(collection,addLiveFunction){var self=this;if(self._private.invalid){return new $$.CyCollection(self.cy())
}var queryMatches=function(query,element){if(query.group!=null&&query.group!="*"&&query.group!=element._private.group){return false}var allColonSelectorsMatch=true;for(var k=0;k<query.colonSelectors.length;k++){var sel=query.colonSelectors[k];var renderer=self.cy().renderer();switch(sel){case":selected":allColonSelectorsMatch=element.selected();break;case":unselected":allColonSelectorsMatch=!element.selected();break;case":selectable":allColonSelectorsMatch=element.selectable();break;case":unselectable":allColonSelectorsMatch=!element.selectable();
break;case":locked":allColonSelectorsMatch=element.locked();break;case":unlocked":allColonSelectorsMatch=!element.locked();break;case":visible":allColonSelectorsMatch=renderer.elementIsVisible(element);break;case":hidden":allColonSelectorsMatch=!renderer.elementIsVisible(element);break;case":grabbed":allColonSelectorsMatch=element.grabbed();break;case":free":allColonSelectorsMatch=!element.grabbed();break;case":removed":allColonSelectorsMatch=element.removed();break;case":inside":allColonSelectorsMatch=!element.removed();
break;case":grabbable":allColonSelectorsMatch=element.grabbable();break;case":ungrabbable":allColonSelectorsMatch=!element.grabbable();break;case":animated":allColonSelectorsMatch=element.animated();break;case":unanimated":allColonSelectorsMatch=!element.animated();break;case":parent":allColonSelectorsMatch=element.children().nonempty();break;case":child":allColonSelectorsMatch=element.parent().nonempty();break}if(!allColonSelectorsMatch){break}}if(!allColonSelectorsMatch){return false}var allIdsMatch=true;
for(var k=0;k<query.ids.length;k++){var id=query.ids[k];var actualId=element._private.data.id;allIdsMatch=allIdsMatch&&(id==actualId);if(!allIdsMatch){break}}if(!allIdsMatch){return false}var allClassesMatch=true;for(var k=0;k<query.classes.length;k++){var cls=query.classes[k];allClassesMatch=allClassesMatch&&element.hasClass(cls);if(!allClassesMatch){break}}if(!allClassesMatch){return false}function operandsMatch(params){var allDataMatches=true;for(var k=0;k<query[params.name].length;k++){var data=query[params.name][k];
var operator=data.operator;var value=data.value;var field=data.field;var matches;if(operator!=null&&value!=null){var fieldStr=""+params.fieldValue(field);var valStr=""+eval(value);var caseInsensitive=false;if(operator.charAt(0)=="@"){fieldStr=fieldStr.toLowerCase();valStr=valStr.toLowerCase();operator=operator.substring(1);caseInsensitive=true}if(operator=="="){operator="=="}switch(operator){case"*=":matches=fieldStr.search(valStr)>=0;break;case"$=":matches=new RegExp(valStr+"$").exec(fieldStr)!=null;
break;case"^=":matches=new RegExp("^"+valStr).exec(fieldStr)!=null;break;default:if(caseInsensitive){var expr="fieldStr "+operator+" valStr";matches=eval(expr)}else{var expr=params.fieldRef(field)+" "+operator+" "+value;matches=eval(expr)}}}else{if(operator!=null){switch(operator){case"?":matches=params.fieldTruthy(field);break;case"!":matches=!params.fieldTruthy(field);break;case"^":matches=params.fieldUndefined(field);break}}else{matches=!params.fieldUndefined(field)}}if(!matches){allDataMatches=false;
break}}return allDataMatches}var allDataMatches=operandsMatch({name:"data",fieldValue:function(field){return element._private.data[field]},fieldRef:function(field){return"element._private.data."+field},fieldUndefined:function(field){return element._private.data[field]===undefined},fieldTruthy:function(field){if(element._private.data[field]){return true}return false}});if(!allDataMatches){return false}var allMetaMatches=operandsMatch({name:"meta",fieldValue:function(field){return element[field]()},fieldRef:function(field){return"element."+field+"()"
},fieldUndefined:function(field){return element[field]()==undefined},fieldTruthy:function(field){if(element[field]()){return true}return false}});if(!allMetaMatches){return false}if(query.collection!=null){var matchesAny=query.collection._private.ids[element.id()]!=null;if(!matchesAny){return false}}if(query.filter!=null&&element.collection().filter(query.filter).size()==0){return false}function confirmRelations(query,elements){if(query!=null){var matches=false;elements=elements();for(var i=0;i<elements.size();
i++){if(queryMatches(query,elements.eq(i))){matches=true;break}}return matches}else{return true}}if(!confirmRelations(query.parent,function(){return element.parent()})){return false}if(!confirmRelations(query.ancestor,function(){return element.parents()})){return false}if(!confirmRelations(query.child,function(){return element.children()})){return false}if(!confirmRelations(query.descendant,function(){return element.descendants()})){return false}return true};var selectorFunction=function(i,element){for(var j=0;
j<self.length;j++){var query=self[j];if(queryMatches(query,element)){return true}}return false};if(self._private.selectorText==null){selectorFunction=function(){return true}}var filteredCollection=collection.filter(selectorFunction);if(addLiveFunction){var key=self.selector();var structs=self.cy()._private;filteredCollection.live=function(events,data,callback){var evts=events.split(/\s+/);$.each(evts,function(i,event){if($$.is.emptyString(event)){return}if(callback===undefined){callback=data;data=undefined
}if(structs.live[event]==null){structs.live[event]={}}if(structs.live[event][key]==null){structs.live[event][key]=[]}structs.live[event][key].push({callback:callback,data:data})});return this};filteredCollection.die=function(event,callback){if(event==null){$.each(structs.live,function(event){if(structs.live[event]!=null){delete structs.live[event][key]}})}else{var evts=event.split(/\s+/);$.each(evts,function(j,event){if(callback==null){if(structs.live[event]!=null){delete structs.live[event][key]
}}else{if(structs.live[event]!=null&&structs.live[event][key]!=null){for(var i=0;i<structs.live[event][key].length;i++){if(structs.live[event][key][i].callback==callback){structs.live[event][key].splice(i,1);i--}}}}})}return this}}return filteredCollection};CySelector.prototype.toString=CySelector.prototype.selector=function(){var str="";function clean(obj){if($$.is.string(obj)){return obj}return""}function queryToString(query){var str="";var group=clean(query.group);str+=group.substring(0,group.length-1);
for(var j=0;j<query.data.length;j++){var data=query.data[j];str+="["+data.field+clean(data.operator)+clean(data.value)+"]"}for(var j=0;j<query.meta.length;j++){var meta=query.meta[j];str+="{"+meta.field+clean(meta.operator)+clean(meta.value)+"}"}for(var j=0;j<query.colonSelectors.length;j++){var sel=query.colonSelectors[i];str+=sel}for(var j=0;j<query.ids.length;j++){var sel="#"+query.ids[i];str+=sel}for(var j=0;j<query.classes.length;j++){var sel="."+query.classes[i];str+=sel}if(query.parent!=null){str=queryToString(query.parent)+" > "+str
}if(query.ancestor!=null){str=queryToString(query.ancestor)+" "+str}if(query.child!=null){str+=" > "+queryToString(query.child)}if(query.descendant!=null){str+=" "+queryToString(query.descendant)}return str}for(var i=0;i<this.length;i++){var query=this[i];str+=queryToString(query);if(this.length>1&&i<this.length-1){str+=", "}}return str}})(jQuery,jQuery.cytoscapeweb);(function(c,a){function b(d){c.cytoscapeweb("debug","Creating null renderer with options (%o)",d)}b.prototype.notify=function(d){c.cytoscapeweb("debug","Notify null renderer with params (%o)",d)
};b.prototype.zoom=function(d){c.cytoscapeweb("debug","Zoom null renderer with params (%o)",d)};b.prototype.fit=function(d){c.cytoscapeweb("debug","Fit null renderer with params (%o)",d)};b.prototype.pan=function(d){c.cytoscapeweb("debug","Pan null renderer with params (%o)",d)};b.prototype.panBy=function(d){c.cytoscapeweb("debug","Relative pan null renderer with params (%o)",d)};b.prototype.showElements=function(d){};b.prototype.hideElements=function(d){};b.prototype.elementIsVisible=function(d){return d._private.visible
};b.prototype.renderedDimensions=function(){return{}};b.prototype.dimensions=function(){return{}};c.cytoscapeweb("renderer","null",b)})(jQuery,jQuery.cytoscapeweb);(function(j,m){var l={minZoom:0.001,maxZoom:1000,maxPan:-1>>>1,minPan:(-(-1>>>1)-1),selectionToPanDelay:500,dragToSelect:true,dragToPan:true,style:{selectors:{node:{fillColor:"#888",fillOpacity:1,borderColor:"#666",borderOpacity:1,opacity:1,borderWidth:0,borderStyle:"solid",height:10,width:10,shape:"ellipse",cursor:"pointer",visibility:"visible",labelValign:"top",labelHalign:"middle",labelText:{defaultValue:"",passthroughMapper:"label"},labelFillColor:"#000",labelOutlineColor:"#666",labelOutlineWidth:0,labelFontSize:"inherit",labelFontStyle:"normal",labelFontDecoration:"none",labelFontVariant:"italic",labelFontFamily:"Arial",labelFontWeight:"bold",labelOpacity:1,labelOutlineOpacity:1},edge:{lineColor:"#ccc",targetArrowColor:"#ccc",sourceArrowColor:"#ccc",targetArrowShape:"none",sourceArrowShape:"none",opacity:1,width:1,style:"solid",cursor:"pointer",visibility:"visible",labelText:{defaultValue:"",passthroughMapper:"label"},labelFillColor:"#000",labelOutlineColor:"#666",labelOutlineWidth:0,labelFontSize:"inherit",labelFontStyle:"normal",labelFontDecoration:"none",labelFontVariant:"italic",labelFontFamily:"Arial",labelFontWeight:"bold",labelOutlineOpacity:1,labelOpacity:1}},global:{panCursor:"grabbing",selectionFillColor:"#ccc",selectionOpacity:0.5,selectionBorderColor:"#888",selectionBorderWidth:1}}};
...
};f.prototype.offsetFix=function(v){var u=this;if(v.offsetX==null||v.offsetY==null){v.offsetX=v.pageX-u.cy.container().offset().left;v.offsetY=v.pageY-u.cy.container().offset().top}};f.prototype.makeBackgroundInteractive=function(){var G=this;var P=G.svgRoot;var R=G.options.selectionToPanDelay;var u=false;var x=false;var U={x:null,y:null};var A=0;G.shiftDown=false;j(window).bind("keydown keyup",function(V){G.shiftDown=V.shiftKey});function w(V){return V.target==P||j(V.target).parents("g:last")[0]==G.edgesGroup||j(V.target)[0]==G.svgBg
}j(window).bind("blur",function(){u=false;x=false}).bind("mousemove",function(W){var V=false;if(Math.abs(W.screenX-U.x)>A||Math.abs(W.screenY-U.y)>A){V=true}U={x:W.screenX,y:W.screenY};if(u&&V){x=true}});j(P).bind("mousedown",function(X){if(X.button!=0){return}if(w(X)){X.preventDefault();G.offsetFix(X);var Y=null;var W={};var Z=true;var ab=true;if(!G.options.dragToSelect){ab=false}if(!G.options.dragToPan){Z=false}if(Z&&ab){Z=false;ab=true}var ai=X.pageX;var af=X.pageY;var V=X.offsetX;var ak=X.offsetY;
var aj=0;var ah=0;var ag=false;function ae(){if(ag){return}ag=true;G.svg.change(P,{cursor:h(G.style.global.panCursor)})}if(G.options.dragToPan){var aa=setTimeout(function(){if(!G.cy.panning()){return}Z=true;ab=false},R)}var ac=function(aq){clearTimeout(aa);var ao=aq.pageX-ai;var an=aq.pageY-af;ai=aq.pageX;af=aq.pageY;aj+=ao;ah+=an;if(Z){G.translation.x+=ao;G.translation.y+=an;ae();G.pan(G.translation)}if(ab){if(Y==null){Y=G.svg.rect(V,ak,0,0,{fill:q(G.style.global.selectionFillColor),opacity:b(G.style.global.selectionOpacity),stroke:q(G.style.global.selectionBorderColor),strokeWidth:c(G.style.global.selectionBorderWidth)})
}else{var ap=Math.abs(aj);var am=Math.abs(ah);var al=aj>=0?V:V+aj;var ar=ah>=0?ak:ak+ah;W={x1:al,y1:ar,x2:al+ap,y2:ar+am};G.svg.change(Y,{x:al,y:ar,width:ap,height:am})}}};j(window).bind("mousemove",ac);var ad=function(al){if(al.type=="mouseup"&&al.button!=0){return}clearTimeout(aa);j(window).unbind("mousemove",ac);j(window).unbind("mouseup",ad);j(window).unbind("blur",ad);j(P).unbind("mouseup",ad);if(Z){G.svg.change(P,{cursor:null})}if(ab){if(Y!=null&&W.x1!=null&&!isNaN(W.x1)){G.selectElementsFromIntersection(Y,W);
G.svgRemove(Y)}else{if(!G.shiftDown){G.unselectAll()}}}};j(window).bind("mouseup",ad);j(window).bind("blur",ad);j(P).bind("mouseup",ad)}}).bind("mouseover",function(){u=true;x=false}).bind("mouseout",function(){u=false;x=false}).bind("mousewheel",function(aa,ab,X,W){if(!G.cy.panning()||!G.cy.zooming()||!x){return}G.offsetFix(aa.originalEvent);var V={x:aa.originalEvent.offsetX,y:aa.originalEvent.offsetY};var Y=0.5;if(j.browser.mozilla||j.browser.msie){Y=0.167}var Z=G.zoom()*(1+W*Y);G.zoomAboutPoint(V,Z);
G.cy.trigger("zoom");G.cy.trigger("pan");aa.preventDefault()});function L(Y,W){var V,ab;var Z=G.cy.container().offset();var X=Y.originalEvent.touches;var aa=X[W];V=aa.pageX-Z.left;ab=aa.pageY-Z.top;return{x:V,y:ab}}function M(W){var X=L(W,0);var V=L(W,1);return{x:(X.x+V.x)/2,y:(X.y+V.y)/2}}function B(W){var X=L(W,0);var V=L(W,1);return G.getDistance(X,V)}function Q(V){return V.originalEvent.touches==null?0:V.originalEvent.touches.length}function z(V,W){return Q(V)>=W}function O(V){if(V>=2){T=true;
v=true;clearTimeout(y);y=setTimeout(function(){v=false},I)}else{T=false}}var I=100;var T=false;var v=false;var y=null;var K=true;var S,C,H,E,D,N,J,F;j(P).bind("touchstart",function(V){if(!w(V)||G.touchingNodes.size()>0){return}V.preventDefault();E=L(V,0);if(z(V,2)){S=M(V);C=G.modelPoint(S);H=B(V);D=L(V,1);O(2)}else{O(1);K=true}F=false}).bind("touchmove",function(W){if(!w(W)||G.touchingNodes.size()>0){return}K=false;if(z(W,2)){O(2);J=L(W,1)}else{O(1)}W.preventDefault();var ac={x:0,y:0};if(z(W,1)&&G.cy.panning()){N=L(W,0);
if(z(W,2)&&G.cy.zooming()){var V=B(W);var X=V/H;S=G.renderedPoint(C);if(X!=1){var ab=1.5;var aa={x:N.x-E.x,y:N.y-E.y};var Z={x:J.x-D.x,y:J.y-D.y};ac={x:(aa.x+Z.x)/2,y:(aa.y+Z.y)/2};if(X>1){X=(X-1)*ab+1}else{X=1-(1-X)*ab}var Y=G.zoom()*X;G.zoomAboutPoint(S,Y,ac);H=V}}else{if(!v){ac={x:N.x-E.x,y:N.y-E.y};G.panBy(ac)}}E=N;D=J}}).bind("touchend",function(V){if(K&&w(V)){G.unselectAll()}});j(P).bind("mousedown mouseup click mouseover mouseout mousemove touchstart touchmove touchend",function(W){if(w(W)){var V=j.extend({},W,{cyTarget:G.cy});
G.cy.background().trigger(V);G.cy.trigger(V)}})};f.prototype.zoomAboutPoint=function(A,B,u){var C=this;var v=C.cy;if(!v.panning()||!v.zooming()){return}var x=C.pan();var z=C.zoom();var y=B;if(u==null){u={x:0,y:0}}var w={x:-y/z*(A.x-x.x-u.x)+A.x,y:-y/z*(A.y-x.y-u.y)+A.y};C.transform({translation:w,scale:y})};f.prototype.zoom=function(v){var x=this.cy;if(!x.zooming()){return}if(v===undefined){return this.scale}else{if(typeof v==typeof{}){var u=v;var w;if(u.position!==undefined){w=this.renderedPoint(u.position)
}else{w=u.renderedPosition}if(w!==undefined){this.zoomAboutPoint(w,v.level)}else{this.transform({scale:u.level})}}else{this.transform({scale:v})}}};f.prototype.fit=function(D){var u=D.elements;var L=D.zoom;var C=this.cy;if(!C.panning()||(L!==undefined&&!C.zooming())){return}if(u==null||u.size()==0){u=this.cy.elements()}if(u.is(":removed")){m.console.debug("SVG renderer does not take into account removed elements when fitting");u=u.filter(":inside")}m.console.debug("Fit SVG renderer to view bounds");
var A=this.nodesGroup.getBBox();var y,I,v,H;function E(O){if(O.height==0||O.width==0){return}var N=O.x;var z=N+O.width;var M=O.y;var w=M+O.height;if(N<y||y==null){y=N}if(z>v||v==null){v=z}if(M<I||I==null){I=M}if(w>H||H==null){H=w}}u.nodes().each(function(){var z=this.renscratch("svg").getBBox();var w=this.renscratch("svgLabel").getBBox();E(z);E(w)});u.edges().each(function(){var O=this.source().id();var P=this.target().id();var z=lf=0.4;if(O==P){var N=this.renscratch("svg").getBBox();N.x2=N.x+N.width;
N.y2=N.y+N.height;N.x1=N.x;N.y1=N.y;var M={};M.x=N.x1+N.width*lf;M.y=N.y1+N.height*lf;M.width=N.x2-M.x;M.height=N.y2-M.y;var w=this.renscratch("svgLabel").getBBox();E(M);E(w)}else{var N=this.renscratch("svg").getBBox();var w=this.renscratch("svgLabel").getBBox();E(N);E(w)}});var J=v-y;var F=H-I;var x=this.cy.container().width();var K=this.cy.container().height();var B=Math.min(x/J,K/F);if(L){this.transform({translation:{x:-y*B-(J*B-x)/2,y:-I*B-(F*B-K)/2},scale:B})}else{var G=this.scale;this.transform({translation:{x:-y*G+x/2-(v-y)/2*G,y:-I*G+K/2-(H-I)/2*G}})
...
};f.prototype.updateNodePositionFromShape=function(v){var x=v.style(false);var w=v.renscratch("svgGroup");var u=v.position(false);o(x.shape).update(this.svg,w,v,u,x)};f.prototype.makeSvgEdgeInteractive=function(y){var v=y.renscratch("svg");var z=y.renscratch("svgTargetArrow");var x=y.renscratch("svgSourceArrow");var w=j(v).parents("svg:first")[0];var u=this;j(v).add(z).add(x).bind("mouseup mousedown click touchstart touchmove touchend mouseover mousemove mouseout",function(A){if(u.edgeEventIsValid(y,A)){y.trigger(A)
}}).bind("click touchend",function(A){u.selectElement(y)})};f.prototype.makeSvgNodeLabelInteractive=function(u){};f.prototype.makeSvgNodeInteractive=function(x){var v=x.renscratch("svg");var w=j(v).parents("svg:first")[0];var u=this;var y=null;j(v).bind("mousedown touchstart",function(B){y=false;x.trigger(B);if(x.grabbed()||x.locked()||!x.grabbable()){B.preventDefault();return}if(B.type=="touchstart"&&B.originalEvent.touches.length>1){return}x._private.grabbed=true;x.trigger(j.extend({},B,{type:"grab"}));
u.touchingNodes=u.touchingNodes.add(x);var J,H;if(B.type=="touchstart"){var E=B.originalEvent.touches;var D=E[E.length-1];J=D.pageX;H=D.pageY}else{J=B.pageX;H=B.pageY}var A;if(x.selected()){A=u.selectedElements.add(x).filter(":grabbable")}else{A=x.collection()}var I=true;var F=function(M){y=true;var P,O;if(M.type=="touchmove"){var N=B.originalEvent.touches;var Q=N[N.length-1];P=Q.pageX;O=Q.pageY}else{P=M.pageX;O=M.pageY}var L=(P-J)/u.zoom();var K=(O-H)/u.zoom();J=P;H=O;A.each(function(R,S){S.element()._private.position.x+=L;
S.element()._private.position.y+=K});u.updatePosition(A);if(I){if(M.type!="touchmove"){u.moveToFront(x)}I=false}else{x.trigger(j.extend({},M,{type:"position"}));x.trigger(j.extend({},M,{type:"drag"}))}};j(window).bind("mousemove touchmove",F);var C=false;var z=0;var G=function(K){if(K.type=="touchend"&&K.originalEvent.touches.length!=0){return}if(!C){C=true}else{return}j(window).unbind("mousemove touchmove",F);j(window).unbind("mouseup touchend blur",G);j(v).unbind("mouseup touchend",G);x._private.grabbed=false;
u.touchingNodes=u.touchingNodes.not(x);x.trigger(j.extend({},K,{type:"free"}))};j(window).bind("mouseup touchend blur",G);j(v).bind("mouseup touchend",G);B.preventDefault()}).bind("mouseup touchend",function(z){x.trigger(j.extend({},z));if(y==false){y=null;x.trigger(j.extend({},z,{type:"click"}));u.selectElement(x)}}).bind("mouseover mouseout mousemove",function(z){if(u.nodeEventIsValid(x,z)){x.trigger(j.extend({},z))}})};f.prototype.edgeEventIsValid=function(w,x){var u=j(x.relatedTarget);var v=this;
...
};f.prototype.renderedStyle=function(w){var v=this;var x=w.style();var u=["width","height","borderWidth","labelOutlineWidth"];j.each(u,function(y,z){if(x[z]!=null){x[z]=x[z]*v.zoom()}});return x};f.prototype.unselectElements=function(u){u=u.collection();u.unselect();this.selectedElements=this.selectedElements.not(u)};f.prototype.selectElementsFromIntersection=function(x,z){var v=this;var w=this.cy.collection();var u=this.cy.collection();function y(F){if(!v.elementIsVisible(F)){return false}var I=o(F.style("shape")).intersectionShape;
var M=v.modelPoint({x:z.x1,y:z.y1});var J=v.modelPoint({x:z.x2,y:z.y2});var D=v.svg.rect(M.x,M.y,J.x-M.x,J.y-M.y);var B=Intersection.intersectShapes(new Rectangle(D),new I(F.renscratch("svg")));v.svgRemove(D);var Q=v.zoom();var O=F.renderedPosition().x;var L=F.renderedPosition().y;var P=F.renderedStyle().width+F.style("borderWidth")*Q;var G=F.renderedStyle().height+F.style("borderWidth")*Q;var C=z.x1;var N=z.y1;var A=z.x2;var K=z.y2;var E=C<=O&&O<=A&&N<=L&&L<=K;var H=B.points.length>0;return E||H
...
this.updateElementStyle(w,x);return v};f.prototype.makeSvgNodeLabel=function(w){var v=this;var u=w.position("x");var z=w.position("y");w.renscratch().svgLabelGroup=v.svg.group(w.renscratch().svgGroup);w.renscratch().svgLabelOutline=v.svg.text(w.renscratch().svgLabelGroup,u,z,"label init");w.renscratch().svgLabel=v.svg.text(w.renscratch().svgLabelGroup,u,z,"label init")};f.prototype.positionSvgNodeLabel=function(w){var v=this;var u=w.position("x");var z=w.position("y");v.svg.change(w.renscratch().svgLabel,{x:u,y:z});
v.svg.change(w.renscratch().svgLabelOutline,{x:u,y:z})};f.prototype.makeSvgEdgePath=function(K){var G=this;var ai=K.target();var Q=K.source();var z=ai.data("id")==Q.data("id");var ad;var M=Q.position("x");var av=Q.position("y");var L=ai.position("x");var au=ai.position("y");if(M==L&&av==au){L++;au++}var J=K.parallelEdges();var D=J.size();var aa;var C;var ac=20;var aj=20;var W,T,S;var Z=G.getDistance({x:M,y:av},{x:L,y:au});var al=200;if(!z&&F){ac=Math.min(20+4000/Z,al)}J.each(function(aw,ax){if(ax==K){aa=aw
}});function v(){var aw=C!=0;var ax=G.svg.createPath();if(ad!=null){G.svgRemove(ad)}if(z){ad=G.svg.path(K.renscratch("svgGroup"),ax.move(M,av).curveC(T.x,T.y,S.x,S.y,L,au))}else{if(aw){ad=G.svg.path(K.renscratch("svgGroup"),ax.move(M,av).curveQ(W.x,W.y,L,au))}else{ad=G.svg.path(K.renscratch("svgGroup"),ax.move(M,av).line(L,au))}}}if(z){var O=Q.style("height");var I=Q.style("width");ac+=Math.max(I,O);C=aa;ac+=aj*(C);var ap=ac;T={x:M,y:av-O/2-ap};S={x:M-I/2-ap,y:av};v()}else{var ae=D%2==0;if(ae){C=aa-D/2+(aa<D/2?0:1);
...
var y=Intersection.intersectShapes(new Path(ad),new af(ai.renscratch("svg")));var ag=y.points[y.points.length-1];y=Intersection.intersectShapes(new Path(ad),new X(Q.renscratch("svg")));var H=y.points[0];var ab=aq*u;var E=-1*(this.getAngle(T,{x:M,y:av})-90);var V=-1*(this.getAngle(S,{x:L,y:au})-90);if(ag!=null){if(w!="none"){var N=G.getPointAlong(ag,S,x/2,ag);L=N.x;au=N.y}else{if(ag!=null){L=ag.x;au=ag.y}}}if(H!=null){if(ar!="none"){var U=G.getPointAlong(H,T,x/2,H);M=U.x;av=U.y}else{M=H.x;av=H.y}}v();
if(K.renscratch("svgTargetArrow")!=null){this.svgRemove(K.renscratch("svgTargetArrow"))}if(w!="none"){var am=k(w);var Y={x:L-am.centerPoint.x*ab,y:au-am.centerPoint.y*ab,};var B=am.centerPoint;var ao=am==null?null:am.svg(this.svg,K.renscratch("svgGroup"),K,K.position(false),K.style(false));K.renscratch("svgTargetArrow",ao);this.svg.change(ao,{transform:"translate("+Y.x+" "+Y.y+") scale("+ab+") rotate("+V+" "+B.x+" "+B.y+")"})}if(K.renscratch("svgSourceArrow")!=null){this.svgRemove(K.renscratch("svgSourceArrow"))
...
return v.renscratch().svg};f.prototype.makeSvgElement=function(v){var u;if(v.group()=="nodes"){u=this.makeSvgNode(v)}else{if(v.group()=="edges"){u=this.makeSvgEdge(v)}}return u};f.prototype.getSvgElement=function(u){if(u.renscratch().svg!=null){return u.renscratch().svg}else{return this.makeSvgElement(u)}};f.prototype.updateSelection=function(u){this.updateElementsStyle(u)};f.prototype.updateClass=function(u){this.updateElementsStyle(u)};f.prototype.updateData=function(v,u){this.updateElementsStyle(v);
