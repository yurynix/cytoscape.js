...
/**
 * This file is part of Cytoscape Web snapshot-2012.03.25-20.20.40.
 * 
...
})}})}else{d.push(new a.CyElement(cy,c))}}}}});this.notify({type:"add",collection:d});return new a.CyCollection(cy,d)},remove:function(d){if(!a.is.elementOrCollection(d)){d=d}else{var c=d;d=this.filter(c)}return d.remove()},load:function(d,e,c){var g=this;g.elements().remove();g.notifications(false);if(d!=null){if(a.is.plainObject(d)){b.each(["nodes","edges"],function(h,j){var k=d[j];if(k!=null){b.each(k,function(m,n){var l=new a.CyElement(g,b.extend({},n,{group:j}))})}})}else{if(a.is.array(d)){b.each(d,function(j,k){var h=new a.CyElement(g,k)
})}}}function f(){g.layout(b.extend({},g._private.options.layout,{ready:function(){g.notifications(true);g.notify({type:"load",collection:g.elements(),style:g._private.style});if(a.is.fn(e)){e.apply(g,[g])}g.trigger("load");g.trigger("layoutready")},stop:function(){if(a.is.fn(c)){c.apply(g,[g])}g.trigger("layoutdone")}}))}if(window.chrome){setTimeout(function(){f()},30)}else{f()}return this}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({startAnimationLoop:function(){var h=this;var n=this._private;
var g=10;var o=false;var k=true;n.animation.elements=new a.CyCollection(h);var l=window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame;if(l==null||!k){l=function(p){window.setTimeout(function(){p(+new Date)},g)}}var d=h.container()[0];function m(){function p(){l(function(q){e(q);m()},d)}if(o){setTimeout(function(){p()},g)}else{p()}}m();function e(p){n.animation.elements.each(function(t,w){try{w=w.element();var y=w._private.animation.current;
var r=w._private.animation.queue;if(y.length==0){var v=r;var u=v.length>0?v.shift():null;if(u!=null){u.callTime=+new Date;y.push(u)}}b.each(y,function(z,q){f(w,q,p)});var s=[];for(var t=0;t<y.length;t++){if(y[t].done){s.push(y[t].params.complete);y.splice(t,1);t--}}b.each(s,function(q,z){if(a.is.fn(complete)){complete.apply(w)}})}catch(x){}});if(n.animation.elements.size()>0){h.notify({type:"draw",collection:n.animation.elements})}n.animation.elements=n.animation.elements.filter(function(){var r=this;
var q=r._private.animation.queue;var s=r._private.animation.current;return s.length>0||q.length>0})}function f(p,u,q){var r=u.properties;var v=u.params;var s=u.callTime;var t;if(v.duration==0){t=1}else{t=Math.min(1,(q-s)/v.duration)}function w(y){if(y.end!=null){var z=y.start;var x=y.end;b.each(x,function(A,B){if(c(z[A],x[A])){p._private[y.field][A]=j(z[A],x[A],t)}})}}if(r.delay==null){w({end:r.position,start:u.startPosition,field:"position"});w({end:r.bypass,start:u.startStyle,field:"bypass"})}if(a.is.fn(v.step)){v.step.apply(p,[q])
}if(t>=1){u.done=true}return t}function c(q,p){if(q==null||p==null){return false}if(a.is.number(q)&&a.is.number(p)){return true}else{if((q)&&(p)){return true}}return false}function j(s,t,x){if(a.is.number(s)&&a.is.number(t)){return s+(t-s)*x}else{if((s)&&(t)){var v=b.Color(s).fix().toRGB();var u=b.Color(t).fix().toRGB();function q(A,r){var B=r-A;var z=A;return Math.round(x*B+z)}var p=q(v.red(),u.red());var w=q(v.green(),u.green());var y=q(v.blue(),u.blue());return b.Color([p,w,y],"RGB").toHEX().toString()
}}return undefined}}})})(jQuery,jQuery.cytoscapeweb);(function(e,a){a.fn.core({one:d({target:"cy",one:true}),bind:d({target:"cy"}),unbind:c({target:"cy"}),trigger:b({target:"cy"}),delegate:function(f,g,j,h){this.$(f).live(g,j,h);return this},undelegate:function(f,g,h){this.$(f).die(g,h);return this},on:function(g,f,j,h){if(a.is.string(f)){this.$(f).live(g,j,h)}else{f=undefined;j=f;h=j;this.bind(g,j,h)}return this},off:function(h,f,g){if(a.is.string(f)){this.$(f).live(events,g)}else{g=f;f=undefined;
...
g=e.extend({},f,g);return function(h,l,k){var m=this;var j=m._private.listeners;if(k===undefined){k=l;l=undefined}h=h.split(/\s+/);e.each(h,function(n,o){if(a.is.emptyString(o)){return}if(j[g.target]==null){j[g.target]={}}if(j[g.target][o]==null){j[g.target][o]=[]}j[g.target][o].push({callback:k,data:l,one:g.one})});return m}}function c(g){var f={target:"cy"};g=e.extend({},f,g);return function(h,k){var l=this;var j=l._private.listeners;if(j[g.target]==null){return}h=h.split(/\s+/);e.each(h,function(m,n){if(a.is.emptyString(n)){return
}if(k===undefined){delete j[g.target][n];return}else{for(var m=0;m<j[g.target][n].length;m++){var o=j[g.target][n][m];if(o.callback==k){j[g.target][n].splice(m,1);m--}}}});return l}}function b(g){var f={target:"cy"};g=e.extend({},f,g);return function(k,l){var n=this;var j=n._private.listeners;var h;var m=g.target;if(a.is.plainObject(k)){h=[k.type]}else{h=k.split(/\s+/)}e.each(h,function(q,r){if(j[m]==null||j[m][r]==null){return}for(var p=0;p<j[m][r].length;p++){var u=j[m][r][p];var s;if(a.is.plainObject(k)){s=k;
k=s.type}else{s=e.Event(k)}s.data=u.data;s.cy=s.cytoscapeweb=n;var o=[s];if(l!=null){e.each(l,function(v,t){o.push(t)})}if(u.one){j[m][r].splice(p,1);p--}u.callback.apply(n,o)}})}}})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({exportTo:function(f){var e=f.name;var d=a.extension("exporter",e);if(d==null){a.console.error("No exporter with name `%s` found; did you remember to register it?",e)}else{var c=new d({cy:cy,renderer:this.renderer()});return c.run()}}})})(jQuery,jQuery.cytoscapeweb);
...
return}this._private.renderer=new c(b.extend({},d,{cy:e,style:e._private.style,styleCalculator:{calculate:function(l,u){if(a.is.plainObject(u)){var p;if(u.customMapper!=null){p=u.customMapper.apply(l,[l.data()])}else{if(u.passthroughMapper!=null){var r=u.passthroughMapper;p=l._private.data[r]}else{if(u.discreteMapper!=null){var r=u.discreteMapper.attr;var m=u.discreteMapper.entries;var s=l.data(r);b.each(m,function(x,y){var w=y.attrVal;var z=y.mappedVal;if(w==s){p=z}})}else{if(u.continuousMapper!=null){var f=u.continuousMapper;
if(f.attr.name==null||typeof f.attr.name!=typeof""){a.console.error("For style.%s.%s, `attr.name` must be defined as a string since it's a continuous mapper",l.group(),styleName);return}var g=e._private.continuousMapperBounds[l._private.group][f.attr.name];g={min:g==null?0:g.min,max:g==null?0:g.max};if(f.attr.min!=null){g.min=f.attr.min}if(f.attr.max!=null){g.max=f.attr.max}if(g!=null){var k=l.data(f.attr.name);var o=(k-g.min)/(g.max-g.min);if(g.max==g.min){o=1}if(o>1){o=1}else{if(o<0||k==null||isNaN(o)){o=0
}}if(k==null&&u.defaultValue!=null){p=u.defaultValue}else{if(a.is.number(f.mapped.min)&&a.is.number(f.mapped.max)){p=o*(f.mapped.max-f.mapped.min)+f.mapped.min}else{if((f.mapped.min)&&(f.mapped.max)){var n=b.Color(f.mapped.min).fix().toRGB();var v=b.Color(f.mapped.max).fix().toRGB();var h=Math.round(n.red()*(1-o)+v.red()*o);var j=Math.round(n.green()*(1-o)+v.green()*o);var t=Math.round(n.blue()*(1-o)+v.blue()*o);p=b.Color([h,j,t],"RGB").toHEX().toString()}else{a.console.error("Unsupported value used in mapper for `style.%s.%s` with min mapped value `%o` and max `%o` (neither number nor colour)",l.group(),f.styleName,f.mapped.min,f.mapped.max);
return}}}}else{a.console.error("Attribute values for `%s.%s` must be numeric for continuous mapper `style.%s.%s` (offending %s: `%s`)",l.group(),f.attr.name,l.group(),styleName,l.group(),l.data("id"));return}}}}}var q=u.defaultValue;if(p==null){p=q}}else{p=u}return p}}}))}})})(jQuery,jQuery.cytoscapeweb);(function($,$$){$$.fn.core({scratch:function(name,value){if(value===undefined){return eval("this._private.scratch."+name)}else{eval("this._private.scratch."+name+" = "+value+";");return this}},removeScratch:function(name){if(name===undefined){structs.scratch={}
}else{eval("delete this._private.scratch."+name+";")}return this}})})(jQuery,jQuery.cytoscapeweb);(function(c,a){a.fn.core({collection:function(){return new a.CyCollection(this)},getElementById:function(d){return this._private.nodes[d]||this._private.edges[d]},nodes:b({addLiveFunction:true,group:"nodes"}),edges:b({addLiveFunction:true,group:"edges"}),elements:function(){return this.$.apply(this,arguments)},$:b({addLiveFunction:true}),filter:function(d){if(a.is.string(d)){return this.$(d)}else{if(a.is.fn(d)){return this.$(d).filter(d)
}}}});function b(f){var e={group:undefined,addLiveFunction:false};f=c.extend({},e,f);var d=[];if(f.group==null){d=["nodes","edges"]}else{d=[f.group]}return function(g){var k=this;var h=[];c.each(d,function(l,m){c.each(k._private[m],function(o,n){h.push(n)})});var j=new a.CyCollection(k,h);return new a.CySelector(k,g).filter(j,f.addLiveFunction)}}})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({style:function(d){var c;if(d===undefined){c=a.util.copy(this._private.style)}else{this._private.style=a.util.copy(d);
...
}var o=this.cy()._private;if(this._private.data.id==null){this._private.data.id=b.generate(this,this._private.group)}else{if(this.cy().getElementById(this._private.data.id)!=null){c.console.error("Can not create element: an element in the visualisation in group `%s` already has ID `%s`",this._private.group,this._private.data.id);return this}}if(this.isEdge()){var m=["source","target"];for(var l=0;l<m.length;l++){var n=m[l];var j=this._private.data[n];if(j==null||j==""){c.console.error("Can not create edge with id `%s` since it has no `%s` attribute set in `data` (must be non-empty value)",this._private.data.id,n);
return}else{if(o.nodes[j]==null){c.console.error("Can not create edge with id `%s` since it specifies non-existant node as its `%s` attribute with id `%s`",this._private.data.id,n,j);return}}}function q(u,s,t,v){var r=s.element()._private.data.id;var w=t.element()._private.data.id;if(u._private.edges[r]==null){u._private.edges[r]={}}u._private.edges[r][w]=d.extend({edge:t},v)}var h=this.cy().getElementById(this._private.data.source);var k=this.cy().getElementById(this._private.data.target);q(h,k,this,{source:true});
q(k,h,this,{target:true})}this._private.removed=false;o[this._private.group][this._private.data.id]=this;var p=this;d.each(this._private.data,function(r,s){p.cy().addContinuousMapperBounds(p,r,s)});g=g.add(this)});if(g.size()>0){if(f){g.rtrigger("add")}else{g.trigger("add")}}return this}});c.fn.collection({removed:function(){return this.element()._private.removed}});c.fn.collection({remove:function(f){var g=new a(this.cy());this.each(function(){var j=this.cy()._private;function h(k){delete j[k._private.group][k._private.data.id];
k._private.removed=true;var m=k._private.group;if(k.isEdge()){function l(q,p){var n=p.element()._private.data.id;var o=k.element()._private.data.id;delete q.element()._private.edges[n][selfId]}l(k.source(),k.target());l(k.target(),k.source())}d.each(k._private.data,function(n,o){k.cy().removeContinuousMapperBounds(k,n,o)});g=g.add(k)}if(!this._private.removed){h(this)}});if(g.size()>0){if(f){this.cy().notify({type:"remove",collection:g})}g.rtrigger("remove")}return this}})})(jQuery,jQuery.cytoscapeweb);
(function(b,a){a.fn.collection({animated:function(){return this.element()._private.animation.current.length>0}});a.fn.collection({clearQueue:function(){return this.each(function(){this.element()._private.animation.queue=[]})}});a.fn.collection({delay:function(c){return this.animate({delay:c},{duration:c})}});a.fn.collection({animate:function(d,e){var c=+new Date;return this.each(function(){var j=this;var k=a.util.copy(j._private.position);var f=a.util.copy(j.style());var g=this.cy()._private;e=b.extend(true,{},{duration:400},e);
...
b.each(e._private.animation.current,function(f,g){if(d){b.each(g.properties,function(h,j){b.each(j,function(l,k){e._private[h][l]=k})})}});e._private.animation.current=[];if(c){e._private.animation.queue=[]}});this.cy().notify({collection:this,type:"draw"});return this}});a.fn.collection({show:function(){this.cy().renderer().showElements(this.collection());return this}});a.fn.collection({hide:function(){this.cy().renderer().hideElements(this.collection());return this}});a.fn.collection({visible:function(){return this.cy().renderer().elementIsVisible(this.element())
}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({addClass:function(d){d=d.split(/\s+/);var c=this;b.each(d,function(f,e){if(a.is.emptyString(e)){return}c.each(function(){this._private.classes[e]=true})});c.rtrigger("class");return c}});a.fn.collection({hasClass:function(c){return this.element()._private.classes[c]==true}});a.fn.collection({toggleClass:function(j,d){var f=j.split(/\s+/);var e=this;var g=[];function c(m,l){var n=m._private.classes[l]!==undefined;delete m._private.classes[l];
if(n){g.push(m)}}function h(m,l){var n=m._private.classes[l]===undefined;m._private.classes[l]=true;if(n){g.push(m)}}e.each(function(){var l=this;b.each(f,function(n,m){if(m==null||m==""){return}if(d===undefined){if(l.hasClass(m)){c(l,m)}else{h(l,m)}}else{if(d){h(l,m)}else{c(l,m)}}})});if(g.length>0){var k=new a.CyCollection(e.cy(),g);k.rtrigger("class")}return e}});a.fn.collection({removeClass:function(e){e=e.split(/\s+/);var c=this;var d=[];b.each(e,function(h,g){if(g==null||g==""){return}c.each(function(){var j=this._private.classes[g]!==undefined;
delete this._private.classes[g];if(j){d.push(this)}})});if(d.length>0){var f=new a.CyCollection(c.cy(),d);f.rtrigger("class")}return c}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.collection({allAre:function(c){return this.filter(c).size()==this.size()}});a.fn.collection({is:function(c){return new a.CySelector(this.cy(),c).filter(this).size()>0}});a.fn.collection({same:function(c){return this.element()===c.element()}});a.fn.collection({anySame:function(h){h=h.collection();var d=false;for(var e=0;
e<h.size();e++){var g=h.eq(e).element();for(var c=0;c<this.size();c++){var f=this.eq(c);d=d||f.same(g);if(d){break}}if(d){break}}return d}});a.fn.collection({allSame:function(k){k=k.collection();if(k.size()!=this.size()){return false}var d=true;for(var e=0;e<k.size();e++){var h=k.eq(e);var g=false;for(var c=0;c<this.size();c++){var f=this.eq(c);g=f.same(h);if(g){break}}d=d&&g;if(!d){break}}return d}});a.fn.collection({allAreNeighbors:function(f){f=f.collection();var e=this.neighborhood();for(var d=0;
d<f.size();d++){var c=f.eq(d);if(c.intersect(e).size()==0){return false}}return true}});a.fn.collection({allAreNeighbours:function(){return this.allAreNeighbors.apply(this,[arguments])}})})(jQuery,jQuery.cytoscapeweb);(function(d,a){a.fn.collection({data:b({attr:"data",allowBinding:true,bindingEvent:"data",settingTriggersEvent:true,settingEvent:"data",validKey:{forSet:function(e){switch(e){case"id":case"source":case"target":return false;default:return true}}},onSet:function(g,e,f){this.cy().updateContinuousMapperBounds(this,g,e,f)
...
})}});b.fn.collection({filter:function(e){var g=this.cy();if(b.is.fn(e)){var f=[];this.each(function(j,h){if(e.apply(h,[j,h])){f.push(h)}});return new b.CyCollection(this.cy(),f)}else{if(b.is.string(e)){return new b.CySelector(this.cy(),e).filter(this)}else{if(e===undefined){return this}}}b.console.warn("You must pass a function or a selector to `filter`");return new b.CyCollection(this.cy())}});b.fn.collection({not:function(e){if(e==null){return this}else{if(b.is.string(e)){e=this.filter(e)}var f=[];
e=e.collection();this.each(function(j,h){var g=e._private.ids[h.id()];if(!g){f.push(h.element())}});return new b.CyCollection(this.cy(),f)}}});b.fn.collection({intersect:function(l){var n=this;if(b.is.string(l)){var h=l;return this.filter(h)}if(b.is.element(l)){l=l.collection()}var e=[];var k=this;var j=l;var m=this.size()<l.size();var g=m?k._private.ids:j._private.ids;var f=m?j._private.ids:k._private.ids;d.each(g,function(o){if(f[o]){e.push(n.cy().getElementById(o))}});return new b.CyCollection(this.cy(),e)
...
}});b.fn.collection({edgesWith:a()});b.fn.collection({edgesTo:a({include:function(g,f,e){return e.source}})});b.fn.collection({edgesFrom:a({include:function(g,f,e){return e.target}})});function a(f){var e={include:function(j,h,g){return true}};f=d.extend(true,{},e,f);return function(h){var g=[];this.nodes().each(function(j,k){h.nodes().each(function(l,m){d.each(k.element()._private.edges[m.id()],function(o,n){if(f.include(k,m,n)){g.push(m.element())}})})});return new b.CyCollection(this.cy(),g)}}b.fn.collection({connectedEdges:function(e){var f=[];
this.nodes().each(function(g,h){d.each(h.element()._private.edges,function(j,k){d.each(k,function(m,l){f.push(l.edge)})})});return new b.CyCollection(this.cy(),f).filter(e)}});b.fn.collection({connectedNodes:function(e){var f=[];this.edges().each(function(g,h){f.push(h.source().element());f.push(h.target().element())});return new b.CyCollection(this.cy(),f).filter(e)}});b.fn.collection({parallelEdges:c()});b.fn.collection({codirectedEdges:c({include:function(f,g,e){return e.source}})});function c(f){var e={include:function(h,j,g){return true
}};f=d.extend(true,{},e,f);return function(g){var h=[];this.edges().each(function(j,k){var l=k.source().element();var m=k.target().element();d.each(l._private.edges[m.id()],function(o,n){if(f.include(l,m,n)){h.push(n.edge)}})});return new b.CyCollection(this.cy(),h).filter(g)}}})(jQuery,jQuery.cytoscapeweb);(function(b){function a(c){this.options=c;this.cy=c.cy;this.renderer=c.renderer}a.prototype.run=function(){var c={};this.cy.elements().each(function(d,e){var f=e.group();if(c[f]==null){c[f]=[]
}c[f].push(e.json())});return c};b.cytoscapeweb("exporter","json",a)})(jQuery);(function($,$$){function CySelector(cy,onlyThisGroup,selector){if(cy===undefined||cy.$==null){$$.console.error("A selector must have a reference to the core");return}if(selector===undefined&&onlyThisGroup!==undefined){selector=onlyThisGroup;onlyThisGroup=undefined}var self=this;self._private={selectorText:null,invalid:true,cy:cy};function newQuery(){return{classes:[],colonSelectors:[],data:[],group:onlyThisGroup,ids:[],meta:[],collection:null,filter:null}
...
this.cy=this.options.cy;var u=this.cy.container();var v=u.svg("get");this.container=u;this.svg=v;if(v!=null){u.svg("destroy")}u.css({padding:"0 !important"});u.svg({onLoad:function(y){if(w.scale==null){w.scale=1}if(w.translation==0){w.translation={x:0,y:0}}u.find("svg").css("overflow","hidden");v=y;w.svg=v;w.svg.change();w.svgBg=v.rect(0,0,"100%","100%",{fill:"white",opacity:0});w.edgesGroup=v.group();w.nodesGroup=v.group();w.svgRoot=j(w.nodesGroup).parents("svg:first")[0];w.selectedElements=w.cy.collection();
w.touchingNodes=w.cy.collection();w.defs=w.svg.defs();w.makeBackgroundInteractive();x()},settings:{height:"100%",width:"100%"}})};f.prototype.offsetFix=function(v){var u=this;if(v.offsetX==null||v.offsetY==null){v.offsetX=v.clientX-u.cy.container().offset().left;v.offsetY=v.clientY-u.cy.container().offset().top}};f.prototype.makeBackgroundInteractive=function(){var D=this;var M=D.svgRoot;var O=150;D.shiftDown=false;j(window).bind("keydown keyup",function(R){D.shiftDown=R.shiftKey});function v(R){return R.target==M||j(R.target).parents("g:last")[0]==D.edgesGroup||j(R.target)[0]==D.svgBg
}j(M).bind("mousedown",function(T){if(T.button!=0){return}if(v(T)){T.preventDefault();D.offsetFix(T);var U=null;var S={};var V=false;var X=true;var ac=T.pageX;var aa=T.pageY;var R=T.offsetX;var ae=T.offsetY;var ad=0;var ab=0;var W=setTimeout(function(){if(!D.cy.panning()){return}V=true;X=false;D.svg.change(M,{cursor:h(D.style.global.panCursor)});D.cy.container().scrollLeft(100)},O);var Y=function(ak){clearTimeout(W);var ai=ak.pageX-ac;var ah=ak.pageY-aa;ac=ak.pageX;aa=ak.pageY;ad+=ai;ab+=ah;if(V){D.translation.x+=ai;
D.translation.y+=ah;D.pan(D.translation)}if(X){if(U==null){U=D.svg.rect(R,ae,0,0,{fill:q(D.style.global.selectionFillColor),opacity:b(D.style.global.selectionOpacity),stroke:q(D.style.global.selectionBorderColor),strokeWidth:c(D.style.global.selectionBorderWidth)})}else{var aj=Math.abs(ad);var ag=Math.abs(ab);var af=ad>=0?R:R+ad;var al=ab>=0?ae:ae+ab;S={x1:af,y1:al,x2:af+aj,y2:al+ag};D.svg.change(U,{x:af,y:al,width:aj,height:ag})}}};j(window).bind("mousemove",Y);var Z=function(af){if(af.type=="mouseup"&&af.button!=0){return
}clearTimeout(W);j(window).unbind("mousemove",Y);j(window).unbind("mouseup",Z);j(window).unbind("blur",Z);j(M).unbind("mouseup",Z);if(V){D.svg.change(M,{cursor:null})}if(X){if(U!=null&&S.x1!=null&&!isNaN(S.x1)){D.selectElementsFromIntersection(U,S);D.svgRemove(U)}else{if(!D.shiftDown){D.unselectAll()}}}};j(window).bind("mouseup",Z);j(window).bind("blur",Z);j(M).bind("mouseup",Z)}}).bind("mousewheel",function(W,X,T,S){if(!D.cy.panning()||!D.cy.zooming()){return}D.offsetFix(W.originalEvent);var R={x:W.originalEvent.offsetX,y:W.originalEvent.offsetY};
var U=0.5;if(j.browser.mozilla||j.browser.msie){U=0.167}var V=D.zoom()*(1+S*U);D.zoomAboutPoint(R,V);D.cy.trigger("zoom");D.cy.trigger("pan");W.preventDefault()});function H(U,S){var R,X;var V=D.cy.container().offset();var T=U.originalEvent.touches;var W=T[S];R=W.pageX-V.left;X=W.pageY-V.top;return{x:R,y:X}}function I(S){var T=H(S,0);var R=H(S,1);return{x:(T.x+R.x)/2,y:(T.y+R.y)/2}}function y(S){var T=H(S,0);var R=H(S,1);return D.getDistance(T,R)}function N(R){return R.originalEvent.touches==null?0:R.originalEvent.touches.length
}function x(R,S){return N(R)>=S}function L(R){if(R>=2){P=true;u=true;clearTimeout(w);w=setTimeout(function(){u=false},F)}else{P=false}}var F=100;var P=false;var u=false;var w=null;var J=true;var Q,z,E,B,A,K,G,C;j(M).bind("touchstart",function(R){if(!v(R)||D.touchingNodes.size()>0){return}R.preventDefault();B=H(R,0);if(x(R,2)){Q=I(R);z=D.modelPoint(Q);E=y(R);A=H(R,1);L(2)}else{L(1);J=true}C=false}).bind("touchmove",function(S){if(!v(S)||D.touchingNodes.size()>0){return}J=false;if(x(S,2)){L(2);G=H(S,1)
}else{L(1)}S.preventDefault();var Y={x:0,y:0};if(x(S,1)&&D.cy.panning()){K=H(S,0);if(x(S,2)&&D.cy.zooming()){var R=y(S);var T=R/E;Q=D.renderedPoint(z);if(T!=1){var X=1.5;var W={x:K.x-B.x,y:K.y-B.y};var V={x:G.x-A.x,y:G.y-A.y};Y={x:(W.x+V.x)/2,y:(W.y+V.y)/2};if(T>1){T=(T-1)*X+1}else{T=1-(1-T)*X}var U=D.zoom()*T;D.zoomAboutPoint(Q,U,Y);E=R}}else{if(!u){Y={x:K.x-B.x,y:K.y-B.y};D.panBy(Y)}}B=K;A=G}}).bind("touchend",function(R){if(J&&v(R)){D.unselectAll()}});j(M).bind("mousedown mouseup click mouseover mouseout mousemove touchstart touchmove touchend",function(S){if(v(S)){var R=j.extend({},S,{cyTarget:D.cy});
D.cy.background().trigger(R);D.cy.trigger(R)}})};f.prototype.zoomAboutPoint=function(A,B,u){var C=this;var v=C.cy;if(!v.panning()||!v.zooming()){return}var x=C.pan();var z=C.zoom();var y=B;if(u==null){u={x:0,y:0}}var w={x:-y/z*(A.x-x.x-u.x)+A.x,y:-y/z*(A.y-x.y-u.y)+A.y};C.transform({translation:w,scale:y})};f.prototype.zoom=function(w){var x=this.cy;if(!x.zooming()){return}if(w===undefined){return this.scale}else{if(typeof w==typeof{}){var v=w;var u=v.position;if(v.renderedPosition!==undefined){u=this.modelPoint(v.renderedPosition)
}if(u!==undefined){this.zoomAboutPoint(u,w.level)}else{this.transform({scale:v.level})}}else{this.transform({scale:w})}}};f.prototype.fit=function(D){var u=D.elements;var M=D.zoom;var C=this.cy;if(!C.panning()||(M!==undefined&&!C.zooming())){return}if(u==null||u.size()==0){u=this.cy.elements()}if(u.is(":removed")){m.console.debug("SVG renderer does not take into account removed elements when fitting");u=u.filter(":inside")}m.console.debug("Fit SVG renderer to view bounds");var A=this.nodesGroup.getBBox();
var G=this.edgesGroup.getBBox();var y,J,v,I;function E(P){var O=P.x;var z=O+P.width;var N=P.y;var w=N+P.height;if(O<y||y==null){y=O}if(z>v||v==null){v=z}if(N<J||J==null){J=N}if(w>I||I==null){I=w}}u.nodes().each(function(){var w=this.renderer("svgGroup").getBBox();E(w)});u.edges().each(function(){var z=this.source().id();var N=this.target().id();var w=this.renderer("svg").getBBox();if(z==N){if(w.x==G.x){y=Math.min(w.x+w.width*0.4,A.x)}if(w.y==G.y){J=Math.min(w.y+w.height*0.4,A.y)}}else{w=this.renderer("svgGroup").getBBox();
E(w)}});var K=v-y;var F=I-J;var x=this.cy.container().width();var L=this.cy.container().height();var B=Math.min(x/K,L/F);if(M){this.transform({translation:{x:-y*B-(K*B-x)/2,y:-J*B-(F*B-L)/2},scale:B})}else{var H=this.scale;this.transform({translation:{x:-y*H+x/2-(v-y)/2*H,y:-J*H+L/2-(I-J)/2*H}})}};f.prototype.panBy=function(u){if(!this.cy.panning()){return}m.console.debug("Relatively pan SVG renderer with position (%o)",u);this.transform({translation:{x:this.translation.x+c(u.x),y:this.translation.y+c(u.y)}})
};f.prototype.pan=function(u){if(!this.cy.panning()){return}m.console.debug("Pan SVG renderer with position (%o)",u);if(u===undefined){return{x:this.translation.x,y:this.translation.y}}if(u==null||typeof u!=typeof{}){m.console.error("You can not pan without specifying a proper position object; `%o` is invalid",u);return}this.transform({translation:{x:c(u.x),y:c(u.y)}})};f.prototype.capTransformation=function(z){var u=z.translation;var y=z.scale;var C=this;var v=C.options.maxZoom;var x=C.options.minZoom;
var B=C.options.minPan;var w=C.options.maxPan;var A=true;var D=true;if(u!=null){if(u.x<B){u.x=B;D=false}else{if(u.x>w){u.x=w;D=false}}if(u.y<B){u.y=B;D=false}else{if(u.y>w){u.y=w;D=false}}}else{u=C.translation}if(y!=null){if(y>v){y=v;A=false}else{if(y<x){y=x;A=false}}}else{y=C.scale}return{scale:y,translation:u,valid:A&&D,validScale:A,validTranslation:D}};f.prototype.transform=function(x){var v=this;var u=v.capTransformation(x);if(u.valid){v.translation=u.translation;v.scale=u.scale}else{if(x.capScale){m.console.debug("Capping zoom level %o to %o",v.scale,u.scale);
v.scale=u.scale}if(x.capTranslation){m.console.debug("Capping translation %o to %o",v.translation,u.translation);v.translation=u.translation}}function w(y){if(v.svg==null||y==null){return}v.svg.change(y,{transform:"translate("+v.translation.x+","+v.translation.y+") scale("+v.scale+")"})}w(v.nodesGroup);w(v.edgesGroup)};f.prototype.calculateStyleField=function(w,A){var u=this;var y=u.options.styleCalculator;var v=u.style.selectors;var x=undefined;var z=w.bypass(false)[A];if(z!==undefined){x=z}else{j.each(v,function(B,C){var D=C[A];
if(D!=null&&w.is(B)){x=D}})}return y.calculate(w,x)};f.prototype.calculateStyle=function(x){var u=this;var B=u.options.styleCalculator;var w=u.style.selectors;var y={};j.each(w,function(C,D){if(x.is(C)){y=j.extend(y,D)}});y=j.extend(y,x.bypass(false));j.each(y,function(C,D){y[C]=B.calculate(x,D)});x._private.style=y;if(x.isEdge()){var z=x.source();var A=x.target();function v(){if(z.style("visibility")=="visible"&&A.style("visibility")=="visible"){return r(y.visibility)}else{return"hidden"}}y.visibility=v()
}return y};f.prototype.svgRemove=function(u){var v=j(u);var x=j(this.svgRoot);function w(z){var A=j(z);var y=false;if(A.parent().size()==0){return false}A.parents().each(function(){if(this==x[0]){y=true}});return y}if(u==null||!w(u)){return}this.svg.remove(u)};f.prototype.updateNodePositionFromShape=function(v){var x=v.style(false);var w=v.renderer("svgGroup");var u=v.position(false);o(x.shape).update(this.svg,w,v,u,x)};f.prototype.makeSvgEdgeInteractive=function(y){var v=y.renderer("svg");var z=y.renderer("svgTargetArrow");
var x=y.renderer("svgSourceArrow");var w=j(v).parents("svg:first")[0];var u=this;j(v).add(z).add(x).bind("mouseup mousedown click touchstart touchmove touchend mouseover mousemove mouseout",function(A){if(u.edgeEventIsValid(y,A)){y.trigger(A)}}).bind("click touchend",function(A){u.selectElement(y)})};f.prototype.makeSvgNodeLabelInteractive=function(u){};f.prototype.makeSvgNodeInteractive=function(x){var v=x.renderer("svg");var w=j(v).parents("svg:first")[0];var u=this;var y=null;j(v).bind("mousedown touchstart",function(A){y=false;
x.trigger(A);if(x.grabbed()||x.locked()||!x.grabbable()){A.preventDefault();return}if(A.type=="touchstart"&&A.originalEvent.touches.length>1){return}x._private.grabbed=true;x.trigger(j.extend({},A,{type:"grab"}));u.touchingNodes=u.touchingNodes.add(x);var I,G;if(A.type=="touchstart"){var D=A.originalEvent.touches;var C=D[D.length-1];I=C.pageX;G=C.pageY}else{I=A.pageX;G=A.pageY}var H=true;var E=function(M){y=true;var P,O;if(M.type=="touchmove"){var N=A.originalEvent.touches;var Q=N[N.length-1];P=Q.pageX;
O=Q.pageY}else{P=M.pageX;O=M.pageY}var K=(P-I)/u.zoom();var J=(O-G)/u.zoom();I=P;G=O;var L;if(x.selected()){L=u.selectedElements.add(x)}else{L=x.collection()}L.each(function(R,S){S.element()._private.position.x+=K;S.element()._private.position.y+=J});u.updatePosition(L);if(H){if(M.type!="touchmove"){u.moveToFront(x)}H=false}else{x.trigger(j.extend({},M,{type:"position"}));x.trigger(j.extend({},M,{type:"drag"}))}};j(window).bind("mousemove touchmove",E);var B=false;var z=0;var F=function(J){if(J.type=="touchend"&&J.originalEvent.touches.length!=0){return
}if(!B){B=true}else{return}j(window).unbind("mousemove touchmove",E);j(window).unbind("mouseup touchend blur",F);j(v).unbind("mouseup touchend",F);x._private.grabbed=false;u.touchingNodes=u.touchingNodes.not(x);x.trigger(j.extend({},J,{type:"free"}))};j(window).bind("mouseup touchend blur",F);j(v).bind("mouseup touchend",F);A.preventDefault()}).bind("mouseup touchend",function(z){x.trigger(j.extend({},z));if(y==false){y=null;x.trigger(j.extend({},z,{type:"click"}));u.selectElement(x)}}).bind("mouseover mouseout mousemove",function(z){if(u.nodeEventIsValid(x,z)){x.trigger(j.extend({},z))
...
});this.updateBypass(w)};f.prototype.elementIsVisible=function(u){return u.style(false).visibility!="hidden"};f.prototype.renderedDimensions=function(v){var u=this;if(v.isNode()){return{height:v.style("height")*u.zoom(),width:v.style("width")*u.zoom()}}else{return{width:v.style("width")*u.zoom()}}};f.prototype.unselectElements=function(u){u=u.collection();u.unselect();this.selectedElements=this.selectedElements.not(u)};f.prototype.selectElementsFromIntersection=function(x,z){var v=this;var w=this.cy.collection();
var u=this.cy.collection();function y(F){if(!v.elementIsVisible(F)){return false}var I=o(F.style("shape")).intersectionShape;var M=v.modelPoint({x:z.x1,y:z.y1});var J=v.modelPoint({x:z.x2,y:z.y2});var D=v.svg.rect(M.x,M.y,J.x-M.x,J.y-M.y);var B=Intersection.intersectShapes(new Rectangle(D),new I(F.renderer("svg")));v.svgRemove(D);var Q=v.zoom();var O=F.renderedPosition().x;var L=F.renderedPosition().y;var P=F.renderedDimensions().width+F.style("borderWidth")*Q;var G=F.renderedDimensions().height+F.style("borderWidth")*Q;
...
var u=v.cy.collection();var x=v.cy.collection();if(!v.shiftDown){u=u.add(v.cy.elements().filter(function(y,z){return z.selected()&&!w.same(z)}))}if(v.shiftDown){if(w.selected()){u=u.add(w)}else{x=x.add(w)}}else{if(!w.selected()){x=x.add(w)}}u.unselect();x.select();v.selectedElements=v.selectedElements.not(u);v.selectedElements=v.selectedElements.add(x);v.moveToFront(x)};f.prototype.moveToFront=function(v){v=v.collection();var u=this;v.each(function(x,w){u.svgRemove(w.renderer("svgGroup"));u.makeSvgElement(w);
u.updatePosition(v.closedNeighborhood().edges())})};f.prototype.unselectAll=function(){this.unselectElements(this.cy.elements())};f.prototype.makeSvgNode=function(w){var y=w.position(false);var u=this;if(y.x==null||y.y==null){m.console.debug("SVG renderer is ignoring creating of node `%s` with position (%o, %o)",w.id(),y.x,y.y);return}var v;var x=this.calculateStyle(w);var z=this.svg.group(this.nodesGroup);w.renderer("svgGroup",z);v=o(x.shape).svg(this.svg,z,w,y,x);w.renderer("svg",v);this.makeSvgNodeLabel(w);
w.renderer("svg",v);m.console.debug("SVG renderer made node `%s` with position (%i, %i)",w.id(),y.x,y.y);this.makeSvgNodeInteractive(w);this.updateElementStyle(w,x);return v};f.prototype.makeSvgNodeLabel=function(w){var v=this;var u=w.position("x");var z=w.position("y");w.renderer().svgLabelGroup=v.svg.group(w.renderer().svgGroup);w.renderer().svgLabelOutline=v.svg.text(w.renderer().svgLabelGroup,u,z,"label init");w.renderer().svgLabel=v.svg.text(w.renderer().svgLabelGroup,u,z,"label init")};f.prototype.positionSvgNodeLabel=function(w){var v=this;
var u=w.position("x");var z=w.position("y");v.svg.change(w.renderer().svgLabel,{x:u,y:z});v.svg.change(w.renderer().svgLabelOutline,{x:u,y:z})};f.prototype.makeSvgEdgePath=function(K){var G=this;var ai=K.target();var Q=K.source();var z=ai.data("id")==Q.data("id");var ad;var M=Q.position("x");var av=Q.position("y");var L=ai.position("x");var au=ai.position("y");if(M==L&&av==au){L++;au++}var J=K.parallelEdges();var D=J.size();var aa;var C;var ac=20;var aj=20;var W,T,S;var Z=G.getDistance({x:M,y:av},{x:L,y:au});
var al=200;if(!z&&F){ac=Math.min(20+4000/Z,al)}J.each(function(aw,ax){if(ax==K){aa=aw}});function v(){var aw=C!=0;var ax=G.svg.createPath();if(ad!=null){G.svgRemove(ad)}if(z){ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).curveC(T.x,T.y,S.x,S.y,L,au))}else{if(aw){ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).curveQ(W.x,W.y,L,au))}else{ad=G.svg.path(K.renderer("svgGroup"),ax.move(M,av).line(L,au))}}}if(z){var O=Q.style("height");var I=Q.style("width");ac+=Math.max(I,O);C=aa;ac+=aj*(C);var ap=ac;
T={x:M,y:av-O/2-ap};S={x:M-I/2-ap,y:av};v()}else{var ae=D%2==0;if(ae){C=aa-D/2+(aa<D/2?0:1);if(C>0){C-=0.5}else{C+=0.5}}else{C=aa-Math.floor(D/2)}var F=C!=0;if(Q.id()>ai.id()){C*=-1}if(F){W=T=S=G.getOrthogonalPoint({x:M,y:av},{x:L,y:au},ac*C)}else{W=T={x:L,y:au};S={x:M,y:av}}v()}var u=G.calculateStyleField(K,"width");var af=G.calculateStyleField(ai,"shape");var X=G.calculateStyleField(Q,"shape");var w=G.calculateStyleField(K,"targetArrowShape");var ar=G.calculateStyleField(K,"sourceArrowShape");var an=3;
var A=10;while(an*u<A){an++}var aq=an;var x=aq*u;var af=o(af).intersectionShape;var X=o(X).intersectionShape;var y=Intersection.intersectShapes(new Path(ad),new af(ai.renderer("svg")));var ag=y.points[y.points.length-1];y=Intersection.intersectShapes(new Path(ad),new X(Q.renderer("svg")));var H=y.points[0];var ab=aq*u;var E=-1*(this.getAngle(T,{x:M,y:av})-90);var V=-1*(this.getAngle(S,{x:L,y:au})-90);if(ag!=null){if(w!="none"){var N=G.getPointAlong(ag,S,x/2,ag);L=N.x;au=N.y}else{if(ag!=null){L=ag.x;
au=ag.y}}}if(H!=null){if(ar!="none"){var U=G.getPointAlong(H,T,x/2,H);M=U.x;av=U.y}else{M=H.x;av=H.y}}v();if(K.renderer("svgTargetArrow")!=null){this.svgRemove(K.renderer("svgTargetArrow"))}if(w!="none"){var am=k(w);var Y={x:L-am.centerPoint.x*ab,y:au-am.centerPoint.y*ab,};var B=am.centerPoint;var ao=am==null?null:am.svg(this.svg,K.renderer("svgGroup"),K,K.position(false),K.style(false));K.renderer("svgTargetArrow",ao);this.svg.change(ao,{transform:"translate("+Y.x+" "+Y.y+") scale("+ab+") rotate("+V+" "+B.x+" "+B.y+")"})
}if(K.renderer("svgSourceArrow")!=null){this.svgRemove(K.renderer("svgSourceArrow"))}if(ar!="none"){var R=k(ar);var P={x:M-R.centerPoint.x*ab,y:av-R.centerPoint.y*ab,};var ak=R.centerPoint;var ah=R==null?null:R.svg(this.svg,K.renderer("svgGroup"),K,K.position(false),K.style(false));K.renderer().svgSourceArrow=ah;this.svg.change(ah,{transform:"translate("+P.x+" "+P.y+") scale("+ab+") rotate("+E+" "+ak.x+" "+ak.y+")"})}var at;if(z){at={x:(T.x+S.x)/2*0.85+ai.position("x")*0.15,y:(T.y+S.y)/2*0.85+ai.position("y")*0.15}
}else{if(F){at={x:(W.x+(M+L)/2)/2,y:(W.y+(av+au)/2)/2}}else{at={x:(M+L)/2,y:(av+au)/2}}}K.renderer("svgLabelGroup",G.svg.group(K.renderer().svgGroup));K.renderer("svgLabelOutline",G.svg.text(K.renderer().svgLabelGroup,at.x,at.y,"label init"));K.renderer("svgLabel",G.svg.text(K.renderer().svgLabelGroup,at.x,at.y,"label init"));K.renderer().svg=ad;return ad};f.prototype.markerDrawFix=function(){this.forceRedraw()};f.prototype.forceRedraw=function(){this.svg.change(this.svgRoot,{opacity:0});this.svg.change(this.svgRoot,{opacity:Math.random()});
this.svg.change(this.svgRoot,{opacity:1});var u=this.svg.rect(0,0,this.cy.container().width(),this.cy.container().height());this.svgRemove(u)};f.prototype.getAngle=function(A,z){var v=function(B){return B*180/Math.PI};var y=this.getDistance(A,z);var x=z.x-A.x;var u=-1*(z.y-A.y);var w=v(Math.acos(x/y));if(u<0){return 360-w}else{return w}};f.prototype.getOrthogonalPoint=function(z,y,v){var x={x:z.x-y.x,y:z.y-y.y};var w=this.getNormalizedPoint({x:x.y,y:-x.x},1);var u={x:(z.x+y.x)/2,y:(z.y+y.y)/2};return{x:u.x+w.x*v,y:u.y+w.y*v}
};f.prototype.getPointAlong=function(y,x,w,z){var u={x:x.x-y.x,y:x.y-y.y};var v=this.getNormalizedPoint({x:u.x,y:u.y},1);if(z==null){z=x}return{x:z.x+v.x*w,y:z.y+v.y*w}};f.prototype.getNormalizedPoint=function(x,w){var v=Math.sqrt(x.x*x.x+x.y*x.y);var u=w/v;return{x:x.x*u,y:x.y*u}};f.prototype.getDistance=function(v,u){return Math.sqrt((u.x-v.x)*(u.x-v.x)+(u.y-v.y)*(u.y-v.y))};f.prototype.makeSvgEdge=function(v){var u=this;var x=v.source().element();var z=v.target().element();if(x==null||z==null){m.console.debug("SVG renderer is ignoring creating of edge `%s` with missing nodes");
...
if(u){this.updateMapperBounds(v)}};f.prototype.updateMapperBounds=function(v){var u=cy.elements();if(v.nodes().size()>0&&v.edges().size()>0){}else{u=u.filter(function(){return this.group()==v[0].group()})}u=u.not(v);this.updateElementsStyle(u)};f.prototype.updateElementsStyle=function(v){var u=this;v=v.collection();v.nodes().each(function(x,w){u.updateElementStyle(w)});v.edges().each(function(x,w){u.updateElementStyle(w)});v.nodes().neighborhood().edges().not(v.edges()).each(function(x,w){u.updatePosition(w)
...
var D=A.renderer().oldStyle==null||A.renderer().oldStyle.width!=u.width;A.renderer().oldSourceStyle=z;A.renderer().oldTargetStyle=w;A.renderer().oldStyle=u;if(I!=G||C!=y||F||D){this.svgRemove(A.renderer().svgGroup);this.makeSvgEdge(A);return}this.svg.change(A.renderer().svg,{"pointer-events":"visibleStroke",stroke:q(u.lineColor),strokeWidth:c(u.width),strokeDashArray:e(u.style).array,"stroke-linecap":"butt",opacity:b(u.opacity),cursor:h(u.cursor),fill:"none",visibility:r(u.visibility)});this.svg.change(A.renderer().svgTargetArrow,{fill:q(u.targetArrowColor),cursor:h(u.cursor),visibility:r(u.visibility)});
this.svg.change(A.renderer().svgSourceArrow,{fill:q(u.sourceArrowColor),cursor:h(u.cursor),visibility:r(u.visibility)});var B={visibility:r(u.visibility),"pointer-events":"none",fill:q(u.labelFillColor),"font-family":u.labelFontFamily,"font-weight":u.labelFontWeight,"font-style":u.labelFontStyle,"text-decoration":u.labelFontDecoration,"font-variant":u.labelFontVariant,"font-size":u.labelFontSize};this.svg.change(A.renderer().svgLabel,B);this.svg.change(A.renderer().svgLabelOutline,j.extend({},B,{fill:"none",stroke:q(u.labelOutlineColor),strokeWidth:c(u.labelOutlineWidth)*2,opacity:b(u.labelOutlineOpacity),}));
this.svg.change(A.renderer().svgLabelGroup,{opacity:b(u.labelOpacity)});var v=u.labelText==null?"":u.labelText;A.renderer().svgLabel.textContent=v;A.renderer().svgLabelOutline.textContent=v;this.updateLabelPosition(A,"middle","middle");m.console.debug("SVG renderer collapsed mappers and updated style for edge `%s` to %o",A.id(),u)};f.prototype.addElements=function(w,v){var u=this;var x=this.cy;w.nodes().each(function(z,y){u.makeSvgElement(y)});w.edges().each(function(z,y){u.makeSvgElement(y)});u.positionEdges(w.edges().parallelEdges());
if(v){u.updateMapperBounds(w)}};f.prototype.updatePosition=function(x){m.console.debug("SVG renderer is updating node positions");x=x.collection();var u=this.cy.container();var w=u.svg("get");var v=this;var y=this.options.cy;x.nodes().each(function(B,A){var z=v.getSvgElement(A);var C=A.position(false);v.updateNodePositionFromShape(A);v.positionSvgNodeLabel(A);m.console.debug("SVG renderer is moving node `%s` to position (%o, %o)",A.id(),C.x,C.y)});v.positionEdges(x.closedNeighborhood().edges())};
f.prototype.positionEdges=function(v){var u=this;v.each(function(w,x){if(x.renderer().svgGroup!=null){u.svgRemove(x.renderer().svgGroup)}u.makeSvgEdge(x);var z=x.source().position(false);var y=x.target().position(false);m.console.debug("SVG renderer is moving edge `%s` to position (%o, %o, %o, %o)",x.id(),z.x,z.y,y.x,y.y)})};f.prototype.drawElements=function(v){var u=this;u.updateElementsStyle(v)};f.prototype.removeElements=function(y,x){m.console.debug("SVG renderer is removing elements");var u=this.cy.container();
var w=u.svg("get");var z=this.options.cy;var v=this;y.each(function(B,A){if(A.renderer().svgGroup!=null){w.remove(A.renderer().svgGroup);delete A.renderer().svg;delete A.renderer().svgGroup;delete A.renderer().svgSourceArrow;delete A.renderer().svgTargetArrow}else{m.console.debug("Element with group `%s` and ID `%s` has no associated SVG element",A.group(),A.id())}});if(v.selectedElements!=null){v.selectedElements=v.selectedElements.not(y)}if(x){this.updateMapperBounds(y)}};f.prototype.notify=function(w){var u=this.options.cy.container();
m.console.debug("Notify SVG renderer with params (%o)",w);if(w.type==null){m.console.error("The SVG renderer should be notified with a `type` field");return}var v=this;switch(w.type){case"load":v.init(function(){v.addElements(w.collection)});break;case"add":this.addElements(w.collection,w.updateMappers);break;case"remove":this.removeElements(w.collection,w.updateMappers);break;case"position":this.updatePosition(w.collection);break;case"style":this.updateStyle(w.style);break;case"bypass":this.updateBypass(w.collection);
break;case"class":this.updateClass(w.collection);break;case"data":this.updateData(w.collection,w.updateMappers);break;case"select":case"unselect":this.updateSelection(w.collection);break;case"draw":this.drawElements(w.collection);break;default:m.console.debug("The SVG renderer doesn't consider the `%s` event",w.type);break}};function a(u){this.options=u;this.cy=u.cy;this.renderer=u.renderer;if(this.renderer.name()!="svg"){m.console.error("The SVG exporter can be used only if the SVG renderer is used")
}}a.prototype.run=function(){return this.options.cy.container().svg("get").toSVG()};j.cytoscapeweb("renderer","svg",f);j.cytoscapeweb("exporter","svg",a)})(jQuery,jQuery.cytoscapeweb);(function(b,a){function c(){b.cytoscapeweb("debug","Creating null layout")}c.prototype.run=function(e){b.cytoscapeweb("debug","Running null layout with options (%o)",e);var f=e.cy;f.nodes().positions(function(){return{x:0,y:0}});function d(g){if(g!=null&&typeof g==typeof function(){}){g()}}f.trigger("layoutready");d(e.ready);
f.trigger("layoutstop");d(e.stop)};b.cytoscapeweb("layout","null",c)})(jQuery,jQuery.cytoscapeweb);(function(c,a){var d={fit:true};function b(){c.cytoscapeweb("debug","Creating random layout with options")}b.prototype.run=function(k){var n=c.extend(true,{},d,k);var j=k.cy;var e=j.nodes();var l=j.edges();var f=j.container();c.cytoscapeweb("debug","Running random layout with options (%o)",k);var g=f.width();var m=f.height();c.cytoscapeweb("debug","Random layout found (w, h, d) = (%i, %i)",g,m);e.positions(function(p,o){if(o.locked()){return false
}return{x:Math.round(Math.random()*g),y:Math.round(Math.random()*m)}});function h(o){if(o!=null&&typeof o==typeof function(){}){o()}}j.trigger("layoutready");h(k.ready);if(n.fit){j.fit()}j.trigger("layoutstop");h(k.stop)};c.cytoscapeweb("layout","random",b)})(jQuery,jQuery.cytoscapeweb);(function(b,a){var d={fit:true,rows:undefined,columns:undefined};function c(){b.cytoscapeweb("debug","Creating grid layout")}c.prototype.run=function(z){var j=b.extend(true,{},d,z);var h=z.cy;var v=h.nodes();var g=h.edges();
var r=h.container();b.cytoscapeweb("debug","Running grid layout with options (%o)",j);var u=r.width();var t=r.height();b.cytoscapeweb("debug","Running grid layout on container of size (w, h) = (%i, %i) with %i nodes",u,t,v.size());if(t==0||u==0){b.cytoscapeweb("warn","Running grid layout on container of size 0");v.positions(function(){return{x:0,y:0}})}else{var k=v.size();var p=Math.sqrt(k*t/u);var o=Math.round(p);var q=Math.round(u/t*p);b.cytoscapeweb("debug","Grid layout decided on initial (cols, rows) = (%i, %i)",q,o);
function n(B){if(B==undefined){return Math.min(o,q)}else{var A=Math.min(o,q);if(A==o){o=B;b.cytoscapeweb("debug","Grid layout set small number of rows to %i",o)}else{q=B;b.cytoscapeweb("debug","Grid layout set small number of columns to %i",q)}}}function f(B){if(B==undefined){return Math.max(o,q)}else{var A=Math.max(o,q);if(A==o){o=B;b.cytoscapeweb("debug","Grid layout set large number of rows to %i",o)}else{q=B;b.cytoscapeweb("debug","Grid layout set large number of columns to %i",q)}}}if(j.rows!=null&&j.columns!=null){o=j.rows;
q=j.columns}else{if(j.rows!=null&&j.columns==null){o=j.rows;q=Math.ceil(k/o)}else{if(j.rows==null&&j.columns!=null){q=j.columns;o=Math.ceil(k/q)}else{if(q*o>k){var y=n();var s=f();b.cytoscapeweb("debug","Grid layout is looking to make a reduction");if((y-1)*s>=k){n(y-1)}else{if((s-1)*y>=k){f(s-1)}}}else{b.cytoscapeweb("debug","Grid layout is looking to make an increase");while(q*o<k){var y=n();var s=f();if((s+1)*y>=k){f(s+1)}else{n(y+1)}}}}}}b.cytoscapeweb("debug","Grid layout split area into cells (cols, rows) = (%i, %i)",q,o);
var x=u/q;var e=t/o;var m=0;var l=0;v.positions(function(C,B){if(B.locked()){return false}var A=l*x+x/2;var D=m*e+e/2;l++;if(l>=q){l=0;m++}return{x:A,y:D}})}if(j.fit){h.reset()}function w(A){if(A!=null&&typeof A==typeof function(){}){A()}}h.trigger("layoutready");w(z.ready);h.trigger("layoutstop");w(z.stop)};b.cytoscapeweb("layout","grid",c)})(jQuery,jQuery.cytoscapeweb);(function(c,a){var d={fit:true};function b(){c.cytoscapeweb("debug","Creating preset layout with options")}b.prototype.run=function(m){var k=c.extend(true,{},d,m);
c.cytoscapeweb("debug","Running preset layout with options (%o)",k);var n=m.cy;var j=n.nodes();var h=n.edges();var g=n.container();function l(o){if(k.positions==null){return null}if(k.positions[o._private.data.id]==null){return null}return k.positions[o._private.data.id]}j.positions(function(p,q){var o=l(q);if(q.locked()||o==null){return false}return o});function f(o){if(o!=null&&typeof o==typeof function(){}){o()}}n.trigger("layoutready");f(m.ready);if(k.fit){n.fit()}n.trigger("layoutstop");f(m.stop)
};c.cytoscapeweb("layout","preset",b);function e(f){this.options=f;this.cy=f.cy;this.renderer=f.renderer}e.prototype.run=function(){var f={};this.cy.elements().each(function(g,h){f[h.data("id")]=h.position()});return f};c.cytoscapeweb("exporter","preset",e)})(jQuery,jQuery.cytoscapeweb);(jQuery.color||(function(a){a.color={isEqual:function(b,d){if(b.length!==d.length){return false}var c=b.length;while(c--){if(b[c]!==d[c]){return false}}return true},fix:function(b,d){var c=d.length;while(c--){if(typeof b[c]==="number"){switch(d.charAt(c)){case"i":b[c]=Math.round(b[c]);
